...
', 'left');
  ...
  function isloggeddIn($user){
	...
	function nextRide($driver_id){
		...
		if($driver_id){
			$this->db->select('r.*,  d.first_name driver_name, d.email, d.mobile, d.country_code as driver_country_code, d.gender, d.photo as driver_photo, d.photo driver_photo, c.first_name customer_name, c.mobile customer_mobile, c.country_code customer_country_code,  t.name taxi_name, 	t.color, t.model, t.number, t.type, tt.name types, ti.image, ti.image_hover, ti.mapcar');		
			...
			$q = $this->db->get();
				...
				if($q->num_rows() > 0){
					...
					$data_value = array(
						...
						'mapcar' => $row->mapcar ? $row->mapcar : '0'
					);
	...
	function mapcar($cab_type_id){
		$image_path = base_url('assets/uploads/');
		$this->db->select('tt.taxi_image_id, ti.image, ti.image_hover, ti.mapcar');
		$this->db->from('taxi_type tt');
		$this->db->join('taxi_image ti', 'ti.id = tt.taxi_image_id');
		$this->db->where('tt.id', $cab_type_id);
		$q = $this->db->get();
		if($q->num_rows()>0){
			if($q->row('mapcar') !=''){
				$mapcar = $image_path.$q->row('mapcar');
			}else{
				$mapcar = '';
			}
