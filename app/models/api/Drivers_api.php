...
class drivers_api extends CI_Model
{
	...
	function getTypebyCategory($category_id, $countryCode){
		$q = $this->db->select('id, name')->where('category_id', $category_id)->where('is_country', $countryCode)->get('taxi_type');
		if($q->num_rows()>0){
	...
	function getTaxicategory($countryCode){
		$q = $this->db->select('id, name')->where('is_country', $countryCode)->get('taxi_category');
		if($q->num_rows()>0){
			foreach (($q->result()) as $row) {
                $data[] = $row;
            }
...
ORDER BY distance ASC";
	...
	function driverUpdateStatus($data, $countryCode){
		...
		$u = $this->db->select('u.id as user_id, IFNULL(t.id, 0) as taxi_id, IFNULL(dcs.id, 0) as current_driver_or_taxi_id')->from('users u')->join('taxi t', 't.driver_id = u.id', 'left')->join('driver_current_status dcs', 'dcs.allocated_status != 2 AND dcs.driver_id = u.id AND (dcs.mode = 2 OR dcs.mode = 3)', 'left')->where('u.id', $data['driver_id'])->get();
		...
		if($u->num_rows() > 0){
			...
			if($mode == 1){
				...
				if($status == 1){
					...
					$dcu = $this->db->update('driver_current_status', array('mode' => 0, 'is_connected' => 0, 'is_allocated' => 2, 'allocated_status' => 2, 'allocated_end_date' => date('Y-m-d H:i:s')), array('driver_id' => $data['driver_id'], 'is_country' => $countryCode));
	...
	*/
	...
	function check_login($login, $countryCode){
		...
		$image_path = base_url('assets/uploads/');
		$query = "select u.id, u.oauth_token, u.devices_imei, u.first_name, IFNULL(u.last_name, 0) as last_name, u.photo, u.devices_imei, u.username, u.country_code, u.mobile, u.mobile_otp, u.password, IFNULL(u.ref_mobile, 0) as driver_ref_code, IFNULL(u.ref_driver, 0) as own_ref_code,  u.active from {$this->db->dbprefix('users')} as u 		
	...
	function submitride($data, $countryCode){
		$this->db->select('r.id as id, r.booking_no, r.payment_id, r.payment_name, r.driver_id as driver_id, r.customer_id as customer_id, rp.estimate_distance, rp.estimate_fare, rp.estimate_time, rp.estimate_time_fare, rp.actual_distance, rp.actual_fare, rp.actual_time, rp.actual_time_fare, rp.min_distance, rp.min_time, rp.min_fare, rp.min_time_fare, rp.additional_distance, rp.additional_time, rp.additional_fare, rp.additional_time_fare, rp.labour_charge, rp.driver_allowance, rp.total_night_halt, rp.total_toll, rp.total_parking, rp.promotion_code, rp.promotion_fare, rp.discount_code, rp.discount_fare, rp.total_distance, rp.total_fare, rp.round_fare, rp.extra_fare, rp.extra_fare_details, rp.outstanding_from_last_trip, rp.final_total');
		$this->db->from('rides r');
		...
		if($q->num_rows()>0){
			...
			if($u){
				...
				$row->final_total = number_format((float)$row->final_total, 2, '.', '');
		...
		if($q){
			...
			if($r->num_rows()>0){
				...
				if(!empty($fare_estimate)){
					...
					$min_distance = $fare_estimate['min_distance'];
					$min_time = $fare_estimate['min_time'];
					$min_fare = $fare_estimate['min_fare'];
					$min_time_fare = $fare_estimate['min_time_fare'];
					
					$additional_distance = $fare_estimate['additional_distance'];
					$additional_time = $fare_estimate['additional_time'];
					$additional_fare = $fare_estimate['additional_fare'];
					$additional_time_fare = $fare_estimate['additional_time_fare'];
					
					$total_distance = $fare_estimate['total_distance'];
					$total_fare = $fare_estimate['total_fare'];
					$labour_charge = $fare_estimate['labour_charge'];
				...
				$payment = $this->db->insert('ride_payment', 
					...
					'final_total' => round($total_fare + $data['total_parking'] + $data['total_toll'] + $labour_charge + $extra_fare + $outstanding_from_last_trip - $discount_fare))
			...
			$result = array(
				...
				'estimate_fare' => $estimate_fare,
				'estimate_time_fare' => $estimate_time_fare,
				'actual_distance' => $actual_distance,
				...
				'total_distance' => $total_distance,
				'labour_charge' => $labour_charge,
				'total_fare' => round($total_fare + $data['total_parking'] + $data['total_toll'] + $labour_charge + $extra_fare + $outstanding_from_last_trip - $discount_fare),
				'round_fare' => round($total_fare + $data['total_parking'] + $data['total_toll'] + $labour_charge + $extra_fare + $outstanding_from_last_trip - $discount_fare),
				'extra_fare' => $extra_fare,
				'extra_fare_details' => $extra_fare_details,
				'outstanding_from_last_trip' => $outstanding_from_last_trip,
			);
		...
		if($q->num_rows()>0){
			...
			foreach (($q->result()) as $row) {
				...
				if($row->end_lat != 0 && $row->end_lng != 0){
					$loc[$row->ride_id] = $this->site->GetDrivingDistanceNew($row->start_lat, $row->start_lng,  $row->end_lat, $row->end_lng, 'km', 2, $countryCode);
				}else{
					$loc[$row->ride_id] = '0';
				}
