...
class Customer_api extends CI_Model
{
	...
	function offerCheck($user_id, $offer_code, $estimate_fare, $countryCode){
		$data = array();
		$q = $this->db->select('offer_fare_type, offer_fare, offer_limit, maximum_amount as minimum_amount')->where('offer_code', $offer_code)->where('is_country', $countryCode)->get('offers');
		
		if ($q->num_rows() > 0) {
			$row = $q->row();
			if($row->offer_fare_type == 1){
				$row->discount = $row->offer_fare.'%';
				$row->offer_fare = (string)($estimate_fare * $row->offer_fare) / 100;
			}else{
				$row->discount = $row->offer_fare;
				$row->offer_fare = $row->offer_fare;
			}
			$row->estimate_fare = $estimate_fare;
			if($estimate_fare < $row->minimum_amount){
				$row->check = 3;
				$data = $row;
			}else{
				$d = $this->db->select('COUNT(id) as offer_count')->where('offer_code', $offer_code)->where('user_id', $user_id)->where('is_country', $countryCode)->get('offers_user');
				//$d = $this->db->select('COUNT(id) as offer_count')->where('offer_code', $offer_code)->get('offers_user');
				//echo $d->row('offer_count');
				//echo $row->offer_limit;
				//print_r($this->db->last_query());die;
				if($d->row('offer_count') != 0){
					if($d->row('offer_count') > $row->offer_limit){
						$row->check = 2;
						$data = $row;
					}else{
						$row->check = 1;
	...
	function getOffers($user_id, $countryCode){
		...
		if(!empty($current_date)){
			$this->db->select('id, offer_name, offer_code, offer_description, maximum_amount as minimum_amount, offer_fare_type, offer_fare, date_type, days, start_date, end_date, start_time, end_time');
			...
			$d = $this->db->get('offers');
			
			if ($d->num_rows() > 0) {
			 foreach (($d->result()) as $dow) {
				 $dow->offer_description = strip_tags($dow->offer_description);
				 $dow->offer_description = str_replace("\r", '', $dow->offer_description);
    			 $dow->offer_description = str_replace("\n", ' ', $dow->offer_description);
				 $data[] = $dow;
			 }
			}
		}
		...
		if(!empty($current_day)){
			...
			$this->db->where('is_country', $countryCode);
			$dd = $this->db->get('offers');
			if ($dd->num_rows() > 0) {
			 foreach (($dd->result()) as $ddow) {
				 $ddow->offer_description = strip_tags($ddow->offer_description);
				 $ddow->offer_description = str_replace("\r", '', $ddow->offer_description);
    			 $ddow->offer_description = str_replace("\n", ' ', $ddow->offer_description);
	...
	function outstanding_price($customer_id, $countryCode){
		$q = $this->db->select('SUM(customer_fare) as customer_fare')->where('is_edit', 1)->where('customer_status', 1)->where('customer_id', $customer_id)->get('outstandingfare');
		if ($q->num_rows() > 0) {
...
$this->db->where('is_country', $countryCode);
	...
	function get_driver_location($user_id, $countryCode){
		$myQuery = "SELECT d.id, IFNULL(dcs.current_latitude, 0) as lat, IFNULL(dcs.current_longitude, 0) as lng,  r.start_lat as pickup_lat, r.start_lng as pickup_lng, r.end_lat as drop_lat, r.end_lng as drop_lng, IFNULL(r.middle, 0) as middle, IFNULL(r.middle_lat, 0) as middle_lat, IFNULL(r.middle_lng, 0) as middle_lng, r.change_type, r.distance_km, r.customer_id, r.id AS ride_id, IFNULL(df.location, 0) as location, IFNULL(df.final_distance, 0) as final_distance, IFNULL(df.final_distance_total, 0) as final_distance_total, r.status, r.cab_type_id, r.booked_type,  us.socket_id, us.id AS usid FROM kapp_users AS d  LEFT  JOIN kapp_rides AS r ON r.customer_id = d.id AND (r.status = 2 OR  r.status = 3) LEFT  JOIN kapp_driver_frequency AS df ON df.ride_id = r.id AND df.driver_id = d.id LEFT  JOIN kapp_user_socket AS us ON us.user_id = r.customer_id AND us.user_type = 1 LEFT JOIN kapp_driver_current_status dcs ON dcs.driver_id = r.driver_id AND dcs.is_connected = 1 AND (dcs.mode = 2 OR dcs.mode = 3) 
		...
		if ($q->num_rows() > 0) {
			...
			$row->waypoint_data = $this->site->getWaypoint($row->ride_id);
			$row->actual_lat = 0;
			$row->actual_lng = 0;
			$distance_km = $row->distance_km;
			
			$distance_array = $this->site->getFareestimate($row->pickup_lat, $row->pickup_lng, $row->cab_type_id, $row->booked_type, $countryCode);
			
			$row->mapcar = $this->site->mapcar($row->cab_type_id);
			...
			$row->final_distance_total = $distance_price ? number_format((float)$distance_price, 2, '.', '') : '0.00';
			
			
			$s = $this->site->get_setting($countryCode);
				$row->estimate_fare_enable = $s->estimate_fare_enable;
	...
	/*$this->db->select('cab_type_id, booked_type, start, start_lat, start_lng, end, end_lat, end_lng, change_type, middle, middle_lat, middle_lng');
			$this->db->where('id', $data['booking_id']);
			$r = $this->db->get('rides');
			if($r->num_rows()>0){
				
		   		$row =  $r->row();
				$distance_array = $this->site->getFareestimate($row->start_lat, $row->start_lng, $row->cab_type_id, $row->booked_type, $countryCode);
				...
				$row->distance_price = $distance_price ? number_format((float)$distance_price, 2, '.', '') : '0.00';
				*/
	...
	function getOutstationPackagetype($from_city, $to_city, $countryCode){
		...
		$myQuery = "SELECT O.id, O.from_city_id, O.to_city_id, O.taxi_type, O.is_oneway, O.is_twoway, O.package_name, O.oneway_package_price, O.twoway_package_price, O.min_per_distance, O.min_per_distance_price, O.per_distance, O.per_distance_price, O.driver_allowance_per_day, O.driver_night_per_day, O.is_default, TT.name as taxi_type_name, TI.image, TI.image_hover, TI.mapcar, TT.outstation_image FROM {$this->db->dbprefix('outstation_fare')} AS O JOIN {$this->db->dbprefix('taxi_type')} AS TT ON TT.id = O.taxi_type JOIN {$this->db->dbprefix('taxi_image')} AS TI ON TI.id = TT.taxi_image_id WHERE O.from_city_id = ".$from_city."  AND O.to_city_id = ".$to_city." AND O.is_country = '".$countryCode."' ";
		...
		if ($q->num_rows() > 0) {
			...
			$myQuery_default = "SELECT O.id, O.from_city_id, O.to_city_id, O.taxi_type, O.is_oneway, O.is_twoway, O.package_name, O.oneway_package_price, O.twoway_package_price, O.min_per_distance, O.min_per_distance_price, O.per_distance, O.per_distance_price, O.driver_allowance_per_day, O.driver_night_per_day, O.is_default, TT.name as taxi_type_name, TI.image, TI.image_hover, TI.mapcar, TI.outstation_image FROM {$this->db->dbprefix('outstation_fare')} AS O JOIN {$this->db->dbprefix('taxi_type')} AS TT ON TT.id = O.taxi_type JOIN {$this->db->dbprefix('taxi_image')} AS TI ON TI.id = TT.taxi_image_id WHERE O.from_city_id = 0  AND O.to_city_id = 0 AND O.is_country = '".$countryCode."' ";
	...
	function getRentalPackagetype($city_id, $package_name, $countryCode){
		...
		$myQuery = "SELECT R.id, R.city_id, R.taxi_type, R.per_distance, R.per_distance_price, R.per_time, R.per_time_price, R.option_type, R.option_price, R.package_name, R.package_price, R.time_type, TT.name as taxi_type_name, TI.image, TI.image_hover, TI.mapcar, TI.outstation_image FROM {$this->db->dbprefix('rental_fare')} AS R JOIN {$this->db->dbprefix('taxi_type')} AS TT ON TT.id = R.taxi_type JOIN {$this->db->dbprefix('taxi_image')} AS TI ON TI.id = TT.taxi_image_id  WHERE R.package_name = '".$package_name."' AND R.is_country = '".$countryCode."' GROUP BY  R.taxi_type ";
		$q = $this->db->query($myQuery);
	...
	function FaregetAllTaxiTypesnew($latitude, $longitude, $distance, $countryCode){
		...
		if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
				...
				$query1 = "SELECT  d.id as available, dcs.current_latitude, dcs.current_longitude, ( 6371 * acos( cos( radians({$latitude}) ) * cos( radians( dcs.current_latitude ) ) * cos( radians( dcs.current_longitude ) - radians({$longitude}) ) + sin( radians({$latitude}) ) * sin( radians( dcs.current_latitude ) ) ) ) AS distance FROM {$this->db->dbprefix('users')}  AS d 
	LEFT JOIN {$this->db->dbprefix('driver_current_status')} AS dcs ON dcs.driver_id = d.id  
	
	LEFT JOIN {$this->db->dbprefix('taxi')} AS t ON t.id = dcs.taxi_id  
	LEFT JOIN {$this->db->dbprefix('taxi_services')} AS ts ON ts.taxi_id = dcs.taxi_id  AND ts.user_id = d.id AND ts.status = 1
	JOIN {$this->db->dbprefix('taxi_type')} AS tt ON tt.id = ts.taxi_type 
	LEFT JOIN {$this->db->dbprefix('groups')} AS g ON g.id = d.group_id 
	
	LEFT JOIN {$this->db->dbprefix('user_setting')} AS us ON us.user_id = d.id  
	WHERE tt.id = ".$row->id." AND (us.ride_stop = 1 OR us.ride_stop = 0)   AND (dcs.mode = 1 OR dcs.mode = 3)  AND dcs.is_connected = 1  GROUP BY d.id   HAVING distance <= {$distance}  
ORDER BY distance ASC";
				...
				if ($t->num_rows() > 0) {
					...
					foreach($t->result() as $trow){
						
							if($i == 1){
								 $time = $this->site->GetDrivingDistance_waypoints($latitude, $longitude, $trow->current_latitude, $trow->current_longitude, 'Time', $countryCode);
								 $row->distance_duration = $time;	
							}
							$trow->end_lat = $trow->status == 3 ? $trow->end_lat : '0';		
							$trow->end_lng = $trow->status == 3 ? $trow->end_lng : '0';	

							$r = $this->db->select('id, end_lat, end_lng')->from('rides')->where('status', 3)->where('driver_id', $trow->available)->order_by('id', 'DESC')->limit(1)->get();
							if ($r->num_rows() > 0) {
								if($r->row('end_lat') != 0 && $r->row('end_lng') != 0){
									$dis = $this->site->GetDrivingDistance_New($latitude, $longitude, $r->row('end_lat'), $r->row('end_lng'), $row->id, $countryCode);
									if(round($dis) > 3){
										$available[$row->id][] = $trow->available;					
									}
								}
							}else{
								$available[$row->id][] = $trow->available;	
					...
					$row->driver_data = $available[$row->id];
					
					if(count($available[$row->id]) != 0){
						$row->available = count($available[$row->id]);
						$data[] = $row;
					}
				}
	...
	function checkMobile($mobile, $country_code, $countryCode){
		...
		$q = $this->db->select('*')->where('mobile', $mobile)->where('country_code', $country_code)->where('group_id', 5)->get('users');
		if($q->num_rows()>0){
			return 1;
		}
	...
	function getCategorytypes($countryCode){
		...
		if($c->num_rows()>0){
			...
			foreach (($c->result()) as $cow) {
				$data[] = $cow;
				$q = $this->db->select('tt.id, tt.name, ti.image, ti.image_hover, ti.mapcar')->from('taxi_type tt')->join('taxi_image ti', 'ti.id=tt.taxi_image_id', 'left')->where('tt.is_country', $countryCode)->where('tt.category_id', $cow->id)->where('tt.status', 1)->get();	
				if($q->num_rows()>0){
	...
	function  mypastrides($customer_id, $countryCode,  $sdate, $edate){
		...
		if($q->num_rows()>0){
			...
			foreach (($q->result()) as $row) {
				...
				$row->sos = "https://35.154.46.42/sos?id=".$row->ride_id;
	...
	function add_customer($customer, $countryCode, $refer_code, $reference_no){
	...
	function registerresendotp($data, $countryCode){
		...
		$query = "select * from {$this->db->dbprefix('users')} where mobile='".$data['mobile']."' AND country_code='".$data['country_code']."' AND group_id = 5  ";
		$q = $this->db->query($query);
		if($q->num_rows()>0){
	...
	function forgototp($data, $countryCode){
		...
		$query = "select * from {$this->db->dbprefix('users')} where mobile='".$data['mobile']."' AND group_id = 5  ";
		$q = $this->db->query($query);
		if($q->num_rows()>0){
	...
	function forgotcheckotp($data, $countryCode){
		...
		$query = "select * from {$this->db->dbprefix('users')} where id='".$data['customer_id']."' AND  forgot_otp='".$data['forgot_otp']."'  ";
		$q = $this->db->query($query);
		if($q->num_rows()>0){
	...
	function forgotresendotp($data, $countryCode){
		...
		$query = "select * from {$this->db->dbprefix('users')} where id='".$data['customer_id']."' AND group_id = 5  ";
		$q = $this->db->query($query);
		if($q->num_rows()>0){
			//$this->db->where('id', $q->row('id'));
			//$this->db->update('users', array('forgot_otp' => $data['forgot_otp'], 'forgot_otp_verify' => 0));
	...
	function social_check_login($login, $countryCode){
		...
		$query = "select u.id, u.oauth_token, u.devices_imei, u.first_name, IFNULL(u.last_name, 0) as last_name, u.photo, u.username, u.country_code, u.mobile, u.mobile_otp, u.active from {$this->db->dbprefix('users')} as u 		
	...
	function check_login($login, $countryCode){
		...
		$image_path = base_url('assets/uploads/');
		//$query = "select u.id, u.oauth_token, u.first_name, IFNULL(u.last_name, 0) as last_name, u.photo, u.devices_imei, u.username, u.country_code, u.mobile, u.mobile_otp, u.active from {$this->db->dbprefix('users')} as u 	where u.password='".$login['password']."' AND  u.mobile='".$login['mobile']."' AND  u.country_code='".$login['country_code']."'  AND u.group_id = 5 AND u.is_country ='".$countryCode."' ";
		
		$query = "select u.id, u.oauth_token, u.first_name, IFNULL(u.last_name, 0) as last_name, u.photo, u.devices_imei, u.username, u.country_code, u.mobile, u.mobile_otp, u.active, u.password from {$this->db->dbprefix('users')} as u 	where u.mobile='".$login['mobile']."' AND  u.country_code='".$login['country_code']."'  AND u.group_id = 5  ";
		...
		if($q->num_rows()>0){
			...
			if ($row->password == $login['password']) {
				if($row->active == 1){
					$row->check_ride = $this->site->checkRide($row->id, 1);
...
//(dcs.mode = 1 OR dcs.mode = 3)   AND dcs.is_connected = 1  AND dcs.allocated_status = 1 
	...
	function driverLoc($driver_id){
		$this->db->select('current_latitude, current_longitude');
		$this->db->from('driver_current_status');
	...
	function onrideCustomerCancel($data, $countryCode){
		...
		$this->db->where('id', $data['booking_id']);
		//$this->db->where('is_country', $countryCode);
		$q = $this->db->update('rides', array('cancel_status' => 2, 'cancel_id' => $data['cancel_id'], 'cancel_msg' => $data['cancel_msg'], 'cancelled_by' => $data['customer_id'], 'cancelled_type' => 2, 'status' => 9, 'actual_loc' => $data['actual_loc'], 'actual_lat' => $data['actual_lat'], 'actual_lng' => $data['actual_lng'], 'travel_distance' => $distance_kilometer, 'driver_final_distance' => $distance_meter, 'ride_timing_end' => date('Y-m-d H:i:s')));
		if($q){
			...
			if($r->num_rows()>0){
				...
				$admin_percentage = $setting->driver_default_set_payment == 0 ? $setting->driver_admin_payment_percentage : 0;
				...
				$payment = $this->db->insert('ride_payment', 
					...
					'final_total' => round($total_fare + $data['total_parking'] + $data['total_toll'] + $extra_fare + $outstanding_from_last_trip - $discount_fare))
					);
			...
			$result = array(
				...
				'promotion_fare' => $discount_fare, 
				'total_distance' => $total_distance,
				'total_fare' => round($total_fare + $data['total_parking'] + $data['total_toll'] + $extra_fare + $outstanding_from_last_trip - $discount_fare),
				'round_fare' => round($total_fare + $data['total_parking'] + $data['total_toll'] + $extra_fare + $outstanding_from_last_trip - $discount_fare),
				'extra_fare' => $extra_fare,
				'extra_fare_details' => $extra_fare_details,
				'outstanding_from_last_trip' => $outstanding_from_last_trip,
			);
	...
	function customerCancel($data, $countryCode){
			...
			$this->db->where('id', $data['booking_id']);
			$this->db->where('is_country', $countryCode);
			$q = $this->db->update('rides', array('cancel_status' => 1, 'cancel_id' => $data['cancel_id'], 'cancel_msg' => $data['cancel_msg'], 'cancelled_by' => $data['customer_id'], 'cancelled_type' => 2, 'cancel_on' => $data['cancel_on'], 'cancel_free_status' => $cancel_free_status, 'status' => 6));
			if($q){
				...
				if($r->num_rows()>0){
					...
					$customer_cancel_charge = $total_fare;
					$customer_cancel_final_km = $final_km;
				}else{
					$driver_final_distance = $this->site->CancellationDistance($data['booking_id'], $countryCode);
					...
					if($data['customer_cancel_charge'] > $data['cancel_maximum_fare']){
						$customer_cancel_charge = $data['cancel_maximum_fare'];
					}else{
						$customer_cancel_charge = $data['customer_cancel_charge'];
					}
				...
				if($data['check_cancel_free_second'] == 1 && $data['ride_started'] == 1){
					if($data['check_cancel_limit'] > $data['no_of_customer_cancel']){
						$this->db->insert('outstandingfare', array('customer_id' => $data['customer_id'], 'customer_created' => date('Y-m-d H:i:s'), 'is_edit' => 1, 'customer_status' => 1, 'customer_fare' => $customer_cancel_charge));
					}
				}
				...
				$data_array =  array(
					"booking_id" => $data['booking_id'],
					"customer_cancel_charge" => $customer_cancel_charge,
					"customer_cancel_final_km" => $customer_cancel_final_km !='' ? $customer_cancel_final_km : 0,
					"countryCode" => $countryCode,
				);
	...
	function checkbookedcustomer($data, $countryCode){
		...
		$check = $this->db->select('*')->where('customer_id', $data['customer_id'])->where_in('status', array('1', '2', '3', '10'))->where('cancel_status', 0)->where('is_country', $countryCode)->get('rides');
	...
	function customerChangeDrop($data, $distance_km, $waypoint_array, $countryCode){
		$this->db->where('id', $data['booking_id']);
		$this->db->where('is_country', $countryCode);
		$q = $this->db->update('rides', array('end' => $data['change_drop'], 'distance_km' => $distance_km, 'end_lat' => $data['change_drop_lat'], 'end_lng' => $data['change_drop_lng'], 'middle' => $data['end'], 'middle_lat' => $data['end_lat'], 'middle_lng' => $data['end_lng'], 'change_pick' => $data['change_pickup'], 'change_pick_lat' => $data['change_pickup_lat'], 'change_pick_lng' => $data['change_pickup_lng'], 'change_type' => $data['change_type']));
		if($q){
			if(!empty($waypoint_array)){
				$this->db->delete('ride_waypoints', array('ride_id' => $data['booking_id']));

				foreach($waypoint_array as $waypoint){
				$waypoint['ride_id'] = $data['booking_id'];				
				$this->db->insert('ride_waypoints', $waypoint);				
				}
			}else{
				$this->db->delete('ride_waypoints', array('ride_id' => $data['booking_id']));
