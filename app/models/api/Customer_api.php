	...
	function getTruckRental($city_id, $cab_type_id, $countryCode){
		...
		if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
				...
				$package_time = explode(':',$row->package_time);
				$res = array(
					'package_id' => $row->package_id,
					'package_name' => $row->package_name,
					'package_price' => $row->package_price,
					'package_distance' => $row->package_distance,
					'package_time' => $package_time[0],
					'per_distance_price' => $row->per_distance_price,
					'per_time_price' => $row->per_time_price
				);
...
ORDER BY distance ASC LIMIT 1";
	...
	function FaregetAllCatgeorytaxitype($latitude, $longitude, $category_id, $distance, $countryCode){
		
		if($category_id != 0){
			$where = " AND tt.category_id = ".$category_id."";
		}
		$query = "SELECT tt.id, tt.category_id, tc.name as category_name, tt.name, tt.taxi_image_id, ti.image, ti.image_hover, ti.mapcar  FROM {$this->db->dbprefix('taxi_type')}  AS tt 
		  ...
		  JOIN {$this->db->dbprefix('taxi_category ')} AS tc ON tc.id = tt.category_id
		 
		 WHERE tt.is_country = '".$countryCode."' ".$where." GROUP BY tt.id
		";
		...
		if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
				...
				$query1 = "SELECT  COUNT(d.id) as available, dcs.current_latitude, dcs.current_longitude, ( 6371 * acos( cos( radians({$latitude}) ) * cos( radians( dcs.current_latitude ) ) * cos( radians( dcs.current_longitude ) - radians({$longitude}) ) + sin( radians({$latitude}) ) * sin( radians( dcs.current_latitude ) ) ) ) AS distance FROM {$this->db->dbprefix('users')}  AS d 
	LEFT JOIN {$this->db->dbprefix('driver_current_status')} AS dcs ON dcs.driver_id = d.id  
...
ORDER BY distance ASC LIMIT 1";
				...
				if ($t->num_rows() > 0) {
					$row->available = $t->row('available');		
					$row->current_longitude = $t->row('current_longitude');		
					$row->current_latitude = $t->row('current_latitude');					
...
ORDER BY distance ASC LIMIT 1";
	...
	public function getShifting($countryCode){
		$image_path = base_url('assets/uploads/');
		$q = $this->db->select('id, name, image')->where('is_country', $countryCode)->where('status', 1)->get('shifting');	
		//print_r($this->db->last_query());die;
		if ($q->num_rows() > 0) {
			foreach (($q->result()) as $row) {
	...
	public function getPaymentmode(){
		...
		//$q = $this->db->select('id, name, image')->where('name !=', 'Payment Gateway')->where('status', 1)->get('payment_mode');
		$q = $this->db->select('id, name, image')->where('status', 1)->get('payment_mode');	
		if ($q->num_rows() > 0) {
			foreach (($q->result()) as $row) {
				
				if($row->image == ''){
					$row->image = $image_path.'uploads/no_image.png';
				}else{
					$row->image = $image_path.$row->image;
				}
	...
	function getCategorytypes($countryCode){
		...
		$c = $this->db->select("id, name, 'types'")->where('is_country', $countryCode)->where('status', 1)->get('taxi_category');
	...
	WHERE d.is_country = '".$countryCode."' AND  (dcs.mode = 1 OR dcs.mode = 3)   AND dcs.is_connected = 1 AND dcs.allocated_status = 1  GROUP BY d.id  HAVING distance <= {$data['distance']} 
...
ORDER BY distance ASC";
	...
	function add_booking($insert, $ride_insert, $ride_type, $ride_timing, $countryCode, $customer_id, $offer_code){
