...
class Vendor_api extends CI_Model
{
	...
	*/
	...
	function notallocatedtaxi($user_id){
		...
		LEFT JOIN {$this->db->dbprefix('taxi_type')} as tt ON tt.id = t.type where t.vendor_id='".$user_id."' AND t.id NOT IN (SELECT taxi_id FROM {$this->db->dbprefix('driver_current_status')} WHERE mode = 0) ";
