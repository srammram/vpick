...
class Verification_model extends CI_Model
{
	...
	function getUserDocument($id){
		$image_path = base_url('assets/uploads/');
		$query = "select ud.id as document_id, ud.aadhaar_no, ud.aadhaar_image, ud.aadhar_verify,  ud.pancard_no, ud.pancard_image, ud.pancard_verify, ud.license_image, ud.license_no, ud.license_dob, ud.license_ward_name, ud.license_type, ud.license_country_id,  ud.license_issuing_authority, ud.license_issued_on, ud.license_validity, ud.license_verify, ud.loan_doc, ud.loan_information, ud.loan_verify, ud.police_image, ud.police_on, ud.police_til, ud.police_verify, ud.badge_no, ud.badge_image, ud.badge_on, ud.badge_til, ud.badge_verify, ud.training_certificate_no, ud.training_certificate_image, ud.training_certificate_on, ud.training_certificate_til, ud.training_certificate_verify, ud.experience_certificate_no, ud.experience_certificate_image, ud.experience_certificate_on, ud.experience_certificate_til, ud.experience_certificate_verify, ud.medical_certificate_no, ud.medical_certificate_image, ud.medical_certificate_on, ud.medical_certificate_til, ud.medical_certificate_verify, ud.health_insurance_no, ud.health_insurance_image, ud.health_insurance_on, ud.health_insurance_til, ud.health_insurance_verify, ud.term_insurance_no, ud.term_insurance_image, ud.term_insurance_on, ud.term_insurance_til, ud.term_insurance_verify, ud.additional_contact_one_type, ud.additional_contact_one_name, ud.additional_contact_one_phone, ud.additional_contact_one_phonecode, ud.additional_contact_two_type, ud.additional_contact_two_name, ud.additional_contact_two_phone, ud.additional_contact_two_phonecode, ud.additional_contact_verify, up.first_name  from {$this->db->dbprefix('user_document')} as ud 
		LEFT JOIN {$this->db->dbprefix('user_profile')}  as up ON up.user_id = ud.user_id AND up.is_edit = 1 
		where ud.user_id='".$id."'  ORDER BY ud.id DESC LIMIT 1  ";
		...
		if($q->num_rows()>0){
			...
			if($row->badge_image !=''){
				$row->badge_image_img = $image_path.$row->badge_image;
			}else{
				$row->badge_image_img = $image_path.'no_image.png';
			}
			
			if($row->training_certificate_image !=''){
				$row->training_certificate_image_img = $image_path.$row->training_certificate_image;
			}else{
				$row->training_certificate_image_img = $image_path.'no_image.png';
			}
			
			if($row->experience_certificate_image !=''){
				$row->experience_certificate_image_img = $image_path.$row->experience_certificate_image;
			}else{
				$row->experience_certificate_image_img = $image_path.'no_image.png';
			}
			
			if($row->medical_certificate_image !=''){
				$row->medical_certificate_image_img = $image_path.$row->medical_certificate_image;
			}else{
				$row->medical_certificate_image_img = $image_path.'no_image.png';
			}
			
			if($row->health_insurance_image !=''){
				$row->health_insurance_image_img = $image_path.$row->health_insurance_image;
			}else{
				$row->health_insurance_image_img = $image_path.'no_image.png';
			}
			
			if($row->term_insurance_image !=''){
				$row->term_insurance_image_img = $image_path.$row->term_insurance_image;
			}else{
				$row->term_insurance_image_img = $image_path.'no_image.png';
			}
	...
	function getTaxi($id){
		$image_path = base_url('assets/uploads/');
		$query = "select t.name, t.make_id, t.is_daily, t.is_country, t.is_rental, t.board, t.is_outstation, t.is_hiring, t.is_corporate, t.model, t.number, t.type, t.is_verify, t.status, t.engine_number, t.chassis_number, t.make, t.fuel_type, t.color, t.manufacture_year, t.capacity, t.photo, up.first_name   from {$this->db->dbprefix('taxi')} as t 
	...
	function getTaxiDocument($id){
		$image_path = base_url('assets/uploads/');
		$query = "select td.id as document_id, td.taxi_id, td.user_id, td.group_id, td.reg_image, td.reg_date, td.reg_due_date, td.reg_owner_name, td.reg_owner_address, td.reg_verify,  td.taxation_image, td.taxation_amount_paid, td.taxation_due_date, td.taxation_verify,  td.insurance_image, td.insurance_policy_no, td.insurance_due_date, td.insurance_verify,  td.permit_image, td.permit_no, td.permit_due_date, td.permit_verify,  td.authorisation_image, td.authorisation_no, td.authorisation_due_date, td.authorisation_verify,  td.fitness_image, td.fitness_due_date, td.fitness_verify, td.speed_image, td.speed_due_date, td.speed_verify,  td.puc_image, td.puc_due_date, td.puc_verify, td.emission_norms, td.emission_norms_on, td.emission_norms_til, td.emission_norms_verify, td.vehicle_tracking_image, td.vehicle_tracking_on, td.vehicle_tracking_til, td.vehicle_tracking_verify, td.fire_extinguisher_image, td.fire_extinguisher_on, td.fire_extinguisher_til, td.fire_extinguisher_verify, td.child_lock_mechanism_image, td.child_lock_mechanism_on, td.child_lock_mechanism_til, td.child_lock_mechanism_verify, td.interior_vehicle_image, td.interior_vehicle_on, td.interior_vehicle_til, td.interior_vehicle_verify, td.taxi_roof_sign_image, td.taxi_roof_sign_on, td.taxi_roof_sign_til, td.taxi_roof_sign_verify, td.e_challans_clearance_image, td.e_challans_clearance_on, td.e_challans_clearance_til, td.e_challans_clearance_verify  from {$this->db->dbprefix('taxi_document')} as td 
		...
		if($q->num_rows()>0){
			...
			if($row->vehicle_tracking_image !=''){
				$row->vehicle_tracking_image_img = $image_path.$row->vehicle_tracking_image;
			}else{
				$row->vehicle_tracking_image_img = $image_path.'no_image.png';
			}
			if($row->fire_extinguisher_image !=''){
				$row->fire_extinguisher_image_img = $image_path.$row->fire_extinguisher_image;
			}else{
				$row->fire_extinguisher_image_img = $image_path.'no_image.png';
			}
			if($row->child_lock_mechanism_image !=''){
				$row->child_lock_mechanism_image_img = $image_path.$row->child_lock_mechanism_image;
			}else{
				$row->child_lock_mechanism_image_img = $image_path.'no_image.png';
			}
			if($row->interior_vehicle_image !=''){
				$row->interior_vehicle_image_img = $image_path.$row->interior_vehicle_image;
			}else{
				$row->interior_vehicle_image_img = $image_path.'no_image.png';
			}
			if($row->taxi_roof_sign_image !=''){
				$row->taxi_roof_sign_image_img = $image_path.$row->taxi_roof_sign_image;
			}else{
				$row->taxi_roof_sign_image_img = $image_path.'no_image.png';
			}
			if($row->e_challans_clearance_image !=''){
				$row->e_challans_clearance_image_img = $image_path.$row->e_challans_clearance_image;
			}else{
				$row->e_challans_clearance_image_img = $image_path.'no_image.png';
			}
	...
	function update_taxi_status($id, $data, $status, $countryCode){
		...
		//print_r($this->db->last_query());die;
		if($q){
			$q = $this->db->select('id')->where('taxi_id', $id)->get('driver_current_status');
			if($q->num_rows()>0){
				$this->db->where('taxi_id', $id);			
				$this->db->update('driver_current_status', $status);
			}else{
				$this->db->insert('driver_current_status', $status);
			}
			return true;
		}
		return false;
