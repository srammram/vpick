...
class Sales_model extends CI_Model
{
    ...
    {        
		$limit = 5;
        $this->db->select('recipe.*, recipe_categories.id as category_id, recipe_categories.name as category_name', FALSE)
    ...
    {
        $this->db->select('recipe.id as id, recipe_combo_items.item_code as code,  recipe.name as name,recipe.type as type')
    ...
    {
      	$this->db->select("recipe_addon.*")->where('recipe_addon.recipe_id', $recipe_id);
 ...
 public function getAllInvoiceItems($sale_id, $return_id = NULL)
    {
        $this->db->select('bil_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.image, products.details as details, product_variants.name as variant, products.hsn_code as hsn_code')
            ->join('recipe', 'recipe.id=bil_items.recipe_id', 'left')
            ->join('recipe_products', 'recipe_products.recipe_id=recipe.id', 'left')
            ->join('products', 'products.id=recipe_products.product_id', 'left')
            ->join('product_variants', 'product_variants.id=bil_items.option_id', 'left')
            ->join('tax_rates', 'tax_rates.id=bil_items.tax_rate_id', 'left')
            ->group_by('bil_items.id')
            ->order_by('id', 'asc');
        if ($sale_id && !$return_id) {
            $this->db->where('bil_id', $sale_id);
        } elseif ($return_id) {
            $this->db->where('bil_id', $return_id);
        }
    ...
    {
        $q = $this->db->get_where('bils', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getInvoicePayments($sale_id)
    {
        ...
        $this->db->order_by('id', 'asc');
        $q = $this->db->get_where('bils', array('sales_id' => $sale_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
//        $query = 'select *,SUM(DISTINCT CASE
