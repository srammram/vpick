...
class booking_crm_model extends CI_Model
{
	...
	WHERE d.is_country = '".$countryCode."' AND (dcs.mode = 1 OR dcs.mode = 3)   AND dcs.is_connected = 1     ".$where." GROUP BY d.id HAVING distance <= {$data['distance']} 
...
//AND (dcs.mode = 1 OR dcs.mode = 3)   AND dcs.is_connected = 1  AND dcs.allocated_status = 1
	...
	function checkbookedcustomer($data, $countryCode){
		...
		$check = $this->db->select('*')->where('customer_id', $data['customer_id'])->where_in('status', array('2,3'))->where('cancel_status', 0)->where('is_country', $countryCode)->get('rides');
