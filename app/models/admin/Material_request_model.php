...
class Material_request_model extends CI_Model
{
    ...
    public function getAllQuotationItems($quotation_id)
    {
       

        $this->db->select('quote_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.unit, products.details as details, product_variants.name as variant, products.hsn_code as hsn_code')
            ...
            ->order_by('id', 'asc');
        $q = $this->db->get_where('quote_items', array('quote_id' => $quotation_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function addMaterialRequest($data, $items)
    {
        if ($this->db->insert('material_request', $data)) {
            ...
            foreach ($items as $item) {
                $item['material_request_id'] = $material_request_id;
    ...
    public function updateMaterialRequest($id, $data, $items = array())
    {
        

        $opurchase = $this->getMaterialRequestByID($id);
        $oitems = $this->getAllQuotationItems($id);
        if ($this->db->update('material_request', $data, array('id' => $id)) && $this->db->delete('material_request_items', array('material_request_id' => $id))) {
            $quote_id = $id;
            foreach ($items as $item) {
                ...
                //     $this->updateAVCO(array('product_id' => $item['product_id'], 'warehouse_id' => $item['warehouse_id'], 'quantity' => $item['quantity'], 'cost' => $item['real_unit_price']));
                // }
            }
            ...
            //         $this->updateAVCO(array('product_id' => $oitem->product_id, 'warehouse_id' => $oitem->warehouse_id, 'quantity' => (0-$oitem->quantity), 'cost' => $oitem->real_unit_price));
            //     }
            // }
    ...
    public function updateStatus($id, $status, $note)
    {
        ...
        if ($this->db->update('quotes', array('note' => $note), array('id' => $id))) {
            foreach ($items as $item) {
 ...
 public function deleteMaterialRequest($id)
    {   
        $material_request = $this->getMaterialRequestByID($id);        
        $material_request_items = $this->getAllMaterialRequestItems($id);     

        if ($this->db->delete('material_request_items', array('material_request_id' => $id)) && $this->db->delete('material_request', array('id' => $id))) {
            
            $this->site->syncQuantity(NULL, NULL, $material_request_items);
            return true;
        }
        return FALSE;
    }
    ...
    {
        $q = $this->db->get_where('material_request', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getAllMaterialRequestItems($material_request_id)
    {
        $q = $this->db->get_where('material_request_items', array('material_request_id' => $material_request_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    //     $purchase_items = $this->site->getAllQuotationItems($data['purchase_id']);
    ...
    public function calculatePurchaseTotals($id, $return_id, $surcharge)
    {
        ...
        if (!empty($items)) {
            ...
            $this->db->delete('quotes', array('id' => $id));
        }
        return FALSE;
    }
