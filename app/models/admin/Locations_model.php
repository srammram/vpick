...
class Locations_model extends CI_Model
{
    ...
    public function __construct()
    {
   	...
   	/*### Daily*/
	function add_daily($data, $slot_array, $is_default, $taxi_type, $tons, $countryCode){
		if($is_default == 1){
	...
	function checkTypewiseCityDaily($taxi_type, $tons, $area_id, $countryCode){
		$this->db->select('*');
	...
	/*### Rental*/
	function add_rental($data, $is_default, $taxi_type, $tons, $package_name, $countryCode){
		if($is_default == 1){
			$this->db->where('is_default', $is_default);
			$this->db->where('taxi_type', $taxi_type);
			$this->db->where('tons', $tons);
	...
	function update_rental($id,$data, $countryCode){
		$this->db->where('id',$id);
		$this->db->where('is_country', $countryCode);
	...
	function checkTypewiseCityRental($taxi_type, $tons, $area_id, $package_name, $countryCode){
		$this->db->select('*');
	...
	/*### Outstation*/
	function add_outstation($data, $is_default, $taxi_type, $tons, $countryCode){
		if($is_default == 1){
			$this->db->where('is_default', $is_default);
			$this->db->where('taxi_type', $taxi_type);
			$this->db->where('tons', $tons);
	...
	function update_outstation($id,$data, $countryCode){
		$this->db->where('id',$id);
		$this->db->where('is_country', $countryCode);
