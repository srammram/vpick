...
class Transfers_model extends CI_Model
{
    ...
    public function updateStatus($id, $status, $note)
    {
        ...
        if ($this->db->update('transfers', array('status' => $status, 'note' => $note), array('id' => $id))) {
            ...
            foreach ($items as $item) {
                ...
                $item['transfer_id'] = $id;
                $item['option_id'] = !empty($item['option_id']) && is_numeric($item['option_id']) ? $item['option_id'] : NULL;
                unset($item['id'], $item['variant'], $item['unit'], $item['hsn_code']);
                if ($status == 'completed') {
                    $item['date'] = date('Y-m-d');
    ...
    {
        if ($status == 'completed') {
            $this->db->select('purchase_items.*, product_variants.name as variant, products.unit, products.hsn_code as hsn_code')
                ->from('purchase_items')
            ...
            $this->db->select('transfer_items.*, product_variants.name as variant, products.unit, products.hsn_code as hsn_code')
                ->from('transfer_items')
    ...
    public function getProduct_Options($product_id)
    {
    ...
    public function getMaterialRequestByID($material_request_id)
    {
        $q = $this->db->get_where('material_request', array('id' => $material_request_id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
...
public function getAllMaterialRequestItems($material_request_id)
    {
        ...
        $this->db->select('material_request_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.unit, products.details as details, product_variants.name as variant, products.hsn_code as hsn_code')
            ->join('products', 'products.id=material_request_items.product_id', 'left')
            ->join('product_variants', 'product_variants.id=material_request_items.option_id', 'left')
            ->join('tax_rates', 'tax_rates.id=material_request_items.tax_rate_id', 'left')
            ->group_by('material_request_items.id')
            ->order_by('id', 'asc');
