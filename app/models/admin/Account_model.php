...
class Account_model extends CI_Model
{
	...
	function getTrip(){
		if($this->session->group_id == 1){
			...
			$country = ' AND is_country="'.$this->countryCode.'"';
		}
		
		$this->db->select('DATE(r.booked_on) as date, SUM(rp.ride_final_fare)as value');
		$this->db->from('rides r');
		$this->db->join('ride_payment rp','rp.ride_id=r.id');
		$this->db->group_by('DATE(r.booked_on)');
		$this->db->order_by('DATE(r.booked_on)', 'ASC');
		$q = $this->db->get();
		if($q->num_rows()>0){
			return $q->result();
