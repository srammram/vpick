...
class Usersenquiry_model extends CI_Model
{
	...
	function checkRides($customer_type, $user_id, $start_date, $end_date, $countryCode){
		...
		if($customer_type == 3){
			$this->db->where('rides.vendor_id',$user_id);
		}elseif($customer_type == 4){
			$this->db->where('rides.driver_id',$user_id);
		}elseif($customer_type == 5){
			$this->db->where('rides.customer_id',$user_id);
		}
	...
	function getDashboard($user_id, $countryCode){
		...
		$q = $this->db->select('*')->where('is_country', $countryCode)->where('customer_id', $user_id)->get('enquiry');
		 if ($q->num_rows() > 0) {
	...
	function checkUser($mobile, $country_code, $customer_type, $countryCode){
		$q = $this->db->select('id')->where('mobile', $mobile)->where('is_country', $countryCode)->where('country_code', $country_code)->where('group_id', $customer_type)->get('users');
		if($q->num_rows()>0){
			return $q->row('id');
		}
		return false;	
	...
	function getUserEnquiry($user_id, $countryCode){
		$this->db->select('e.id as id, e.enquiry_date, e.enquiry_code, e.customer_id, e.help_department, e.services_id, e.help_id, e.help_message, e.suggestion, e.enquiry_status, e.customer_status, h.name as help');
	...
	function create_ticket($enquiry, $customer_id, $help_department, $countryCode){
		...
		if($enquiry_id){
			...
			if($enquiry_support_id){
				$this->db->insert('follows', array('enquiryid' => $enquiry_id, 'enquiry_support_id' => $enquiry_support_id, 'calltype' => 'Website',  'followup_date_time' => date('Y-m-d H:i:s'), 'created_on' => date('Y-m-d H:i:s'), 'is_edit' => 1, 'is_country' => $countryCode));
