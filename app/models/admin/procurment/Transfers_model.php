...
class Transfers_model extends CI_Model
{
    ...
    {
        $status = $data['status'];
        if ($this->db->insert('pro_transfers', $data)) {
            $transfer_id = $this->db->insert_id();
            if ($this->siteprocurment->getReference('to') == $data['transfer_no']) {
                $this->siteprocurment->updateReference('to');
            }
            foreach ($items as $item) {
                ...
                if ($status == 'completed') {
                    ...
                    $this->db->insert('pro_purchase_items', $item);
                } else {
                    $this->db->insert('pro_transfer_items', $item);
                }
    ...
    public function updateTransfer($id, $data = array(), $items = array())
    {
        ...
        if ($this->db->update('pro_transfers', $data, array('id' => $id))) {
    ...
    public function updateStatus($id, $status, $note)
    {
        ...
        if ($this->db->update('pro_transfers', array('status' => $status, 'note' => $note), array('id' => $id))) {
            $tbl = $ostatus == 'completed' ? 'purchase_items' : 'transfer_items';
            $this->db->delete($tbl, array('transfer_id' => $id));
            ...
            foreach ($items as $item) {
                ...
                if ($status == 'approved' || $status == 'completed') {
                    ...
                    $this->siteprocurment->syncQuantity(NULL, NULL, NULL, $item['product_id']);
                }
            }
            return true;
        }
    ...
    public function insertQuantity($product_id, $warehouse_id, $quantity)
    {
        if ($this->db->insert('warehouses_products', array('product_id' => $product_id, 'warehouse_id' => $warehouse_id, 'quantity' => $quantity))) {
            $this->siteprocurment->syncProductQty($product_id, $warehouse_id);
            return true;
        }
        return false;
    }
    ...
    public function getTransferByID($id)
    {
        ...
        $q = $this->db->get_where('pro_transfers', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    {
        if ($status == 'completed') {
            $this->db->select('pro_purchase_items.*, product_variants.name as variant, products.unit, products.hsn_code as hsn_code, products.name as second_name')
                ->from('pro_purchase_items')
                ->join('products', 'products.id=pro_purchase_items.product_id', 'left')
                ->join('product_variants', 'product_variants.id=pro_purchase_items.option_id', 'left')
                ->group_by('pro_purchase_items.id')
                ->where('transfer_id', $transfer_id);
        } else {
            $this->db->select('transfer_items.*, product_variants.name as variant, products.unit, products.hsn_code as hsn_code, products.name as second_name')
                ->from('pro_transfer_items')
    ...
    public function resetTransferActions($id, $delete = NULL)
    {
        ...
        if ($ostatus == 'approved' || $ostatus == 'completed') {
            foreach ($oitems as $item) {
                ...
                $this->siteprocurment->setPurchaseItem($clause, $item->quantity);
                if ($delete) {
                    $option_id = (isset($item->option_id) && ! empty($item->option_id)) ? $item->option_id : NULL;
                    ...
                    $this->siteprocurment->setPurchaseItem($clause, ($item->quantity_balance - $item->quantity));
                }
            }
        }
        return $ostatus;
    ...
    public function deleteTransfer($id)
    {
        ...
        $tbl = $ostatus == 'completed' ? 'purchase_items' : 'transfer_items';
        if ($this->db->delete('pro_transfers', array('id' => $id)) && $this->db->delete($tbl, array('transfer_id' => $id))) {
            foreach ($oitems as $item) {
                $this->siteprocurment->syncQuantity(NULL, NULL, NULL, $item->product_id);
            }
            return true;
    ...
    {
        if ($pis = $this->siteprocurment->getPurchasedItems($product_id, $warehouse_id, $option_id)) {
            $balance_qty = $quantity;
            foreach ($pis as $pi) {
                if ($balance_qty <= $quantity && $quantity > 0) {
                    if ($pi->quantity_balance >= $quantity) {
                        $balance_qty = $pi->quantity_balance - $quantity;
                        $this->db->update('pro_purchase_items', array('quantity_balance' => $balance_qty), array('id' => $pi->id));
                        $quantity = 0;
                    } elseif ($quantity > 0) {
                        $quantity = $quantity - $pi->quantity_balance;
                        $balance_qty = $quantity;
                        $this->db->update('pro_purchase_items', array('quantity_balance' => 0), array('id' => $pi->id));
                    }
                }
                if ($quantity == 0) { break; }
            }
            ...
            $this->siteprocurment->setPurchaseItem($clause, (0-$quantity));
        }
        $this->siteprocurment->syncQuantity(NULL, NULL, NULL, $product_id);
    }
