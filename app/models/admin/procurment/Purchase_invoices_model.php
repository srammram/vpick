...
class Purchase_invoices_model extends CI_Model
{
	...
	$store_default_id = $this->siteprocurment->defaultStores();
		...
		if($data['status']=="approved"){
		    ...
		    $this->siteprocurment->stock_update($item['product_id'],$item['quantity']);
...
//echo '<pre>';print_R($items);exit;
	$store_default_id = $this->siteprocurment->defaultStores();
		...
		if($data['status']=="approved"){
			...
			$this->siteprocurment->item_cost_update($item['product_id'],$item['cost'],$item['selling_price'],$item['tax_rate_id']);
			$this->siteprocurment->stock_update($item['product_id'],$update_qty);
		    }
		}
    ...
    $this->db->where(array('store_id'=>$store_id,'product_id'=>$product_id,'invoice_date'=>$inv_date,'invoice_id'=>$invoice_id,'batch'=>$batch,'expiry'=>$expiry));
    $q = $this->db->get();
