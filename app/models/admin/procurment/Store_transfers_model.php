...
class Store_transfers_model extends CI_Model
{
    ...
    {
        $this->db->select('pro_store_request_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.unit, products.details as details, product_variants.name as variant, products.hsn_code as hsn_code')
            ->join('products', 'products.id=pro_store_request_items.product_id', 'left')
            ->join('product_variants', 'product_variants.id=pro_store_request_items.option_id', 'left')
            ->join('tax_rates', 'tax_rates.id=pro_store_request_items.tax_rate_id', 'left')
            ->group_by('pro_store_request_items.id')
            ->order_by('id', 'asc');
        $q = $this->db->get_where('pro_store_request_items', array('store_request_id' => $store_transfers_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getAllStore_transfersItems($store_transfers_id)
    {
        $this->db->select('pro_store_transfer_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.unit, products.details as details, product_variants.name as variant, products.hsn_code as hsn_code')
            ...
            ->group_by('pro_store_transfer_items.id')
            ->order_by('id', 'asc');
        $q = $this->db->get_where('pro_store_transfer_items', array('store_transfer_id' => $store_transfers_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function addStore_transfers($data, $items, $store_transfers, $order_id)
    {
        ...
        if ($this->db->insert('pro_store_transfers', $data)) {
