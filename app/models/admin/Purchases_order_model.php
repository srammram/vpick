...
class Purchases_Order_model extends CI_Model
{
	...
	public function getAllPurchaseOrderItems($purchase_id)
    {
        ...
        $this->db->select('quote_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.unit, products.details as details, product_variants.name as variant, products.hsn_code as hsn_code')
            ...
            ->order_by('id', 'asc');
        $q = $this->db->get_where('quote_items', array('quote_id' => $purchase_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
        $this->db->select('purchase_order_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.unit, products.details as details, product_variants.name as variant, products.hsn_code as hsn_code')
    ...
    {
        $q = $this->db->get_where('purchase_order', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function addPurchasesOrder($data, $items, $purchases_order, $order_id)
    {
		
        if ($this->db->insert('purchase_order', $data)) {
             $purchase_order_id = $this->db->insert_id();
			
			$this->db->update('quotes', $purchases_order, array('id' => $order_id));
            ...
            if ($data['status'] == 'returned') {
                $this->db->update('purchases', array('return_purchase_ref' => $data['return_purchase_ref'], 'surcharge' => $data['surcharge'],'return_purchase_total' => $data['grand_total'], 'return_id' => $purchase_order_id), array('id' => $data['purchase_order_id']));
            }
    ...
    public function updatePurchasesOrder($id, $data, $items = array())
    {
        $opurchase = $this->getPurchasesOrderByID($id);
        $oitems = $this->getAllPurchasesOrderItems($id);
        if ($this->db->update('purchase_order', $data, array('id' => $id)) && $this->db->delete('purchase_order_items', array('purchase_order_id' => $id))) {
    ...
    {
        // $purchase = $this->getPurchasesOrderByID($id);
        $items = $this->site->getAllPurchasesOrderItems($id);

        if ($this->db->update('purchase_order_items', array('status' => $status, 'note' => $note), array('id' => $id))) {
            foreach ($items as $item) {
    ...
    {
        $purchase = $this->getPurchasesOrderByID($id);
        $purchase_items = $this->site->getAllPurchasesOrderItems($id);
        if ($this->db->delete('purchase_order_items', array('purchase_order_id' => $id)) && $this->db->delete('purchase_order', array('id' => $id))) {
    ...
    public function returnPurchase($data = array(), $items = array())
    {
        ...
        $purchase_items = $this->site->getAllPurchasesOrderItems($data['purchase_order_id']);
    ...
    {
        $purchase = $this->getPurchasesOrderByID($id);
        $items = $this->getAllPurchasesOrderItems($id);
        if (!empty($items)) {
            $total = 0;
            $product_tax = 0;
            $order_tax = 0;
