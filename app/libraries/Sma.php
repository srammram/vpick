...
class Sma
{
    ...
    public function checkPermissions($action = null, $js = null)
    {

       
		
        if ($this->actionPermissions($action) == FALSE) {
    ...
    public function actionPermissions($action = null){
       
       
        if ($this->session->userdata('group_id') == 1 || $this->session->userdata('group_id') == 2) {
            return true;
        }else{
            if ($this->data['GP']->{$action} == 1) {
                return true;
            } 
        }
        return false;
    ...
    public function generate_pdf($content, $name = 'download.pdf', $output_type = null, $footer = null, $margin_bottom = null, $header = null, $margin_top = null, $orientation = 'P')
    {
       ...
       // } else {
		...
		//echo $this->pdf->generate($content, $name, $output_type, $footer, $margin_bottom, $header, $margin_top, $orientation);
		//die;
