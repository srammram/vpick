...
class Quotes extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->lang->admin_load('quotations_lang', $this->Settings->user_language);
    ...
    public function index($warehouse_id = null)
    {
        ...
        $meta = array('page_title' => lang('Quatation'), 'bc' => $bc);
        $this->page_construct('quotes/index', $meta, $this->data);

    }

    public function getQuotation($warehouse_id = null)
    {
        ...
        $detail_link = anchor('admin/quotes/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('quotation_details'));
        $payments_link = anchor('admin/quotes/payments/$1', '<i class="fa fa-money"></i> ' . lang('view_payments'), 'data-toggle="modal" data-target="#myModal"');
        $add_payment_link = anchor('admin/quotes/add_payment/$1', '<i class="fa fa-money"></i> ' . lang('add_payment'), 'data-toggle="modal" data-target="#myModal"');
        $email_link = anchor('admin/quotes/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_quotation'), 'data-toggle="modal" data-target="#myModal"');
        $edit_link = anchor('admin/quotes/edit/$1', '<i class="fa fa-edit"></i> ' . lang('edit_quotation'));
        ...
        $return_link = anchor('admin/quotes/return_purchase/$1', '<i class="fa fa-angle-double-left"></i> ' . lang('return_purchase'));
        $delete_link_link = "<a href='#' class='po' title='<b>" . $this->lang->line("delete_quotation") . "</b>' data-content=\"<p>"
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('quotes/delete/$1') . "'>"
        . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
        . lang('delete_quotation') . "</a>";
        ...
        // echo "fg";exit;    
            $this->datatables
                ->select("'sno',{$this->db->dbprefix('quotes')}.id as id,{$this->db->dbprefix('warehouses')}.name as branch, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, customer, total, total_discount, total_tax, grand_total")
                ->from('quotes')
    ...
    public function modal_view($quotation_id = null)
    {
        ...
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $inv = $this->quotes_model->getQuoteByID($quotation_id);
        if (!$this->session->userdata('view_right')) {
            $this->sma->view_rights($inv->created_by, true);
        }
        $this->data['rows'] = $this->quotes_model->getAllQuoteItems($quotation_id);
        $this->data['supplier'] = $this->site->getCompanyByID($inv->customer_id);
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
        $this->data['payments'] = $this->quotes_model->getPaymentsForPurchase($quotation_id);
    ...
    public function view($quotation_id = null)
    {
        $this->sma->checkPermissions('index');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('quotes'), 'page' => lang('quotetation')), array('link' => '#', 'page' => lang('view')));
        $meta = array('page_title' => lang('view_quotation_details'), 'bc' => $bc);
...
//generate pdf and force to download
    ...
    public function pdf($quotation_id = null, $view = null, $save_bufffer = null)
    {
        $this->sma->checkPermissions();
        ...
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $quo = $this->quotes_model->getQuoteByID($quotation_id);
        if (!$this->session->userdata('view_right')) {
        ...
        $name = $this->lang->line("quotation") . "_" . str_replace('/', '_', $quo->reference_no) . ".pdf";
    ...
    public function email($quotation_id = null)
    {
        ...
        $quo = $this->quotes_model->getQuotationByID($quotation_id);
        ...
        if ($this->form_validation->run() == true) {
            ...
            $parse_data = array(
                'reference_number' => $quo->reference_no,
            ...
            $attachment = $this->pdf($quotation_id, null, 'S');

            try {
                if ($this->sma->send_email($to, $subject, $message, null, null, $attachment, $cc, $bcc)) {
                    delete_files($attachment);
                    $this->db->update('quotes', array('status' => 'ordered'), array('id' => $quotation_id));
            ...
            if (file_exists('./themes/' . $this->Settings->theme . '/admin/views/email_templates/quotation.html')) {
                $quotation_temp = file_get_contents('themes/' . $this->Settings->theme . '/admin/views/email_templates/quotation.html');
            } else {
                $quotation_temp = file_get_contents('./themes/default/admin/views/email_templates/quotation.html');
            }
            $this->data['subject'] = array('name' => 'subject',
                'id' => 'subject',
                'type' => 'text',
                'value' => $this->form_validation->set_value('subject', lang('quote_order_no').' (' . $quo->reference_no . ') '.lang('from').' ' . $this->Settings->site_name),
            );
            $this->data['note'] = array('name' => 'note',
                'id' => 'note',
                'type' => 'text',
                'value' => $this->form_validation->set_value('note', $quotation_temp),
            );
            $this->data['supplier'] = $this->site->getCompanyByID($quo->customer_id);
    ...
    public function add($quote_id = null)
    {
		...
		$this->form_validation->set_rules('purchase_order_no', $this->lang->line("purchase_order_no"), 'required');*/
...
/*sma_quote_items*/
       ...
       /*print_r($data);exit;*/
        if ($this->form_validation->run() == true && $this->quotes_model->addQuatation($data, $products)) {
              // print_r($products);exit;
    ...
    public function edit($id = null)
    {
        ...
        $inv = $this->quotes_model->getQuotationByID($id);
       ...
       /* $this->form_validation->set_rules('reference_no', $this->lang->line("ref_no"), 'required');*/
        ...
        if ($this->form_validation->run() == true) {
            ...
            $reference = $this->input->post('quotation_order_no');
            if ($this->Owner || $this->Admin) {
               /* $date = $this->sma->fld(trim($this->input->post('quotation_order_date')));*/

               $date = $this->input->post('quotation_order_date');
        ...
        if ($this->form_validation->run() == true && $this->quotes_model->updateQuotation($id, $data, $items)) {
             
            $this->session->set_userdata('remove_pols', 1);
            $this->session->set_flashdata('message', $this->lang->line("quotation_added"));
            admin_redirect('quotes');
        } else {
            ...
            $this->data['id'] = $id;
            $this->data['suppliers'] = $this->site->getAllCompanies('supplier');
            $this->data['quotation'] = $this->quotes_model->getQuotationByID($id);
            $this->data['categories'] = $this->site->getAllCategories();
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->load->helper('string');
            ...
            $this->session->set_userdata('remove_pols', 1);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('quotes'), 'page' => lang('quotation')), array('link' => '#', 'page' => lang('edit_quotation')));
            $meta = array('page_title' => lang('edit_quotation'), 'bc' => $bc);
           /* echo "<pre>";
   ...
   public function delete($id = null)
    {
        ...
        if ($this->quotes_model->deleteQuotation($id)) {
    ...
    public function suggestions()
    {
        ...
        $rows = $this->quotes_model->getProductNames($sr);
        if ($rows) {
