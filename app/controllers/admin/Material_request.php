...
class Material_request extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->lang->admin_load('material_request_lang', $this->Settings->user_language);
        $this->load->library('form_validation');
        $this->load->admin_model('material_request_model');
    ...
    public function index($warehouse_id = null)
    {
        ...
        $meta = array('page_title' => lang('material_request'), 'bc' => $bc);
        $this->page_construct('material_request/index', $meta, $this->data);

    }

    public function getMaterialrequest($warehouse_id = null)
    {
        ...
        $detail_link = anchor('admin/material_request/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('material_request_details'));
        $payments_link = anchor('admin/material_request/payments/$1', '<i class="fa fa-money"></i> ' . lang('view_payments'), 'data-toggle="modal" data-target="#myModal"');
        $add_payment_link = anchor('admin/material_request/add_payment/$1', '<i class="fa fa-money"></i> ' . lang('add_payment'), 'data-toggle="modal" data-target="#myModal"');
        $email_link = anchor('admin/material_request/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_material_request'), 'data-toggle="modal" data-target="#myModal"');
        $edit_link = anchor('admin/material_request/edit/$1', '<i class="fa fa-edit"></i> ' . lang('edit_material_request'));
        $pdf_link = anchor('admin/material_request/pdf/$1', '<i class="fa fa-file-pdf-o"></i> ' . lang('download_pdf'));
        $print_barcode = anchor('admin/products/print_barcodes/?purchase=$1', '<i class="fa fa-print"></i> ' . lang('print_barcodes'));
        $return_link = anchor('admin/material_request/return_purchase/$1', '<i class="fa fa-angle-double-left"></i> ' . lang('return_purchase'));
        $delete_link_link = "<a href='#' class='po' title='<b>" . $this->lang->line("delete_material_request") . "</b>' data-content=\"<p>"
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('material_request/delete/$1') . "'>"
        . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
        . lang('delete_material_request') . "</a>";
        ...
        if ($warehouse_id) {        
            ...
            $this->datatables
                ->select("'sno',material_request.id AS id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, material_request_no,fw.name as fname, tw.name as tname, total, total_discount, total_tax, grand_total")
                ->from('material_request')
                ->join('warehouses  fw', 'fw.id=material_request.warehouse_id','left')
                ->join('warehouses  tw', 'tw.id=material_request.to_warehouse_id','left')
    ...
    public function modal_view($material_request_id = null)
    {
        $this->sma->checkPermissions('index', true);

        if ($this->input->get('id')) {
            $material_request_id = $this->input->get('id');
        }
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $inv = $this->material_request_model->getQuoteByID($material_request_id);
        if (!$this->session->userdata('view_right')) {
            $this->sma->view_rights($inv->created_by, true);
        }
        $this->data['rows'] = $this->material_request_model->getAllMaterialRequestItems($material_request_id);
        /*$this->data['supplier'] = $this->site->getCompanyByID($inv->customer_id);*/
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
        $this->data['payments'] = $this->material_request_model->getPaymentsForPurchase($material_request_id);
        $this->data['created_by'] = $this->site->getUser($inv->created_by);
        $this->data['updated_by'] = $inv->updated_by ? $this->site->getUser($inv->updated_by) : null;
        // $this->data['return_purchase'] = $inv->return_id ? $this->quotes_model->getQuoteByID($inv->return_id) : NULL;
        // $this->data['return_rows'] = $inv->return_id ? $this->quotes_model->getAllQuoteItems($inv->return_id) : NULL;

        $this->load->view($this->theme . 'material_request/modal_view', $this->data);

    }

    public function view($material_request_id = null)
    {
        ...
        $inv = $this->material_request_model->getMaterialRequestByID($material_request_id);
        ...
        $this->data['created_by'] = $this->site->getUser($inv->created_by);
        $this->data['updated_by'] = $inv->updated_by ? $this->site->getUser($inv->updated_by) : null;
        $this->data['return_purchase'] = $inv->return_id ? $this->material_request_model->getMaterialRequestByID($inv->return_id) : NULL;
        $this->data['return_rows'] = $inv->return_id ? $this->material_request_model->getAllMaterialRequestItems($inv->return_id) : NULL;

        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('material_request'), 'page' => lang('quotetation')), array('link' => '#', 'page' => lang('view')));
        $meta = array('page_title' => lang('view_material_request_details'), 'bc' => $bc);
        $this->page_construct('material_request/view', $meta, $this->data);
...
//generate pdf and force to download
    ...
    public function pdf($material_request_id = null, $view = null, $save_bufffer = null)
    {
        $this->sma->checkPermissions();
        ...
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $quo = $this->material_request_model->getMaterialRequestByID($material_request_id);
        if (!$this->session->userdata('view_right')) {
            $this->sma->view_rights($quo->created_by);
        }

        $this->data['rows'] = $this->material_request_model->getAllMaterialRequestItems($material_request_id);
        $this->data['supplier'] = $this->site->getCompanyByID($quo->customer_id);
        // print_r($this->data['supplier']);exit; 
        $this->data['warehouse'] = $this->site->getWarehouseByID($quo->warehouse_id);
        $this->data['created_by'] = $this->site->getUser($quo->created_by);
        $this->data['inv'] = $quo;

        // $this->data['return_purchase'] = $quo->return_id ? $this->quotes_model->getQuoteByID($quo->return_id) : NULL;
        // $this->data['return_rows'] = $quo->return_id ? $this->quotes_model->getAllQuoteItems($quo->return_id) : NULL;
        $name = $this->lang->line("material_request") . "_" . str_replace('/', '_', $quo->material_request_no) . ".pdf";
        $html = $this->load->view($this->theme . 'material_request/pdf', $this->data, true);
        ...
        if ($view) {
            ...
            $this->load->view($this->theme . 'material_request/pdf', $this->data);
        } elseif ($save_bufffer) {
    ...
    public function combine_pdf($purchases_id)
    {
        ...
        foreach ($purchases_id as $purchase_id) {
            ...
            $inv_html = $this->load->view($this->theme . 'quotes/pdf', $this->data, true);
            if (! $this->Settings->barcode_img) {
    ...
    public function email($material_request_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $customer = $this->site->getCompanyByID($quo->customer_id);
            $this->load->library('parser');
            $parse_data = array(
                'reference_number' => $quo->material_request_no,
            ...
            $attachment = $this->pdf($material_request_id, null, 'S');

            try {
                if ($this->sma->send_email($to, $subject, $message, null, null, $attachment, $cc, $bcc)) {
                    delete_files($attachment);
                    $this->db->update('material_request', array('status' => 'ordered'), array('id' => $material_request_id));
                    $this->session->set_flashdata('message', $this->lang->line("email_sent"));
                    admin_redirect("material_request");
                }
            } catch (Exception $e) {
            ...
            if (file_exists('./themes/' . $this->Settings->theme . '/admin/views/email_templates/material_request.html')) {
                $material_request_temp = file_get_contents('themes/' . $this->Settings->theme . '/admin/views/email_templates/material_request.html');
            } else {
                $material_request_temp = file_get_contents('./themes/default/admin/views/email_templates/material_request.html');
            }
            $this->data['subject'] = array('name' => 'subject',
                'id' => 'subject',
                'type' => 'text',
                'value' => $this->form_validation->set_value('subject', lang('material_request_no').' (' . $quo->material_request_no . ') '.lang('from').' ' . $this->Settings->site_name),
            );
            $this->data['note'] = array('name' => 'note',
                'id' => 'note',
                'type' => 'text',
                'value' => $this->form_validation->set_value('note', $material_request_temp),
            );
            $this->data['supplier'] = $this->site->getCompanyByID($quo->customer_id);

            $this->data['id'] = $material_request_id;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'material_request/email', $this->data);
    ...
    public function add($material_request_id = null)
    {
        $this->sma->checkPermissions();

        $this->form_validation->set_message('is_natural_no_zero', $this->lang->line("no_zero_required"));
        $this->form_validation->set_rules('warehouse', $this->lang->line("warehouse"), 'required|is_natural_no_zero');
        $this->form_validation->set_rules('to_warehouse', $this->lang->line("to_warehouse"), 'required');

        $this->form_validation->set_rules('material_request_no', $this->lang->line("material_request_no"), 'required');
		
        $this->session->unset_userdata('csrf_token');
        if ($this->form_validation->run() == true) {
            /*echo "<pre>";
            print_r($this->input->post());die;*/
            $reference = $this->input->post('material_request_no');
            if ($this->Owner || $this->Admin) {
                $date = $this->sma->fld(trim($this->input->post('material_request_date')));
            } else {
            ...
            for ($r = 0; $r < $i; $r++) {
				...
				$item_tax_method = isset($_POST['tax2'][$r]) ? $_POST['tax2'][$r] : null;
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    $product_details = $this->material_request_model->getProductByCode($item_code);
            ...
            $data = array(
                'material_request_no' => 'MR-'.date('YmdHis'),   
                'warehouse_id' => $warehouse_id,
        ...
        if ($this->form_validation->run() == true && $this->material_request_model->addMaterialRequest($data, $products)) {
           
              // print_r($products);exit;
            $this->session->set_userdata('remove_pols', 1);
            $this->session->set_flashdata('message', $this->lang->line("material_request_added"));
            admin_redirect('material_request');
        } else {
          

            if ($material_request_id) {
                $this->data['material_request'] = $this->material_request_model->getMaterialRequestByID($material_request_id);
                $supplier_id = $this->data['material_request']->supplier_id;
                $items = $this->material_request_model->getAllMaterialRequestItems($material_request_id);
                ...
                foreach ($items as $item) {
                    ...
                    if ($row->type == 'combo') {
                        ...
                        foreach ($combo_items as $citem) {
                            ...
                            $options = $this->quotes_model->getProductOptions($crow->id);
                            $units = $this->site->getUnitsByBUID($row->base_unit);
                            $tax_rate = $this->site->getTaxRateByID($crow->tax_rate);
                            $ri = $this->Settings->item_addition ? $crow->id : $crow->id;
                ...
                $this->data['material_request_items'] = json_encode($pr);
            }
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('material_request'), 'page' => lang('material_request')), array('link' => '#', 'page' => lang('add_material_request')));
            $meta = array('page_title' => lang('add_material_request'), 'bc' => $bc);
            $this->page_construct('material_request/add', $meta, $this->data);
        }
    }
    ...
    public function edit($id = null)
    {
        ...
        $inv = $this->material_request_model->getMaterialRequestByID($id);
        ...
        if ($this->form_validation->run() == true) {
            $reference = $this->input->post('material_request_no');
            if ($this->Owner || $this->Admin) {
               $date = $this->input->post('material_request_date');
            } else {
                $date = $inv->date;
            }
            $warehouse_id = $this->input->post('warehouse');
        ...
        if ($this->form_validation->run() == true && $this->material_request_model->updateMaterialRequest($id, $data, $items)) {
            ...
            $inv_items = $this->material_request_model->getAllMaterialRequestItems($id); 
            ...
            foreach ($inv_items as $item) {
                ...
                $row->discount = $item->discount ? $item->discount : '0';
                $options = $this->material_request_model->getProductOptions($row->id);
            ...
            $this->data['id'] = $id;
            $this->data['suppliers'] = $this->site->getAllCompanies('supplier');
            $this->data['quotation'] = $this->material_request_model->getMaterialRequestByID($id);
            $this->data['categories'] = $this->site->getAllCategories();
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->load->helper('string');
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('material_request'), 'page' => lang('material_request')), array('link' => '#', 'page' => lang('edit_material_request')));
            $meta = array('page_title' => lang('edit_material_request'), 'bc' => $bc);
          /*  echo "<pre>";
            print_r($this->data);exit;
            echo "</pre>";*/
            $this->page_construct('material_request/edit', $meta, $this->data);
        }
    }
    ...
    public function purchase_by_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                if (!$this->upload->do_upload()) {
                    $error = $this->upload->display_errors();
                    $this->session->set_flashdata('error', $error);
                    admin_redirect("quotes/purchase_by_csv");
                }
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products)) {
            ...
            $this->page_construct('quotes/purchase_by_csv', $meta, $this->data);
   ...
   public function delete($id = null)
    {
        ...
        if ($this->material_request_model->deleteMaterialRequest($id)) {

            if ($this->input->is_ajax_request()) {
                $this->sma->send_json(array('error' => 0, 'msg' => lang("material_request_deleted")));
            }
            $this->session->set_flashdata('message', lang('material_request_deleted'));
            admin_redirect('material_request');
        }
    }
    ...
    public function suggestions()
    {
        ...
        $rows = $this->material_request_model->getProductNames($sr);
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $options = $this->material_request_model->getProductOptions($row->id);
                if ($options) {
                    $opt = $option_id && $r == 0 ? $this->material_request_model->getProductOptionByID($option_id) : current($options);
                    if (!$option_id || $r > 0) {
                        $option_id = $opt->id;
                    }
                } else {
    ...
    public function quotes_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $this->excel->getActiveSheet()->setTitle(lang('quotation'));
    ...
    public function payments($id = null)
    {
        ...
        $this->load->view($this->theme . 'quotes/payments', $this->data);
    }
    ...
    public function payment_note($id = null)
    {
        ...
        $this->load->view($this->theme . 'quotes/payment_note', $this->data);
    }
    ...
    public function email_payment($id = null)
    {
        ...
        $html = $this->load->view($this->theme . 'quotes/payment_note', $this->data, TRUE);
    ...
    public function add_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPayment($payment)) {
            ...
            $this->load->view($this->theme . 'quotes/add_payment', $this->data);
        }
    }
    ...
    public function edit_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePayment($id, $payment)) {
            ...
            $this->load->view($this->theme . 'quotes/edit_payment', $this->data);
        }
    }
    ...
    public function expenses($id = null)
    {
        ...
        $this->page_construct('quotes/expenses', $meta, $this->data);
    }
    ...
    public function getExpenses()
    {
        ...
        $detail_link = anchor('admin/quotes/expense_note/$1', '<i class="fa fa-file-text-o"></i> ' . lang('expense_note'), 'data-toggle="modal" data-target="#myModal2"');
        $edit_link = anchor('admin/quotes/edit_expense/$1', '<i class="fa fa-edit"></i> ' . lang('edit_expense'), 'data-toggle="modal" data-target="#myModal"');
        ...
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('quotes/delete_expense/$1') . "'>"
        . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
    ...
    public function expense_note($id = null)
    {
        ...
        $this->load->view($this->theme . 'quotes/expense_note', $this->data);
    }
    ...
    public function add_expense()
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addExpense($data)) {
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'quotes/add_expense', $this->data);
        }
    }
    ...
    public function edit_expense($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updateExpense($id, $data)) {
            ...
            admin_redirect("quotes/expenses");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['categories'] = $this->purchases_model->getExpenseCategories();
            $this->load->view($this->theme . 'quotes/edit_expense', $this->data);
        }
    }
    ...
    public function view_return($id = null)
    {
        ...
        $this->load->view($this->theme.'quotes/view_return', $this->data);
    }
    ...
    public function return_purchase($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products)) {
            ...
            $inv_items = $this->quotes_model->getAllQuotationItems($id);
            // krsort($inv_items);
            $c = rand(100000, 9999999);
            foreach ($inv_items as $item) {

                $row = $this->site->getProductByID($item->product_id);
                $row->expiry = (($item->expiry_date && $item->expiry_date != '0000-00-00') ? $this->sma->hrsd($item->expiry_date) : '');
                $row->base_quantity = $item->quantity;
                $row->base_unit = $row->unit ? $row->unit : $item->product_unit_id;
                $row->base_unit_cost = $row->cost ? $row->cost : $item->unit_cost;
                $row->unit = $item->product_unit_id;
            ...
            $this->page_construct('quotes/return_purchase', $meta, $this->data);
        }
    }
