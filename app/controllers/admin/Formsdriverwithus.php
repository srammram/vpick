...
class Formsdriverwithus extends MY_Controller
{
    ...
    function __construct()
    {
       ...
       $this->image_types = 'gif|jpg|png|jpeg|pdf';
		...
		$this->load->admin_model('formsdriverwithus_model');
    }
    ...
    {
		$this->sma->checkPermissions('formsdriverwithus-index');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('Formsdriverwithus')));
        $meta = array('page_title' => lang('Formsdriverwithus'), 'bc' => $bc);
        $this->page_construct('forms/driverwithus', $meta, $this->data);
    }
	 function formsdriverwithus_actions($wh = NULL)
    {
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
       ...
       $this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
        ...
        if ($this->form_validation->run() == true) {
                ...
                if ($this->input->post('form_action') == 'export_excel') {
                    ...
                    $this->excel->getActiveSheet()->setTitle('Formsdriverwithus');
                    ...
                    $row = 2;
					$res = $this->formsdriverwithus_model->getALLFormsdriverwithus($countryCode);
					if(!empty($res)){
    ...
    function getFormsdriverwithus(){
        ...
        $this->datatables
            ->select(" {$this->db->dbprefix('enquiry_forms')}.name, {$this->db->dbprefix('enquiry_forms')}.mobile_number, {$this->db->dbprefix('enquiry_forms')}.email_address, {$this->db->dbprefix('enquiry_forms')}.description, country.name as instance_country ")
            ->from("enquiry_forms")
