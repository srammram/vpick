...
class Account extends MY_Controller
{
	...
	public function dashboard() {
		$this->sma->checkPermissions('accounts_dashboard-index');
		$meta = array('page_title' => lang('dashboard'), 'bc' => $bc);
		
		
		$this->data['trip'] = $this->account_model->getTrip();
		$this->page_construct('account/dashboard', $meta, $this->data);

	}
	
	public function account_owner() {
		$this->sma->checkPermissions('account_owner-index');
	...
	public function account_settlementlist() {
		$this->sma->checkPermissions('account_settlementlist-index');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
	...
	public function account_settlement_verify($id) {
		$this->sma->checkPermissions('account_settlementlist-approved');
	...
	public function reconcilation() {
		$this->sma->checkPermissions('reconcilation-index');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
	...
	public function settlement_branch() {
		$this->sma->checkPermissions('settlement_branch-index');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
	...
	public function bank_excel() {
		...
		$this->sma->checkPermissions('bank_excel-index');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
	...
	function trip($action=false){
		$this->sma->checkPermissions('trip-index');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
	...
	function getTrip(){
       ...
       // $this->sma->checkPermissions('index');
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("{$this->db->dbprefix('rides')}.booked_on as booked_on, {$this->db->dbprefix('rides')}.id as id, {$this->db->dbprefix('rides')}.booking_no as booking_no,   du.first_name as driver_name, du.mobile as driver_mobile,  cu.first_name as customer_name, cu.mobile as customer_mobile, rp.driver_fare, rp.driver_tax_name, rp.driver_tax_fare, rp.driver_total_fare, rp.admin_percentage, rp.admin_fare, rp.admin_tax_name, rp.admin_tax_fare, rp.admin_total_fare, rp.ride_final_fare, country.name as instance_country ")
            ->from("rides")
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
    ...
    function driver($action=false){
		$this->sma->checkPermissions('driver_payment-index');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
