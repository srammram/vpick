...
class Order_purchases extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->lang->admin_load('order_purchases', $this->Settings->user_language);
        $this->load->library('form_validation');
        $this->load->admin_model('order_purchases_model');
    ...
    public function index($warehouse_id = null)
    {
        ...
        $this->page_construct('order_purchases/index', $meta, $this->data);
    ...
    public function getPurchases($warehouse_id = null)
    {
       /* $this->sma->checkPermissions('index');
        ...
        $detail_link = anchor('admin/order_purchases/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('purchase_details'));
        $payments_link = anchor('admin/order_purchases/payments/$1', '<i class="fa fa-money"></i> ' . lang('view_payments'), 'data-toggle="modal" data-target="#myModal"');
        $add_payment_link = anchor('admin/order_purchases/add_payment/$1', '<i class="fa fa-money"></i> ' . lang('add_payment'), 'data-toggle="modal" data-target="#myModal"');
        $email_link = anchor('admin/order_purchases/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_purchase'), 'data-toggle="modal" data-target="#myModal"');
        $edit_link = anchor('admin/order_purchases/edit/$1', '<i class="fa fa-edit"></i> ' . lang('edit_purchase'));
        $pdf_link = anchor('admin/order_purchases/pdf/$1', '<i class="fa fa-file-pdf-o"></i> ' . lang('download_pdf'));
        $print_barcode = anchor('admin/order_purchases/print_barcodes/?purchase=$1', '<i class="fa fa-print"></i> ' . lang('print_barcodes'));
        $return_link = anchor('admin/order_purchases/return_purchase/$1', '<i class="fa fa-angle-double-left"></i> ' . lang('return_purchase'));
        $delete_link = "<a href='#' class='po' title='<b>" . $this->lang->line("delete_purchase") . "</b>' data-content=\"<p>"
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('order_purchases/delete/$1') . "'>"
    ...
    public function modal_view($purchase_id = null)
    {
        ...
        $this->load->view($this->theme . 'order_purchases/modal_view', $this->data);
    ...
    public function view($purchase_id = null)
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('order_purchases'), 'page' => lang('order purchases')), array('link' => '#', 'page' => lang('view')));
        $meta = array('page_title' => lang('view_purchase_details'), 'bc' => $bc);
        $this->page_construct('order_purchases/view', $meta, $this->data);
...
//generate pdf and force to download
    ...
    public function pdf($purchase_id = null, $view = null, $save_bufffer = null)
    {
        ...
        $html = $this->load->view($this->theme . 'order_purchases/pdf', $this->data, true);
        if (! $this->Settings->barcode_img) {
            $html = preg_replace("'\<\?xml(.*)\?\>'", '', $html);
        }
        if ($view) {
            $this->load->view($this->theme . 'order_purchases/pdf', $this->data);
        } elseif ($save_bufffer) {
    ...
    public function combine_pdf($purchases_id)
    {
        ...
        foreach ($purchases_id as $purchase_id) {
            ...
            $this->data['return_purchase'] = $inv->return_id ? $this->purchases_model->getPurchaseByID($inv->return_id) : NULL;
            $this->data['return_rows'] = $inv->return_id ? $this->purchases_model->getAllPurchaseItems($inv->return_id) : NULL;
            $inv_html = $this->load->view($this->theme . 'order_purchases/pdf', $this->data, true);
            if (! $this->Settings->barcode_img) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            try {
                if ($this->sma->send_email($to, $subject, $message, null, null, $attachment, $cc, $bcc)) {
                    ...
                    admin_redirect("order_purchases");
                }
            } catch (Exception $e) {
            ...
            $this->load->view($this->theme . 'order_purchases/email', $this->data);
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products)) {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('order_purchases'), 'page' => lang('order purchases')), array('link' => '#', 'page' => lang('add_purchase')));
            $meta = array('page_title' => lang('add_purchase'), 'bc' => $bc);
            $this->page_construct('order_purchases/add', $meta, $this->data);
        }
    }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePurchase($id, $data, $products)) {
            ...
            $this->session->set_userdata('remove_pols', 1);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('order_purchases'), 'page' => lang('order purchases')), array('link' => '#', 'page' => lang('edit_purchase')));
            $meta = array('page_title' => lang('edit_purchase'), 'bc' => $bc);
            $this->page_construct('order_purchases/edit', $meta, $this->data);
        }
    }
    ...
    public function purchase_by_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                if (!$this->upload->do_upload()) {
                    $error = $this->upload->display_errors();
                    $this->session->set_flashdata('error', $error);
                    admin_redirect("order_purchases/purchase_by_csv");
                }
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products)) {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('order_purchases'), 'page' => lang('order purchases')), array('link' => '#', 'page' => lang('add_purchase_by_csv')));
            $meta = array('page_title' => lang('add_purchase_by_csv'), 'bc' => $bc);
            $this->page_construct('order_purchases/purchase_by_csv', $meta, $this->data);
    ...
    public function payments($id = null)
    {
        ...
        $this->load->view($this->theme . 'order_purchases/payments', $this->data);
    }
    ...
    public function payment_note($id = null)
    {
        ...
        $this->load->view($this->theme . 'order_purchases/payment_note', $this->data);
    }
    ...
    public function email_payment($id = null)
    {
        ...
        $html = $this->load->view($this->theme . 'order_purchases/payment_note', $this->data, TRUE);
    ...
    public function add_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPayment($payment)) {
            ...
            $this->load->view($this->theme . 'order_purchases/add_payment', $this->data);
        }
    }
    ...
    public function edit_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePayment($id, $payment)) {
            ...
            $this->load->view($this->theme . 'order_purchases/edit_payment', $this->data);
        }
    }
    ...
    public function expenses($id = null)
    {
        ...
        $this->page_construct('order_purchases/expenses', $meta, $this->data);
    }
    ...
    public function getExpenses()
    {
        ...
        $detail_link = anchor('admin/order_purchases/expense_note/$1', '<i class="fa fa-file-text-o"></i> ' . lang('expense_note'), 'data-toggle="modal" data-target="#myModal2"');
        $edit_link = anchor('admin/order_purchases/edit_expense/$1', '<i class="fa fa-edit"></i> ' . lang('edit_expense'), 'data-toggle="modal" data-target="#myModal"');
        ...
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('order_purchases/delete_expense/$1') . "'>"
        . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
    ...
    public function expense_note($id = null)
    {
        ...
        $this->load->view($this->theme . 'order_purchases/expense_note', $this->data);
    }
    ...
    public function add_expense()
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addExpense($data)) {
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'order_purchases/add_expense', $this->data);
        }
    }
    ...
    public function edit_expense($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updateExpense($id, $data)) {
            ...
            admin_redirect("order_purchases/expenses");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['categories'] = $this->purchases_model->getExpenseCategories();
            $this->load->view($this->theme . 'order_purchases/edit_expense', $this->data);
        }
    }
    ...
    public function view_return($id = null)
    {
        ...
        $this->load->view($this->theme.'order_purchases/view_return', $this->data);
    }
    ...
    public function return_purchase($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products)) {
            ...
            $this->data['reference'] = '';
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('order_purchases'), 'page' => lang('order purchases')), array('link' => '#', 'page' => lang('return_purchase')));
            $meta = array('page_title' => lang('return_purchase'), 'bc' => $bc);
            $this->page_construct('order_purchases/return_purchase', $meta, $this->data);
        }
    }
    ...
    public function update_status($id)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updateStatus($id, $status, $note)) {
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme.'order_purchases/update_status', $this->data);
