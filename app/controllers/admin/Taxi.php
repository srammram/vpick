...
class Taxi extends MY_Controller
{
    ...
    function index($action=false){
		$this->sma->checkPermissions('cab-index');
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
    ...
    function getTaxi(){
        ...
        $this->datatables
            ->select("{$this->db->dbprefix('taxi')}.id as id, {$this->db->dbprefix('taxi')}.created_on, up.first_name, {$this->db->dbprefix('taxi')}.name as taxi_name, {$this->db->dbprefix('taxi')}.number, {$this->db->dbprefix('taxi')}.model,   tf.name as fuel_type, tt.name as type_name, GROUP_CONCAT(tts.name) as services_type,   If({$this->db->dbprefix('taxi')}.is_verify = 1 && td.reg_verify = 1 && td.taxation_verify = 1 && td.insurance_verify = 1 && td.permit_verify = 1 && td.authorisation_verify = 1 && td.fitness_verify = 1 && td.speed_verify = 1 && td.emission_norms_verify = 1  && td.vehicle_tracking_verify = 1  && td.fire_extinguisher_verify = 1  && td.child_lock_mechanism_verify = 1  && td.interior_vehicle_verify = 1  && td.taxi_roof_sign_verify = 1 && td.e_challans_clearance_verify = 1 && td.puc_verify = 1 , '1', '0') as status, country.name as instance_country ")
            ->from("taxi")
			...
			->join("taxi_services ts", "ts.taxi_id = taxi.id AND ts.status = 1 ", 'left')
			->join("taxi_type tts", "tts.id = ts.taxi_type ", 'left')
			...
			$edit = "<a href='" . admin_url('taxi/edit_taxi/$1') . "' data-toggle='tooltip'  data-original-title='' aria-describedby='tooltip' title='".lang('click_here_to_full_details')."'  ><i class='fa fa-pencil-square-o' aria-hidden='true'  style='color:#656464; font-size:18px'></i></a>";
			
			$driver = "<a href='" . admin_url('people/driver/?cab=$1') . "' data-toggle='tooltip'  data-original-title='' aria-describedby='tooltip' title='".lang('click_here_to_driver')."'  ><i class='fa fa-users' aria-hidden='true'  style='color:#656464; font-size:18px'></i></a>";
			
			$vendor = "<a href='" . admin_url('people/vendor/?cab=$1') . "' data-toggle='tooltip'  data-original-title='' aria-describedby='tooltip' title='".lang('click_here_to_vendor')."'  ><i class='fa fa-users' aria-hidden='true'  style='color:#656464; font-size:18px'></i></a>";
			
			$delete = "<a href='" . admin_url('welcome/delete/taxi/$1') . "' data-toggle='tooltip'  data-original-title='' aria-describedby='tooltip' title='".lang('click_here_to_delete')."'  ><i class='fa fa-trash' style='color:#656464; font-size:18px'></i></a>";
	...
	function add_taxi(){
       if($this->session->userdata('group_id') == 1){
        ...
        $this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
		$this->site->users_logs($countryCode,$this->session->userdata('user_id'), $this->getUserIpAddr, json_encode($_POST), $_SERVER['REQUEST_URI']);
		$this->form_validation->set_rules('taxi_name', lang("cab_name"), 'required');
        ...
        if ($this->form_validation->run() == true && $this->taxi_model->add_taxi($taxi, $taxi_document, $countryCode)){
            ...
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('taxi'), 'page' => lang('cab')), array('link' => '#', 'page' => lang('add_cab')));
            $meta = array('page_title' => lang('add_cab'), 'bc' => $bc);
	...
	function edit_taxi($id, $view){
		$this->sma->checkPermissions('cab-edit');
		...
		if($q->num_rows()>0){
			if($q->row('status') == 0){
				$this->session->set_flashdata('error', lang("your_cab_deactive."));
            	admin_redirect('taxi');
			}
		}
        ...
        if ($this->form_validation->run() == true) {
		  ...
		  // print_r($taxi);die;
		   ...
		   if ($_FILES['photo']['size'] > 0) {
			...
			if($this->input->post('emission_norms') ==$result->emission_norms && $this->input->post('emission_norms_on') ==$result->emission_norms_on && $this->input->post('emission_norms_til') ==$result->emission_norms_til){
				$emission_norms_verify = $result->emission_norms_verify;
				$emission_norms_approved_by = $result->emission_norms_approved_by;
				$emission_norms_approved_on = $result->emission_norms_approved_on;
			}else{
				$emission_norms_verify = 0;
				$emission_norms_approved_by = 0;
				$emission_norms_approved_on = '0000:00:00 00:00:00';
			}
			
			if($this->input->post('vehicle_tracking_on') ==$result->vehicle_tracking_on && $this->input->post('vehicle_tracking_til') ==$result->vehicle_tracking_til && $_FILES['vehicle_tracking_image']['size'] == 0){
				$vehicle_tracking_verify = $result->vehicle_tracking_verify;
				$vehicle_tracking_approved_by = $result->vehicle_tracking_approved_by;
				$vehicle_tracking_approved_on = $result->vehicle_tracking_approved_on;
			}else{
				$vehicle_tracking_verify = 0;
				$vehicle_tracking_approved_by = 0;
				$vehicle_tracking_approved_on = '0000:00:00 00:00:00';
			}
			
			if($this->input->post('fire_extinguisher_on') ==$result->fire_extinguisher_on && $this->input->post('fire_extinguisher_til') ==$result->fire_extinguisher_til && $_FILES['fire_extinguisher_image']['size'] == 0){
				$fire_extinguisher_verify = $result->fire_extinguisher_verify;
				$fire_extinguisher_approved_by = $result->fire_extinguisher_approved_by;
				$fire_extinguisher_approved_on = $result->fire_extinguisher_approved_on;
			}else{
				$fire_extinguisher_verify = 0;
				$fire_extinguisher_approved_by = 0;
				$fire_extinguisher_approved_on = '0000:00:00 00:00:00';
			}
			
			if($this->input->post('child_lock_mechanism_on') ==$result->child_lock_mechanism_on && $this->input->post('child_lock_mechanism_til') ==$result->child_lock_mechanism_til && $_FILES['child_lock_mechanism_image']['size'] == 0){
				$child_lock_mechanism_verify = $result->child_lock_mechanism_verify;
				$child_lock_mechanism_approved_by = $result->child_lock_mechanism_approved_by;
				$child_lock_mechanism_approved_on = $result->child_lock_mechanism_approved_on;
			}else{
				$child_lock_mechanism_verify = 0;
				$child_lock_mechanism_approved_by = 0;
				$child_lock_mechanism_approved_on = '0000:00:00 00:00:00';
			}
			
			if($this->input->post('interior_vehicle_on') ==$result->interior_vehicle_on && $this->input->post('interior_vehicle_til') ==$result->interior_vehicle_til && $_FILES['interior_vehicle_image']['size'] == 0){
				$interior_vehicle_verify = $result->interior_vehicle_verify;
				$interior_vehicle_approved_by = $result->interior_vehicle_approved_by;
				$interior_vehicle_approved_on = $result->interior_vehicle_approved_on;
			}else{
				$interior_vehicle_verify = 0;
				$interior_vehicle_approved_by = 0;
				$interior_vehicle_approved_on = '0000:00:00 00:00:00';
			}
			
			if($this->input->post('taxi_roof_sign_on') ==$result->taxi_roof_sign_on && $this->input->post('taxi_roof_sign_til') ==$result->taxi_roof_sign_til && $_FILES['taxi_roof_sign_image']['size'] == 0){
				$taxi_roof_sign_verify = $result->taxi_roof_sign_verify;
				$taxi_roof_sign_approved_by = $result->taxi_roof_sign_approved_by;
				$taxi_roof_sign_approved_on = $result->taxi_roof_sign_approved_on;
			}else{
				$taxi_roof_sign_verify = 0;
				$taxi_roof_sign_approved_by = 0;
				$taxi_roof_sign_approved_on = '0000:00:00 00:00:00';
			}
			
			if($this->input->post('e_challans_clearance_on') ==$result->e_challans_clearance_on && $this->input->post('e_challans_clearance_til') ==$result->e_challans_clearance_til && $_FILES['e_challans_clearance_image']['size'] == 0){
				$e_challans_clearance_verify = $result->e_challans_clearance_verify;
				$e_challans_clearance_approved_by = $result->e_challans_clearance_approved_by;
				$e_challans_clearance_approved_on = $result->e_challans_clearance_approved_on;
			}else{
				$e_challans_clearance_verify = 0;
				$e_challans_clearance_approved_by = 0;
				$e_challans_clearance_approved_on = '0000:00:00 00:00:00';
			}
			...
			$taxi_document = array(
				...
				'e_challans_clearance_verify' => $e_challans_clearance_verify,
				'e_challans_clearance_approved_by' => $e_challans_clearance_approved_by,
				'e_challans_clearance_approved_on' => $e_challans_clearance_approved_on,
				
				'taxi_roof_sign_verify' => $taxi_roof_sign_verify,
				'taxi_roof_sign_approved_by' => $taxi_roof_sign_approved_by,
				'taxi_roof_sign_approved_on' => $taxi_roof_sign_approved_on,
				
				'interior_vehicle_verify' => $interior_vehicle_verify,
				'interior_vehicle_approved_by' => $interior_vehicle_approved_by,
				'interior_vehicle_approved_on' => $interior_vehicle_approved_on,
				
				'child_lock_mechanism_verify' => $child_lock_mechanism_verify,
				'child_lock_mechanism_approved_by' => $child_lock_mechanism_approved_by,
				'child_lock_mechanism_approved_on' => $child_lock_mechanism_approved_on,
				
				'fire_extinguisher_verify' => $fire_extinguisher_verify,
				'fire_extinguisher_approved_by' => $fire_extinguisher_approved_by,
				'fire_extinguisher_approved_on' => $fire_extinguisher_approved_on,
				
				'vehicle_tracking_verify' => $vehicle_tracking_verify,
				'vehicle_tracking_approved_by' => $vehicle_tracking_approved_by,
				'vehicle_tracking_approved_on' => $vehicle_tracking_approved_on,
				...
				'emission_norms_verify' => $emission_norms_verify,
				'emission_norms_approved_by' => $emission_norms_approved_by,
				'emission_norms_approved_on' => $emission_norms_approved_on,
			...
			if ($_FILES['vehicle_tracking_image']['size'] > 0) {
				...
				$taxi_document['vehicle_tracking_image'] = $result->vehicle_tracking_image;	
			}
			...
			if ($_FILES['fire_extinguisher_image']['size'] > 0) {
				...
				$taxi_document['fire_extinguisher_image'] = $result->fire_extinguisher_image;	
			}
			...
			if ($_FILES['child_lock_mechanism_image']['size'] > 0) {
				...
				$taxi_document['child_lock_mechanism_image'] = $result->child_lock_mechanism_image;	
			}
			...
			if ($_FILES['interior_vehicle_image']['size'] > 0) {
				...
				$taxi_document['interior_vehicle_image'] = $result->interior_vehicle_image;	
			}
			...
			if ($_FILES['taxi_roof_sign_image']['size'] > 0) {
				...
				$taxi_document['taxi_roof_sign_image'] = $result->taxi_roof_sign_image;	
			}
			...
			if ($_FILES['e_challans_clearance_image']['size'] > 0) {
				...
				$taxi_document['e_challans_clearance_image'] = $result->e_challans_clearance_image;	
			}
        ...
        if ($this->form_validation->run() == true && $this->taxi_model->edit_taxi($taxi, $taxi_document, $id, $countryCode)){
            ...
            $this->session->set_flashdata('message', lang("cab_edited"));
            admin_redirect('taxi');
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('taxi'), 'page' => lang('cab')), array('link' => '#', 'page' => lang('edit_cab')));
            $meta = array('page_title' => lang('edit_cab'), 'bc' => $bc);
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
	...
	function taxi_status($status, $id){
		$this->sma->checkPermissions('cab-approved');
		$result = $this->taxi_model->getTaxiDataedit($id, $countryCode);
		...
		$this->site->users_logs($countryCode,$this->session->userdata('user_id'), $this->getUserIpAddr, json_encode($_POST), $_SERVER['REQUEST_URI']);
		if($status == 'active'){
			$this->session->set_flashdata('message', lang('your_cab_active'));
            admin_redirect('taxi');
		}
		...
		$this->form_validation->set_rules('taxi_name', $this->lang->line("cab_name"), 'required');
		...
		if ($this->form_validation->run() == true) {
			...
			$data_emission_norms = array(
				'emission_norms' => $this->input->post('emission_norms'),
				'emission_norms_on' => $this->input->post('emission_norms_on'),
				'emission_norms_til' => $this->input->post('emission_norms_til'),
				'emission_norms_verify' => $this->input->post('is_approved'),
				'emission_norms_approved_by' => $this->session->userdata('user_id'),
				'emission_norms_approved_on' => date('Y-m-d H:i:s'),
			);
			
			$data_vehicle_tracking = array(
				'vehicle_tracking_on' => $this->input->post('vehicle_tracking_on'),
				'vehicle_tracking_til' => $this->input->post('vehicle_tracking_til'),
				'vehicle_tracking_verify' => $this->input->post('is_approved'),
				'vehicle_tracking_approved_by' => $this->session->userdata('user_id'),
				'vehicle_tracking_approved_on' => date('Y-m-d H:i:s'),
			);
			...
			if ($_FILES['vehicle_tracking_image']['size'] > 0) {
				...
				$vehicle_tracking_image = $this->upload->file_name;
				$data_vehicle_tracking['vehicle_tracking_image'] = 'document/vehicle_tracking/'.$vehicle_tracking_image;
				$config = NULL;
			}
			
			$data_fire_extinguisher = array(
				'fire_extinguisher_on' => $this->input->post('fire_extinguisher_on'),
				'fire_extinguisher_til' => $this->input->post('fire_extinguisher_til'),
				'fire_extinguisher_verify' => $this->input->post('is_approved'),
				'fire_extinguisher_approved_by' => $this->session->userdata('user_id'),
				'fire_extinguisher_approved_on' => date('Y-m-d H:i:s'),
			);
			...
			if ($_FILES['fire_extinguisher_image']['size'] > 0) {
				...
				$fire_extinguisher_image = $this->upload->file_name;
				$data_fire_extinguisher['fire_extinguisher_image'] = 'document/fire_extinguisher/'.$fire_extinguisher_image;
				$config = NULL;
			}
			
			$data_child_lock_mechanism = array(
				'child_lock_mechanism_on' => $this->input->post('child_lock_mechanism_on'),
				'child_lock_mechanism_til' => $this->input->post('child_lock_mechanism_til'),
				'child_lock_mechanism_verify' => $this->input->post('is_approved'),
				'child_lock_mechanism_approved_by' => $this->session->userdata('user_id'),
				'child_lock_mechanism_approved_on' => date('Y-m-d H:i:s'),
			);
			...
			if ($_FILES['child_lock_mechanism_image']['size'] > 0) {
				...
				$child_lock_mechanism_image = $this->upload->file_name;
				$data_child_lock_mechanism['child_lock_mechanism_image'] = 'document/child_lock_mechanism/'.$child_lock_mechanism_image;
				$config = NULL;
			}
			
			$data_interior_vehicle = array(
				'interior_vehicle_on' => $this->input->post('interior_vehicle_on'),
				'interior_vehicle_til' => $this->input->post('interior_vehicle_til'),
				'interior_vehicle_verify' => $this->input->post('is_approved'),
				'interior_vehicle_approved_by' => $this->session->userdata('user_id'),
				'interior_vehicle_approved_on' => date('Y-m-d H:i:s'),
			);
			...
			if ($_FILES['interior_vehicle_image']['size'] > 0) {
				...
				$interior_vehicle_image = $this->upload->file_name;
				$data_interior_vehicle['interior_vehicle_image'] = 'document/interior_vehicle/'.$interior_vehicle_image;
				$config = NULL;
			}
			
			$data_taxi_roof_sign = array(
				'taxi_roof_sign_on' => $this->input->post('taxi_roof_sign_on'),
				'taxi_roof_sign_til' => $this->input->post('taxi_roof_sign_til'),
				'taxi_roof_sign_verify' => $this->input->post('is_approved'),
				'taxi_roof_sign_approved_by' => $this->session->userdata('user_id'),
				'taxi_roof_sign_approved_on' => date('Y-m-d H:i:s'),
			);
			...
			if ($_FILES['taxi_roof_sign_image']['size'] > 0) {
				...
				$taxi_roof_sign_image = $this->upload->file_name;
				$data_taxi_roof_sign['taxi_roof_sign_image'] = 'document/taxi_roof_sign/'.$taxi_roof_sign_image;
				$config = NULL;
			}
			
			$data_e_challans_clearance = array(
				'e_challans_clearance_on' => $this->input->post('e_challans_clearance_on'),
				'e_challans_clearance_til' => $this->input->post('e_challans_clearance_til'),
				'e_challans_clearance_verify' => $this->input->post('is_approved'),
				'e_challans_clearance_approved_by' => $this->session->userdata('user_id'),
				'e_challans_clearance_approved_on' => date('Y-m-d H:i:s'),
			);
			...
			if ($_FILES['e_challans_clearance_image']['size'] > 0) {
				...
				$e_challans_clearance_image = $this->upload->file_name;
				$data_e_challans_clearance['e_challans_clearance_image'] = 'document/e_challans_clearance/'.$e_challans_clearance_image;
				$config = NULL;
			}
			
			
			$check_taxi = $this->verification_model->update_taxi_status($this->input->post('taxi_id'), $data, $status, $countryCode) && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_doc, $countryCode) && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_taxation, $countryCode)  && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_insurance, $countryCode)  && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_permit, $countryCode)  && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_authorisation, $countryCode)  && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_fitness, $countryCode)  && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_speed, $countryCode) && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_puc, $countryCode) && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_emission_norms, $countryCode) && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_vehicle_tracking, $countryCode) && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_fire_extinguisher, $countryCode) && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_child_lock_mechanism, $countryCode) && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_interior_vehicle, $countryCode) && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_taxi_roof_sign, $countryCode) && $this->verification_model->update_taxi_common_status($this->input->post('document_id'), $data_e_challans_clearance, $countryCode);
		...
		if ($this->form_validation->run() && $check_taxi)
		{
			...
			$this->site->taxiServicesInsert($result->driver_id, $id, $this->input->post('type'), $countryCode);
			
			$q = $this->db->select("t.id as id, t.name as taxi_name, t.number, t.model, t.engine_number, t.chassis_number,  t.fuel_type, tt.name as type_name,   If(t.is_verify = 1 && td.reg_verify = 1 && td.taxation_verify = 1 && td.insurance_verify = 1 && td.permit_verify = 1 && td.authorisation_verify = 1 && td.fitness_verify = 1 && td.speed_verify = 1 && td.puc_verify = 1  && td.emission_norms_verify = 1  && td.vehicle_tracking_verify = 1  && td.fire_extinguisher_verify = 1  && td.child_lock_mechanism_verify = 1  && td.interior_vehicle_verify = 1  && td.taxi_roof_sign_verify = 1  && td.e_challans_clearance_verify = 1 , '1', '0') as status ")
            ->from("taxi t")
			->join("taxi_type tt", "tt.id = t.type ", "left")
			->join("taxi_document td", "td.taxi_id = t.id AND td.is_edit = 1 ", "left")
			->where("t.id", $id)
			...
			if($q->num_rows()>0){
				if($q->row('status') == 0){
					$this->session->set_flashdata('error', $this->input->post('first_name_hidden').lang('details_has_been_not_verified'));
					admin_redirect('taxi');
				}else{
				...
				$this->site->insertNotification($notification);
					...
					$this->session->set_flashdata('message', $this->input->post('first_name_hidden').lang('details_has_been_verified'));
            		admin_redirect('taxi');
				}
			}
