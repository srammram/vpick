...
class Formsfranchisee extends MY_Controller
{
    ...
    function __construct()
    {
       ...
       $this->image_types = 'gif|jpg|png|jpeg|pdf';
		...
		$this->load->admin_model('formsfranchisee_model');
    }
    ...
    {
		$this->sma->checkPermissions('formsfranchisee-index');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('Formsfranchisee')));
        $meta = array('page_title' => lang('Formsfranchisee'), 'bc' => $bc);
        $this->page_construct('forms/franchisee', $meta, $this->data);
    }
	 function formsfranchisee_actions($wh = NULL)
    {
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
       ...
       $this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
        ...
        if ($this->form_validation->run() == true) {
                ...
                if ($this->input->post('form_action') == 'export_excel') {
                    ...
                    $this->excel->getActiveSheet()->setTitle('Formsfranchisee');
                    ...
                    $row = 2;
					$res = $this->formsfranchisee_model->getALLFormsfranchisee($countryCode);
					if(!empty($res)){
