...
class Returns extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->lang->admin_load('procurment/returns', $this->Settings->user_language);
        $this->load->library('form_validation');
        $this->load->admin_model('procurment/returns_model');
    ...
    public function index($warehouse_id = null)
    {
        ...
        $this->page_construct('procurment/returns/index', $meta, $this->data);
    }
    ...
    public function getReturns($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("{$this->db->dbprefix('pro_returns')}.id as id, DATE_FORMAT({$this->db->dbprefix('pro_returns')}.date, '%Y-%m-%d %T') as date, reference_no, biller, {$this->db->dbprefix('pro_returns')}.customer, grand_total, {$this->db->dbprefix('pro_returns')}.attachment")
                ->from('pro_returns')
        ...
        $this->datatables->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('procurment/returns/edit/$1') . "' class='tip' title='" . lang("edit_return") . "'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_return") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('procurment/returns/delete/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $reference = $this->input->post('reference_no') ? $this->input->post('reference_no') : $this->siteprocurment->getReference('re');
            $warehouse_id = $this->input->post('warehouse');
            $customer_id = $this->input->post('customer');
            $biller_id = $this->input->post('biller');
            $total_items = $this->input->post('total_items');
            $customer_details = $this->siteprocurment->getCompanyByID($customer_id);
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    $product_details = $item_type != 'manual' ? $this->siteprocurment->getProductByCode($item_code) : null;
                    ...
                    if (isset($item_tax_rate) && $item_tax_rate != 0) {
                        ...
                        $ctax = $this->siteprocurment->calculateTax($product_details, $tax_details, $unit_price);
            ...
            $order_discount = $this->siteprocurment->calculateDiscount($this->input->post('order_discount'), ($total + $product_tax));
            $total_discount = $this->sma->formatDecimal(($order_discount + $product_discount), 4);
            $order_tax = $this->siteprocurment->calculateOrderTax($this->input->post('order_tax'), ($total + $product_tax - $order_discount));
        ...
        if ($this->form_validation->run() == true && $this->returns_model->addReturn($data, $products)) {
            ...
            admin_redirect("procurment/returns");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('procurment/returns'), 'page' => lang('returns')), array('link' => '#', 'page' => lang('add_return')));
            $meta = array('page_title' => lang('add_return'), 'bc' => $bc);
            $this->page_construct('procurment/returns/add', $meta, $this->data);
        }
    }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->returns_model->updateReturn($id, $data, $products)) {
            ...
            foreach ($inv_items as $item) {
                ...
                $combo_items = false;
                if ($row->type == 'combo') {
                    $combo_items = $this->siteprocurment->getProductComboItems($row->id);
                }
            ...
            $this->data['warehouses'] = $this->siteprocurment->getAllWarehouses();
            $this->data['tax_rates'] = $this->siteprocurment->getAllTaxRates();
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('procurment/returns'), 'page' => lang('returns')), array('link' => '#', 'page' => lang('edit_return')));
            $meta = array('page_title' => lang('edit_return'), 'bc' => $bc);
            $this->page_construct('procurment/returns/edit', $meta, $this->data);
        }
    }
