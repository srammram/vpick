...
class Quotes extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->lang->admin_load('procurment/quotes', $this->Settings->user_language);
        $this->load->library('form_validation');
        $this->load->admin_model('procurment/quotes_model');
        $this->digital_upload_path = 'files/';
        $this->upload_path = 'assets/uploads/';
        $this->thumbs_path = 'assets/uploads/thumbs/';
        $this->image_types = 'gif|jpg|jpeg|png|tif';
        $this->digital_upload_path = 'assets/uploads/procurment/quotes/';
	...
	public function quotes_list(){
		...
		foreach ($inv_items as $item) {
			...
			$ri = $this->Settings->item_addition ? $row->id : $row->id;
			 $ri = $row->id;
			$pr[$ri.'_'.$item->store_id] = array('store_id'=>$item->store_id,'id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")",
    ...
    { 
        ...
        $email_link = anchor('admin/procurment/quotes/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_quotes'), 'data-toggle="modal" data-target="#myModal"');
        $edit_link = anchor('admin/procurment/quotes/edit/$1', '<i class="fa fa-edit"></i> ' . lang('edit_quotes'));
        $view_link = '<a href="'.admin_url('procurment/quotes/view/$1').'" data-toggle="modal" data-target="#myModal"><i class="fa fa-edit"></i>'.lang('view_purchase_quotes').'</a>';
        $pdf_link = anchor('admin/procurment/quotes/pdf/$1', '<i class="fa fa-file-pdf-o"></i> ' . lang('download_pdf'));
        $print_barcode = anchor('admin/procurment/products/print_barcodes/?purchase=$1', '<i class="fa fa-print"></i> ' . lang('print_barcodes'));
        $return_link = anchor('admin/procurment/quotes/return_purchase/$1', '<i class="fa fa-angle-double-left"></i> ' . lang('return_purchase'));
...
// echo "string";exit;
        ...
        // echo "fg";exit;    
            $this->datatables
                ->select("id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, req_reference_no, supplier, status,attachment")
                ->from('pro_quotes')
    ...
    public function modal_view($quotes_id = null)
    {
        ...
        $this->data['rows'] = $this->quotes_model->getAllQuotesItems($quotes_id);
...
$inv_items = $this->quotes_model->getAllQuotesItems($id);   
	    ...
	    //echo '<pre>';print_R($this->data['quotations']);exit;
            $this->data['id'] = $id;
...
//generate pdf and force to download
    ...
    public function pdf($quotes_id = null, $view = null, $save_bufffer = null)
    {
        ...
        $this->data['return_purchase'] = $po->return_id ? $this->quotes_model->getQuotesByID($po->return_id) : NULL;
        $this->data['return_rows'] = $po->return_id ? $this->quotes_model->getAllQuotesItems($po->return_id) : NULL;
        $name = $this->lang->line("quotes") . "_" . str_replace('/', '_', $po->reference_no) . ".pdf";
    ...
    public function combine_pdf($quotes_id)
    {
        ...
        foreach ($quotes_id as $quotes_id) {
            ...
            $this->data['inv'] = $inv;
            $this->data['return_purchase'] = $inv->return_id ? $this->quotes_model->getQuotesByID($inv->return_id) : NULL;
            $this->data['return_rows'] = $inv->return_id ? $this->quotes_model->getAllQuotesItems($inv->return_id) : NULL;
    ...
    public function email($quotes_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $attachment = $this->pdf($quotes_id, null, 'S');

            try {
                if ($this->sma->send_email($to, $subject, $message, null, null, $attachment, $cc, $bcc)) {
                    delete_files($attachment);
                    $this->db->update('quotes', array('status' => 'ordered'), array('id' => $quotes_id));
            ...
            if (file_exists('./themes/' . $this->Settings->theme . '/admin/views/email_templates/quotes.html')) {
                ...
                $quotes_temp = file_get_contents('./themes/default/admin/views/email_templates/quotes.html');
            }
            $this->data['subject'] = array('name' => 'subject',
                'id' => 'subject',
                'type' => 'text',
                'value' => $this->form_validation->set_value('subject', lang('quotes').' (' . $po->reference_no . ') '.lang('from').' ' . $this->Settings->site_name),
            );
    ...
    public function add($quotes_id = null)
    {
        ////$this->sma->checkPermissions();
		
		if(!$this->siteprocurment->GETaccessModules('purchase_quatation_add'))
		{
			$this->session->set_flashdata('warning', lang('access_denied'));
            redirect($_SERVER["HTTP_REFERER"]);	
		}
       ...
       // $this->form_validation->set_message('is_natural_no_zero', $this->lang->line("no_zero_required"));
		...
		$this->form_validation->set_rules('quotes_no', $this->lang->line("quotes_no"), 'required');*/

        $this->session->unset_userdata('csrf_token');
        if ($this->form_validation->run() == true) {          
	    $n = $this->siteprocurment->lastidQuotation();
	    $reference = 'PQ'.str_pad($n + 1, 5, 0, STR_PAD_LEFT);
            //$reference = $this->input->post('quotation_no') ? $this->input->post('quotation_no') : $new_reference;
			// $reference = 'QU'.date('YmdHis');            
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($unit_cost) &&  isset($quantity)) {
                    ...
                    $pr_item_discount = $this->sma->formatDecimal($pr_discount * $quantity);
                    ...
                    if (isset($item_tax_rate) && $item_tax_rate != 0) {
                        ...
                        $pr_item_tax = $this->sma->formatDecimal($item_tax * $quantity, 4);
...
/*common_quote_items*/
			...
			$join_ref_no = $this->quotes_model->getReqBYID($this->input->post('requestnumber'));
            /*echo "<pre>";
            print_r($join_ref_no);die;*/
			...
			if($status == 'process'){
             ...
             $data = array('date' => $date,
                ...
                'request_id' => $join_ref_no->id ? $join_ref_no->id : 0,
		...
		'total_qty' => $this->input->post('total_no_qty'),
           ...
           //$this->sma->print_arrays($data, $products, $quotes_array, $this->input->post('requestnumber'));
		   //die;
        }
        // echo "<pre>";
        // print_r($data);die;
        ...
        // var_dump($products);
        var_dump($this->input->post('requestnumber'));
        var_dump($quotes_array);
        die;
		 */
        if ($this->form_validation->run() == true && $this->quotes_model->addQuotes($data, $products, $quotes_array, $this->input->post('requestnumber'))) {
            ...
            if ($quotes_id) {
                ...
                $this->data['quotes_items'] = json_encode($pr);
            }
				
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['quotes_id'] = $quotes_id;
           $this->data['suppliers'] = $this->siteprocurment->getAllCompanies('supplier');
           $this->data['currencies'] = $this->siteprocurment->getAllCurrencies();
            $this->data['categories'] = $this->siteprocurment->getAllCategories();
           $this->data['tax_rates'] = $this->siteprocurment->getAllTaxRates();
           ...
           $this->data['ponumber'] = ''; //$this->siteprocurment->getReference('po');
            ...
            $this->session->set_userdata('user_csrf', $value);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('procurment/quotes'), 'page' => lang('quotes')), array('link' => '#', 'page' => lang('add_quotes')));
            $meta = array('page_title' => lang('add_quotes'), 'bc' => $bc);
            $this->page_construct('procurment/quotes/add', $meta, $this->data);
        }
    }
    ...
    public function edit($id = null)
    {
        ...
        $inv = $this->quotes_model->getQuotesByID($id);
       ...
       /* if ($inv->status == 'returned' || $inv->return_id || $inv->return_purchase_ref) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $reference = $this->input->post('quotation_no');
...
// echo "string";die;
            ...
            $grand_total = $this->sma->formatDecimal(($total + $total_tax + $this->sma->formatDecimal($shipping) - $order_discount), 4);
            $join_ref_no = $this->quotes_model->getReqBYID($this->input->post('requestnumber'));
            /*update common_quotes*/
            
             $data = array('date' => $date,
                'reference_no' => $reference ?  $reference : 'QU'.date('YmdHis'),
                // 'biller_id' => $biller_id,
                // 'biller' => $biller,
                'supplier_id' => $supplier_id,
                'request_id' => $join_ref_no->id ? $join_ref_no->id : 0,
                'requestdate' => $join_ref_no->date ? $join_ref_no->date : $date,
                'req_reference_no' => $join_ref_no->reference_no ? $join_ref_no->reference_no : '',
                'supplier' => $supplier,
                'supplier_address' => $supplier_address,
                'warehouse_id' => $warehouse_id,
        ...
        if ($this->form_validation->run() == true && $this->quotes_model->updateQuotes($id, $data, $item)) {             
            $this->session->set_userdata('remove_pols', 1);
            $this->session->set_flashdata('message', $this->lang->line("quotes_updated"));
            admin_redirect('procurment/quotes');
        } else {
            ...
            if ($this->Settings->disable_editing) {
                if ($this->data['inv']->date <= date('Y-m-d', strtotime('-'.$this->Settings->disable_editing.' days'))) {
                    $this->session->set_flashdata('error', sprintf(lang("quotes_x_edited_older_than_x_days"), $this->Settings->disable_editing));
                    redirect($_SERVER["HTTP_REFERER"]);
                }
            }
            $inv_items = $this->quotes_model->getAllQuotesItems($id);   
            ...
            $this->data['warehouses'] = $this->siteprocurment->getAllWarehouses();
			$this->data['ref_requestnumber'] = $_GET['ref'];
			 $this->data['requestnumber'] = $this->siteprocurment->getAllREQUESTNUMBERedit();
            ...
            echo "</pre>";*/
            $this->page_construct('procurment/quotes/edit', $meta, $this->data);
        }
    }
    ...
    public function quotes_by_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                if (!$this->upload->do_upload()) {
                    $error = $this->upload->display_errors();
                    $this->session->set_flashdata('error', $error);
                    admin_redirect("procurment/quotes/quotes_by_csv");
                }
                ...
                foreach ($final as $csv_pr) {
                    ...
                    if (isset($csv_pr['code']) && isset($csv_pr['net_unit_cost']) && isset($csv_pr['quantity'])) {
                        ...
                        if ($product_details = $this->quotes_model->getProductByCode($csv_pr['code'])) {
                            ...
                            $gst_data = [];
                            $tax_details = ((isset($item_tax_rate) && !empty($item_tax_rate)) ? $this->quotes_model->getTaxRateByName($item_tax_rate) : $this->siteprocurment->getTaxRateByID($product_details->tax_rate));
                            if ($tax_details) {
                                $ctax = $this->siteprocurment->calculateTax($product_details, $tax_details, $unit_cost);
                                $item_tax = $ctax['amount'];
                                $tax = $ctax['tax'];
        ...
        if ($this->form_validation->run() == true && $this->quotes_model->addPurchase($data, $products)) {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('procurment/quotes'), 'page' => lang('quotes')), array('link' => '#', 'page' => lang('add_quotes_by_csv')));
            $meta = array('page_title' => lang('add_quotes_by_csv'), 'bc' => $bc);
            $this->page_construct('procurment/quotes/quotes__orderby_csv', $meta, $this->data);
   ...
   public function delete($id = null)
    {
        ...
        if ($this->quotes_model->deleteQuotes($id)) {

            if ($this->input->is_ajax_request()) {
                $this->sma->send_json(array('error' => 0, 'msg' => lang("quotes_deleted")));
            }
            $this->session->set_flashdata('message', lang('quotes_deleted'));
    ...
    public function suggestions()
    {
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $options = $this->quotes_model->getProductOptions($row->id);
                if ($options) {
                    $opt = $option_id && $r == 0 ? $this->quotes_model->getProductOptionByID($option_id) : current($options);
                    if (!$option_id || $r > 0) {
                        $option_id = $opt->id;
                    }
                } else {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        $this->quotes_model->deleteQuotes($id);
                    }
                    ...
                    $row = 2;
                    foreach ($_POST['val'] as $id) {
                        $quotes = $this->quotes_model->getQuotesByID($id);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($quotes->date));
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $quotes->reference_no);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $quotes->supplier);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $quotes->status);
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $this->sma->formatMoney($quotes->grand_total));
                        $row++;
                    }
                    ...
                    $filename = 'quotes_' . date('Y_m_d_H_i_s');
                    $this->load->helper('excel');
                    create_excel($this->excel, $filename);
                }
            } else {
                $this->session->set_flashdata('error', $this->lang->line("no_quotes_selected"));
                redirect($_SERVER["HTTP_REFERER"]);
            }
        } else {
    ...
    public function payments($id = null)
    {
        ...
        $this->data['payments'] = $this->quotes_model->getPurchasePayments($id);
        $this->data['inv'] = $this->quotes_model->getQuotesByID($id);
    ...
    public function payment_note($id = null)
    {
        ...
        $payment = $this->quotes_model->getPaymentByID($id);
        $inv = $this->quotes_model->getQuotesByID($payment->purchase_id);
        $this->data['supplier'] = $this->siteprocurment->getCompanyByID($inv->supplier_id);
        $this->data['warehouse'] = $this->siteprocurment->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
    ...
    public function add_payment($id = null)
    {
        ...
        $purchase = $this->quotes_model->getQuotesByID($id);
        ...
        if ($this->form_validation->run() == true && $this->quotes_model->addPayment($payment)) {
    ...
    public function edit_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->quotes_model->updatePayment($id, $payment)) {
            ...
            $this->data['payment'] = $this->quotes_model->getPaymentByID($id);
            $this->data['modal_js'] = $this->siteprocurment->modal_js();
    ...
    public function delete_payment($id = null)
    {
        ...
        if ($this->quotes_model->deletePayment($id)) {
    ...
    public function expenses($id = null)
    {
        ...
        $this->page_construct('procurment/quotes/expenses', $meta, $this->data);
    }
    ...
    public function getExpenses()
    {
        ...
        $detail_link = anchor('admin/procurment/quotes/expense_note/$1', '<i class="fa fa-file-text-o"></i> ' . lang('expense_note'), 'data-toggle="modal" data-target="#myModal2"');
        $edit_link = anchor('admin/procurment/quotes/edit_expense/$1', '<i class="fa fa-edit"></i> ' . lang('edit_expense'), 'data-toggle="modal" data-target="#myModal"');
        ...
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('procurment/quotes/delete_expense/$1') . "'>"
        . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
    ...
    {
        $expense = $this->quotes_model->getExpenseByID($id);
        $this->data['user'] = $this->siteprocurment->getUser($expense->created_by);
        $this->data['category'] = $expense->category_id ? $this->quotes_model->getExpenseCategoryByID($expense->category_id) : NULL;
    ...
    public function add_expense()
    {
        ...
        if ($this->form_validation->run() == true && $this->quotes_model->addExpense($data)) {
            ...
            $this->data['warehouses'] = $this->siteprocurment->getAllWarehouses();
            $this->data['categories'] = $this->quotes_model->getExpenseCategories();
            $this->data['modal_js'] = $this->siteprocurment->modal_js();
    ...
    public function edit_expense($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->quotes_model->updateExpense($id, $data)) {
            $this->session->set_flashdata('message', lang("expense_updated"));
            admin_redirect("procurment/quotes/expenses");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['expense'] = $this->quotes_model->getExpenseByID($id);
            $this->data['warehouses'] = $this->siteprocurment->getAllWarehouses();
            $this->data['modal_js'] = $this->siteprocurment->modal_js();
            $this->data['categories'] = $this->quotes_model->getExpenseCategories();
    ...
    public function delete_expense($id = null)
    {
        ...
        $expense = $this->quotes_model->getExpenseByID($id);
        if ($this->quotes_model->deleteExpense($id)) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    //$this->sma->checkPermissions('delete');
                    foreach ($_POST['val'] as $id) {
                        $this->quotes_model->deleteExpense($id);
                    }
    ...
    {
        //$this->sma->checkPermissions('return_quotes');

        if ($this->input->get('id')) {
            $id = $this->input->get('id');
        }
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $inv = $this->quotes_model->getReturnByID($id);
        if (!$this->session->userdata('view_right')) {
            $this->sma->view_rights($inv->created_by);
        }
        ...
        $this->data['supplier'] = $this->siteprocurment->getCompanyByID($inv->supplier_id);
        $this->data['payments'] = $this->quotes_model->getPaymentsForPurchase($id);
        $this->data['user'] = $this->siteprocurment->getUser($inv->created_by);
        $this->data['warehouse'] = $this->siteprocurment->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
        $this->data['rows'] = $this->quotes_model->getAllReturnItems($id);
        $this->data['purchase'] = $this->quotes_model->getQuotesByID($inv->purchase_id);
    ...
    public function return_purchase($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->quotes_model->addPurchase($data, $products)) {
            ...
            $inv_items = $this->quotes_model->getAllPurchaseItems($id);
             krsort($inv_items);
            $c = rand(100000, 9999999);
            foreach ($inv_items as $item) {
                $row = $this->siteprocurment->getProductByID($item->product_id);
            ...
            $this->data['reference'] = '';
            $this->data['tax_rates'] = $this->siteprocurment->getAllTaxRates();
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('procurment/quotes'), 'page' => lang('quotes')), array('link' => '#', 'page' => lang('return_purchase')));
            $meta = array('page_title' => lang('return_purchase'), 'bc' => $bc);
            $this->page_construct('procurment/quotes/return_purchase', $meta, $this->data);
        }
    }
