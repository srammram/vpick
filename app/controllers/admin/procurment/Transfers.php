...
class Transfers extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->lang->admin_load('procurment/transfers', $this->Settings->user_language);
        $this->load->library('form_validation');
        $this->load->admin_model('procurment/transfers_model');
    ...
    function index()
    {
        ...
        $this->page_construct('procurment/transfers/index', $meta, $this->data);
    }
    ...
    function getTransfers()
    {
        ...
        $detail_link = anchor('admin/procurment/transfers/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('transfer_details'), 'data-toggle="modal" data-target="#myModal"');
        $email_link = anchor('admin/procurment/transfers/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_transfer'), 'data-toggle="modal" data-target="#myModal"');
        $edit_link = anchor('admin/procurment/transfers/edit/$1', '<i class="fa fa-edit"></i> ' . lang('edit_transfer'));
        $pdf_link = anchor('admin/procurment/transfers/pdf/$1', '<i class="fa fa-file-pdf-o"></i> ' . lang('download_pdf'));
        $print_barcode = anchor('admin/procurment/products/print_barcodes/?transfer=$1', '<i class="fa fa-print"></i> ' . lang('print_barcodes'));
        $delete_link = "<a href='#' class='tip po' title='<b>" . lang("delete_transfer") . "</b>' data-content=\"<p>"
            . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' id='a__$1' href='" . admin_url('procurment/transfers/delete/$1') . "'>"
    ...
    function add()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $transfer_no = $this->input->post('reference_no') ? $this->input->post('reference_no') : $this->siteprocurment->getReference('to');
            ...
            $from_warehouse_details = $this->siteprocurment->getWarehouseByID($from_warehouse);
            $from_warehouse_code = $from_warehouse_details->code;
            $from_warehouse_name = $from_warehouse_details->name;
            $to_warehouse_details = $this->siteprocurment->getWarehouseByID($to_warehouse);
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    ...
                    // if (!$this->Settings->overselling) {
                        ...
                        if ($warehouse_quantity->quantity < $item_quantity) {
                            ...
                            admin_redirect("procurment/transfers/add");
                        }
                   // }
        ...
        if ($this->form_validation->run() == true && $this->transfers_model->addTransfer($data, $products)) {
            ...
            admin_redirect("procurment/transfers");
        } else {
            ...
            $this->data['rnumber'] = ''; //$this->siteprocurment->getReference('to');

            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('procurment/transfers'), 'page' => lang('transfers')), array('link' => '#', 'page' => lang('add_transfer')));
            $meta = array('page_title' => lang('transfer_quantity'), 'bc' => $bc);
            $this->page_construct('procurment/transfers/add', $meta, $this->data);
        }
    }
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->transfers_model->updateTransfer($id, $data, $products)) {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('procurment/transfers'), 'page' => lang('transfers')), array('link' => '#', 'page' => lang('edit_transfer')));
            $meta = array('page_title' => lang('edit_transfer_quantity'), 'bc' => $bc);
            $this->page_construct('procurment/transfers/edit', $meta, $this->data);
        }
    }
    ...
    function transfer_by_csv()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                if (!$this->upload->do_upload()) {
                    $error = $this->upload->display_errors();
                    $this->session->set_flashdata('error', $error);
                    admin_redirect("procurment/transfers/transfer_bt_csv");
                }
                ...
                foreach ($final as $csv_pr) {
                    ...
                    if (isset($item_code) && isset($unit_cost) && isset($item_quantity)) {
                        ...
                        if (isset($product_details->tax_rate) && $product_details->tax_rate != 0) {
                            ...
                            $tax_details = $this->siteprocurment->getTaxRateByID($product_details->tax_rate);
                            $ctax = $this->siteprocurment->calculateTax($product_details, $tax_details, $unit_cost);
                            $item_tax = $ctax['amount'];
                            $tax = $ctax['tax'];
                            if (!empty($product_details) && $product_details->tax_method != 1) {
        ...
        if ($this->form_validation->run() == true && $this->transfers_model->addTransfer($data, $products)) {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('procurment/transfers'), 'page' => lang('transfers')), array('link' => '#', 'page' => lang('transfer_by_csv')));
            $meta = array('page_title' => lang('add_transfer_by_csv'), 'bc' => $bc);
            $this->page_construct('procurment/transfers/transfer_by_csv', $meta, $this->data);
        }
    }
    ...
    function view($transfer_id = NULL)
    {
        ...
        $this->data['from_warehouse'] = $this->siteprocurment->getWarehouseByID($transfer->from_warehouse_id);
        $this->data['to_warehouse'] = $this->siteprocurment->getWarehouseByID($transfer->to_warehouse_id);
        $this->data['transfer'] = $transfer;
        $this->data['tid'] = $transfer_id;
        $this->data['created_by'] = $this->siteprocurment->getUser($transfer->created_by);
    ...
    function email($transfer_id = NULL)
    {
        ...
        $transfer = $this->transfers_model->getTransferByID($transfer_id);
        $this->form_validation->set_rules('to', lang("to") . " " . lang("email_address"), 'trim|required|valid_email');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->data['warehouse'] = $this->siteprocurment->getWarehouseByID($transfer->to_warehouse_id);
