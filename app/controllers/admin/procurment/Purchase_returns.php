...
class Purchase_returns extends MY_Controller
{
	...
	public function purchase_returns_list(){
		...
		$data['purchase_returns'] = $this->purchase_returns_model->getRequestByID($poref);
		
		$inv_items = $this->purchase_returns_model->getAllRequestItems($poref);
	...
	public function supplier(){
		...
		$data = $this->purchase_returns_model->getSupplierdetails($supplier_id);
    ...
    { 
        ...
        $email_link = anchor('admin/procurment/purchase_returns/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_purchase_returns'), 'data-toggle="modal" data-target="#myModal"');
        $edit_link = anchor('admin/procurment/purchase_returns/edit/$1', '<i class="fa fa-edit"></i> ' . lang('edit_purchase_returns'));
        ...
        . lang('delete_purchase_returns') . "</a>";
...
// echo "string";exit;
        ...
        // echo "fg";exit;    
            $this->datatables
                ->select("id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, req_reference_no, supplier, total, total_discount, total_tax, grand_total, status")
    ...
    public function modal_view($purchase_returns_id = null)
    {
        //$this->sma->checkPermissions('index', true);

        if ($this->input->get('id')) {
            $purchase_returns_id = $this->input->get('id');
        }
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $po = $this->purchase_returns_model->getPurchase_returnsByID($purchase_returns_id);
        if (!$this->session->userdata('view_right')) {
            $this->sma->view_rights($po->created_by, true);
        }
        $this->data['rows'] = $this->purchase_returns_model->getAllPurchase_returnsItems($purchase_returns_id);
        ...
        $this->data['inv'] = $po;
        $this->data['payments'] = $this->purchase_returns_model->getPaymentsForPurchase($purchase_returns_id);
        ...
        // $this->data['return_purchase'] = $po->return_id ? $this->purchase_returns_model->getPurchase_returnsByID($po->return_id) : NULL;
        // $this->data['return_rows'] = $po->return_id ? $this->purchase_returns_model->getAllPurchase_returnsItems($po->return_id) : NULL;

        $this->load->view($this->theme . 'purchase_returns/modal_view', $this->data);

    }

    public function view($purchase_returns_id = null)
    {
        ...
        $this->data['inv'] = $po;
        //$this->data['payments'] = $this->purchase_returns_model->getPaymentsForPurchase_returns($purchase_returns_id);
        $this->data['created_by'] = $this->siteprocurment->getUser($po->created_by);
        $this->data['updated_by'] = $po->updated_by ? $this->siteprocurment->getUser($po->updated_by) : null;
        $this->data['return_purchase'] = $po->return_id ? $this->purchase_returns_model->getPurchase_returnsByID($po->return_id) : NULL;
        $this->data['return_rows'] = $po->return_id ? $this->purchase_returns_model->getAllPurchase_returnsItems($po->return_id) : NULL;

        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('procurment/purchase_returns'), 'page' => lang('purchase_returns')), array('link' => '#', 'page' => lang('view')));
        $meta = array('page_title' => lang('view_purchase_returns_details'), 'bc' => $bc);
        $this->page_construct('procurment/purchase_returns/view', $meta, $this->data);
...
//generate pdf and force to download
    ...
    public function pdf($purchase_returns_id = null, $view = null, $save_bufffer = null)
    {
        //$this->sma->checkPermissions();
        ...
        $this->data['return_purchase'] = $po->return_id ? $this->purchase_returns_model->getPurchase_returnsByID($po->return_id) : NULL;
        $this->data['return_rows'] = $po->return_id ? $this->purchase_returns_model->getAllPurchase_returnsItems($po->return_id) : NULL;
        $name = $this->lang->line("purchase_returns") . "_" . str_replace('/', '_', $po->reference_no) . ".pdf";
        $html = $this->load->view($this->theme . 'purchase_returns/pdf', $this->data, true);
        if (! $this->Settings->barcode_img) {
            $html = preg_replace("'\<\?xml(.*)\?\>'", '', $html);
        }
        if ($view) {
            $this->load->view($this->theme . 'purchase_returns/pdf', $this->data);
        } elseif ($save_bufffer) {
    ...
    public function combine_pdf($purchase_returns_id)
    {
        //$this->sma->checkPermissions('pdf');

        foreach ($purchase_returns_id as $purchase_returns_id) {

            $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
            $inv = $this->purchase_returns_model->getPurchase_returnsByID($purchase_returns_id);
            if (!$this->session->userdata('view_right')) {
            ...
            $this->data['inv'] = $inv;
            $this->data['return_purchase'] = $inv->return_id ? $this->purchase_returns_model->getPurchase_returnsByID($inv->return_id) : NULL;
            $this->data['return_rows'] = $inv->return_id ? $this->purchase_returns_model->getAllPurchase_returnsItems($inv->return_id) : NULL;
            $inv_html = $this->load->view($this->theme . 'purchase_returns/pdf', $this->data, true);
            if (! $this->Settings->barcode_img) {
        ...
        $name = lang("purchase_returns") . ".pdf";
        $this->sma->generate_pdf($html, $name);

    }

    public function email($purchase_returns_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $attachment = $this->pdf($purchase_returns_id, null, 'S');

            try {
                if ($this->sma->send_email($to, $subject, $message, null, null, $attachment, $cc, $bcc)) {
                    delete_files($attachment);
                    $this->db->update('purchase_returns', array('status' => 'ordered'), array('id' => $purchase_returns_id));
            ...
            if (file_exists('./themes/' . $this->Settings->theme . '/admin/views/email_templates/purchase_returns.html')) {
                $purchase_returns_temp = file_get_contents('themes/' . $this->Settings->theme . '/admin/views/email_templates/purchase_returns.html');
            } else {
                $purchase_returns_temp = file_get_contents('./themes/default/admin/views/email_templates/purchase_returns.html');
            }
            $this->data['subject'] = array('name' => 'subject',
                'id' => 'subject',
                'type' => 'text',
                'value' => $this->form_validation->set_value('subject', lang('purchase_returns').' (' . $po->reference_no . ') '.lang('from').' ' . $this->Settings->site_name),
            );
            $this->data['note'] = array('name' => 'note',
                'id' => 'note',
                'type' => 'text',
                'value' => $this->form_validation->set_value('note', $purchase_returns_temp),
            );
            $this->data['supplier'] = $this->siteprocurment->getCompanyByID($po->customer_id);

            $this->data['id'] = $purchase_returns_id;
            $this->data['modal_js'] = $this->siteprocurment->modal_js();
            $this->load->view($this->theme . 'purchase_returns/email', $this->data);
	...
	public function saveStorepurchasewise(){
		...
		$result = $this->purchase_returns_model->addStorepurchasewise($insert_array, $quote_id, $product_id);
		if(!empty($result))
		{
			$response['status'] = 'Success';
		}else{
	...
	public function getProductStores(){
		...
		$res = $this->purchase_returns_model->productQuotesID($product_id, $quote_id);
		
		$result = $this->purchase_returns_model->getProductStores($product_id);
    ...
    public function add($purchase_returns_id = null)
    {
        //$this->sma->checkPermissions();
       ...
       // $this->form_validation->set_message('is_natural_no_zero', $this->lang->line("no_zero_required"));
		...
		$this->form_validation->set_rules('purchase_returns_no', $this->lang->line("purchase_returns_no"), 'required');*/
		$store_id = $this->siteprocurment->defaultStores();
        $this->session->unset_userdata('csrf_token');
        if ($this->form_validation->run() == true) {          
            
			$reference = 'PN'.date('YmdHis');            
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    $product_details = $this->purchase_returns_model->getProductByCode($item_code);
                    // if ($item_expiry) {
                    //     $today = date('Y-m-d');
...
/*common_quote_items*/
			...
			$join_ref_no = $this->purchase_returns_model->getReqBYID($this->input->post('requestnumber'));
            ...
            $data = array(
				...
				//'purchase_returns_details' => $this->input->post('purchase_returns_details') ? $this->input->post('purchase_returns_details') : '',
				//'purchase_returns_no' => $this->input->post('purchase_returns_no'),
				//'purchase_returns_date' => $this->input->post('purchase_returns_date'),
				//'purchase_returns_expected_date' => $this->input->post('purchase_returns_expected_date'),
                'note' => $note,
                'total' => $total,
			...
			if($this->input->post('requestnumber') != ''){
				$purchase_returns_array = array(
					'status' => 'completed',
				);
			}
			
           //$this->sma->print_arrays($data, $products, $purchase_returns_array, $this->input->post('requestnumber'));
		   //die;
        }
		 
        if ($this->form_validation->run() == true && $this->purchase_returns_model->addPurchase_returns($data, $products, $purchase_returns_array, $this->input->post('requestnumber'))) {
            
            $this->session->set_userdata('remove_pols', 1);
            $this->session->set_flashdata('message', $this->lang->line("purchase_returns_added"));
            admin_redirect('procurment/purchase_returns');
        } else {
			
			
            if ($purchase_returns_id) {
                $this->data['purchase_returns'] = $this->purchase_returns_model->getPurchase_returnsByID($purchase_returns_id);
                $supplier_id = $this->data['purchase_returns']->supplier_id;
                $items = $this->purchase_returns_model->getAllPurchase_returnsItems($purchase_returns_id);
                krsort($items);
                $c = rand(100000, 9999999);
                foreach ($items as $item) {
                    ...
                    if ($row->type == 'combo') {
                        ...
                        foreach ($combo_items as $citem) {
                            ...
                            $crow->expiry = '';
                            $options = $this->purchase_returns_model->getProductOptions($crow->id);
                            $units = $this->siteprocurment->getUnitsByBUID($row->base_unit);
                ...
                $this->data['purchase_returns_items'] = json_encode($pr);
            }

            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['purchase_returns_id'] = $purchase_returns_id;
           ...
           $this->data['tax_rates'] = $this->siteprocurment->getAllTaxRates();
			...
			$this->data['ref_requestnumber'] = $_GET['ref'];
			 $this->data['requestnumber'] = $this->siteprocurment->getAllPRETURN_NUMBER();
           ...
           $this->data['ponumber'] = ''; //$this->siteprocurment->getReference('po');
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('procurment/purchase_returns'), 'page' => lang('purchase_returns')), array('link' => '#', 'page' => lang('add_purchase_returns')));
            $meta = array('page_title' => lang('add_purchase_returns'), 'bc' => $bc);
    ...
    public function edit($id = null)
    {
        ...
        $inv = $this->purchase_returns_model->getPurchase_returnsByID($id);
       ...
       $this->form_validation->set_rules('warehouse', $this->lang->line("warehouse"), 'required|is_natural_no_zero');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $total_tax = $this->sma->formatDecimal(($product_tax + $order_tax), 4);
            $grand_total = $this->sma->formatDecimal(($total + $total_tax + $this->sma->formatDecimal($shipping) - $order_discount), 4);
            /*update common_purchase_returns*/
            $data = array(
        ...
        if ($this->form_validation->run() == true && $this->purchase_returns_model->updatePurchase_returns($id, $data, $items, $status)) {             
            $this->session->set_userdata('remove_pols', 1);
            $this->session->set_flashdata('message', $this->lang->line("purchase_returns_added"));
            admin_redirect('procurment/purchase_returns');
        } else {
            ...
            if ($this->Settings->disable_editing) {
                if ($this->data['inv']->date <= date('Y-m-d', strtotime('-'.$this->Settings->disable_editing.' days'))) {
                    $this->session->set_flashdata('error', sprintf(lang("purchase_returns_x_edited_older_than_x_days"), $this->Settings->disable_editing));
                    redirect($_SERVER["HTTP_REFERER"]);
                }
            }
            $inv_items = $this->purchase_returns_model->getAllPurchase_returnsItems($id);   
            ...
            $this->data['id'] = $id;
            $this->data['suppliers'] = $this->siteprocurment->getAllCompanies('supplier');
            $this->data['purchase_returns'] = $this->purchase_returns_model->getPurchase_returnsByID($id);
            $this->data['categories'] = $this->siteprocurment->getAllCategories();
            $this->data['tax_rates'] = $this->siteprocurment->getAllTaxRates();
            $this->data['warehouses'] = $this->siteprocurment->getAllWarehouses();
			$this->data['ref_requestnumber'] = $_GET['ref'];
            ...
            $this->session->set_userdata('remove_pols', 1);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('procurment/purchase_returns'), 'page' => lang('purchase_returns')), array('link' => '#', 'page' => lang('edit_purchase_returns')));
            $meta = array('page_title' => lang('edit_purchase_returns'), 'bc' => $bc);
          /*  echo "<pre>";
    ...
    public function purchase_returns_by_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                if (!$this->upload->do_upload()) {
                    $error = $this->upload->display_errors();
                    $this->session->set_flashdata('error', $error);
                    admin_redirect("procurment/purchase_returns/purchase_returns_by_csv");
                }
        ...
        if ($this->form_validation->run() == true && $this->purchase_returns_model->addPurchase($data, $products)) {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('procurment/purchase_returns'), 'page' => lang('purchase_returns')), array('link' => '#', 'page' => lang('add_purchase_returns_by_csv')));
            $meta = array('page_title' => lang('add_purchase_returns_by_csv'), 'bc' => $bc);
            $this->page_construct('procurment/purchase_returns/purchase_returns__orderby_csv', $meta, $this->data);
   ...
   public function delete($id = null)
    {
        ...
        if ($this->purchase_returns_model->deletePurchase_returns($id)) {

            if ($this->input->is_ajax_request()) {
                $this->sma->send_json(array('error' => 0, 'msg' => lang("purchase_returns_deleted")));
            }
            $this->session->set_flashdata('message', lang('purchase_returns_deleted'));
    ...
    public function suggestions()
    {
        ...
        $rows = $this->purchase_returns_model->getProductNames($sr);
    ...
    public function purchase_returns_actions()
    {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        $this->purchase_returns_model->deletePurchase_returns($id);
                    }
                    $this->session->set_flashdata('message', $this->lang->line("purchase_returns_deleted"));
                    redirect($_SERVER["HTTP_REFERER"]);
                    ...
                    $this->excel->getActiveSheet()->setTitle(lang('purchase_returns'));
                    ...
                    $row = 2;
                    foreach ($_POST['val'] as $id) {
                        $purchase_returns = $this->purchase_returns_model->getPurchase_returnsByID($id);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($purchase_returns->date));
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $purchase_returns->reference_no);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $purchase_returns->supplier);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $purchase_returns->status);
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $this->sma->formatMoney($purchase_returns->grand_total));
                        $row++;
                    }
                    ...
                    $filename = 'purchase_returns_' . date('Y_m_d_H_i_s');
                    $this->load->helper('excel');
                    create_excel($this->excel, $filename);
                }
            } else {
                $this->session->set_flashdata('error', $this->lang->line("no_purchase_returns_selected"));
                redirect($_SERVER["HTTP_REFERER"]);
            }
        } else {
    ...
    public function payments($id = null)
    {
        ...
        $this->data['payments'] = $this->purchase_returns_model->getPurchasePayments($id);
        $this->data['inv'] = $this->purchase_returns_model->getPurchase_returnsByID($id);
        $this->load->view($this->theme . 'purchase_returns/payments', $this->data);
    }

    public function payment_note($id = null)
    {
        //$this->sma->checkPermissions('payments', true);
        $payment = $this->purchase_returns_model->getPaymentByID($id);
        $inv = $this->purchase_returns_model->getPurchase_returnsByID($payment->purchase_id);
        $this->data['supplier'] = $this->siteprocurment->getCompanyByID($inv->supplier_id);
        $this->data['warehouse'] = $this->siteprocurment->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
        ...
        $this->load->view($this->theme . 'purchase_returns/payment_note', $this->data);
    }
    ...
    public function email_payment($id = null)
    {
        ...
        $html = $this->load->view($this->theme . 'purchase_returns/payment_note', $this->data, TRUE);
    ...
    public function add_payment($id = null)
    {
        ...
        $purchase = $this->purchase_returns_model->getPurchase_returnsByID($id);
        ...
        if ($this->form_validation->run() == true && $this->purchase_returns_model->addPayment($payment)) {
            ...
            $this->load->view($this->theme . 'purchase_returns/add_payment', $this->data);
        }
    }
    ...
    public function edit_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchase_returns_model->updatePayment($id, $payment)) {
            ...
            $this->data['payment'] = $this->purchase_returns_model->getPaymentByID($id);
            $this->data['modal_js'] = $this->siteprocurment->modal_js();

            $this->load->view($this->theme . 'purchase_returns/edit_payment', $this->data);
        }
    }
    ...
    public function delete_payment($id = null)
    {
        ...
        if ($this->purchase_returns_model->deletePayment($id)) {
    ...
    public function expenses($id = null)
    {
        ...
        $this->page_construct('procurment/purchase_returns/expenses', $meta, $this->data);
    }
    ...
    public function getExpenses()
    {
        ...
        $detail_link = anchor('admin/procurment/purchase_returns/expense_note/$1', '<i class="fa fa-file-text-o"></i> ' . lang('expense_note'), 'data-toggle="modal" data-target="#myModal2"');
        $edit_link = anchor('admin/procurment/purchase_returns/edit_expense/$1', '<i class="fa fa-edit"></i> ' . lang('edit_expense'), 'data-toggle="modal" data-target="#myModal"');
        ...
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('procurment/purchase_returns/delete_expense/$1') . "'>"
        . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
    ...
    {
        $expense = $this->purchase_returns_model->getExpenseByID($id);
        $this->data['user'] = $this->siteprocurment->getUser($expense->created_by);
        $this->data['category'] = $expense->category_id ? $this->purchase_returns_model->getExpenseCategoryByID($expense->category_id) : NULL;
        ...
        $this->load->view($this->theme . 'purchase_returns/expense_note', $this->data);
    }
    ...
    public function add_expense()
    {
        ...
        if ($this->form_validation->run() == true && $this->purchase_returns_model->addExpense($data)) {
            ...
            $this->data['warehouses'] = $this->siteprocurment->getAllWarehouses();
            $this->data['categories'] = $this->purchase_returns_model->getExpenseCategories();
            $this->data['modal_js'] = $this->siteprocurment->modal_js();
            $this->load->view($this->theme . 'purchase_returns/add_expense', $this->data);
        }
    }
    ...
    public function edit_expense($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchase_returns_model->updateExpense($id, $data)) {
            $this->session->set_flashdata('message', lang("expense_updated"));
            admin_redirect("procurment/purchase_returns/expenses");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['expense'] = $this->purchase_returns_model->getExpenseByID($id);
            $this->data['warehouses'] = $this->siteprocurment->getAllWarehouses();
            $this->data['modal_js'] = $this->siteprocurment->modal_js();
            $this->data['categories'] = $this->purchase_returns_model->getExpenseCategories();
            $this->load->view($this->theme . 'purchase_returns/edit_expense', $this->data);
        }
    }
    ...
    public function delete_expense($id = null)
    {
        ...
        $expense = $this->purchase_returns_model->getExpenseByID($id);
        if ($this->purchase_returns_model->deleteExpense($id)) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    //$this->sma->checkPermissions('delete');
                    foreach ($_POST['val'] as $id) {
                        $this->purchase_returns_model->deleteExpense($id);
                    }
    ...
    {
        //$this->sma->checkPermissions('return_purchase_returns');

        if ($this->input->get('id')) {
            $id = $this->input->get('id');
        }
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $inv = $this->purchase_returns_model->getReturnByID($id);
        if (!$this->session->userdata('view_right')) {
            $this->sma->view_rights($inv->created_by);
        }
        ...
        $this->data['supplier'] = $this->siteprocurment->getCompanyByID($inv->supplier_id);
        $this->data['payments'] = $this->purchase_returns_model->getPaymentsForPurchase($id);
        $this->data['user'] = $this->siteprocurment->getUser($inv->created_by);
        $this->data['warehouse'] = $this->siteprocurment->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
        $this->data['rows'] = $this->purchase_returns_model->getAllReturnItems($id);
        $this->data['purchase'] = $this->purchase_returns_model->getPurchase_returnsByID($inv->purchase_id);
        $this->load->view($this->theme.'purchase_returns/view_return', $this->data);
    }
    ...
    public function return_purchase($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchase_returns_model->addPurchase($data, $products)) {
            ...
            $inv_items = $this->purchase_returns_model->getAllPurchaseItems($id);
             krsort($inv_items);
            $c = rand(100000, 9999999);
            foreach ($inv_items as $item) {
                $row = $this->siteprocurment->getProductByID($item->product_id);
            ...
            $this->data['reference'] = '';
            $this->data['tax_rates'] = $this->siteprocurment->getAllTaxRates();
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('procurment/purchase_returns'), 'page' => lang('purchase_returns')), array('link' => '#', 'page' => lang('return_purchase')));
            $meta = array('page_title' => lang('return_purchase'), 'bc' => $bc);
            $this->page_construct('procurment/purchase_returns/return_purchase', $meta, $this->data);
        }
    }
    ...
    public function update_status($id)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchase_returns_model->updateStatus($id, $status, $note)) {
            ...
            $this->data['modal_js'] = $this->siteprocurment->modal_js();
            $this->load->view($this->theme.'purchase_returns/update_status', $this->data);
