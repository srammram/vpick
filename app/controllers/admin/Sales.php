...
class Sales extends MY_Controller
{
    ...
    /*  <li>' . $detail_link . '</li>
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("'sno',{$this->db->dbprefix('sales')}.id as id,{$this->db->dbprefix('warehouses')}.name as branch,{$this->db->dbprefix('sales_type')}.name as sale_type, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, biller, {$this->db->dbprefix('sales')}.customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.attachment, return_id")
                ->from('sales')
    ...
    public function modal_view($id = null)
    {
        ...
        $this->data['default_currency'] = $this->site->getCurrencyByID($this->Settings->default_currency);
    ...
    <ul class="dropdown-menu pull-right" role="menu">
        ...
        $this->datatables
            ->select("'sno',deliveries.id as id,warehouses.name as branch, date, do_reference_no, sale_reference_no, customer, deliveries.address as address, status, attachment")
            ->from('deliveries')
    ...
    public function payment_note($id = null)
    {
        ...
        $payment = $this->sales_model->getPaymentDetailsByBillID($id);//getPaymentByID($id);
    ...
    public function suggestions()
    {
        ...
        $recipe_standard = $this->input->get('recipe_standard', true);
        ...
        $rows = $this->sales_model->getProductNames($sr, $warehouse_id, $recipe_standard);
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
				...
				$addons = $this->sales_model->getrecipeAddons($row->id);
                if ($options) {
               ...
               $row->price =$row->price + $row->tax_amount;
