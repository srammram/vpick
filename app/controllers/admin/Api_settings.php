...
class Api_settings extends MY_Controller
{
    ...
    function index() {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('api_keys')));
        $meta = array('page_title' => lang('api_keys'), 'bc' => $bc);
    ...
    public function delete($id) {
        ...
        if (DEMO) {
            $this->sma->send_json(array('error' => 1, 'msg' => lang("disabled_in_demo")));
        }

        if ($this->api_model->deleteApiKey($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("api_key_deleted")));
        } else {
            $this->sma->send_json(array('error' => 1, 'msg' => lang("delete_failed")));
        }
    }
