...
class Rides extends MY_Controller
{
    ...
    {
		$this->sma->checkPermissions('rides-index');
	...
	function pdf($id = NULL, $view = NULL)
    {
		...
		$this->data['is_country'] = $pr_details->is_country;
        if (!$id || !$pr_details) {
            $this->session->set_flashdata('error', lang('not_found'));
            redirect($_SERVER["HTTP_REFERER"]);
        }
       
        $name = $pr_details->booking_no.".pdf";
        if ($view) {
			$this->data['rides'] = $pr_details;
			$this->data['commision'] = $this->site->getRideCommision($pr_details->id);
    ...
    function getOnRides(){
        ...
        $this->datatables
			...
			$edit = "<a href='" . admin_url('rides/track/$1') . "' data-toggle='tooltip'  data-original-title='' aria-describedby='tooltip' title='".lang('click_here_to_full_details')."'  ><i class='fa fa-eye' aria-hidden='true'  style='color:#656464; font-size:18px'></i></a>";
    ...
    {
		$this->sma->checkPermissions('rides-view');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
		...
		$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
		$this->site->users_logs($countryCode,$this->session->userdata('user_id'), $this->getUserIpAddr, json_encode($_POST), $_SERVER['REQUEST_URI']);
		$rides_result = $this->rides_model->getRides($id, $countryCode);
       $booked_status = $rides_result->status; 
		...
		$this->data['rides'] = $rides_result;
		$this->data['waypoints'] = $this->site->getWaypoint($id);
		$this->data['commision'] = $this->site->getRideCommision($id);
