...
class Heatmap extends MY_Controller
{
		...
		if(!empty($locations) && $locations != NULL){
			foreach($locations as $loc){
				...
				$val[] = array(
					...
					'icon' => 'https://35.154.46.42/themes/default/admin/assets/images/track.png'
				);
    ...
    {
		$this->sma->checkPermissions('live_tracking-index');
    ...
    {
		$this->sma->checkPermissions('search_heat_map-index');
		$markers = [];
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
