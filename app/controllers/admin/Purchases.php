...
class Purchases extends MY_Controller
{
	...
	public function supplier(){
		...
		$data = $this->purchases_model->getSupplierdetails($supplier_id);
	...
	public function purchase_list(){
		...
		$data['purchases'] = $this->purchases_model->getPurchaseOrderByID($poref);
		
		$inv_items = $this->purchases_model->getAllPurchaseOrderItems($poref);
		// krsort($inv_items);
    ...
    public function getPurchases($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("'sno',{$this->db->dbprefix('purchases')}.id, {$this->db->dbprefix('warehouses')}.name as branch,DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, supplier, status, grand_total, paid, (grand_total-paid) as balance, payment_status, attachment")
                ->from('purchases')
    ...
    public function view($purchase_id = null)
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('purchases'), 'page' => lang('Inventory')), array('link' => '#', 'page' => lang('view')));
        $meta = array('page_title' => lang('view_Inventory_details'), 'bc' => $bc);
...
//generate pdf and force to download
    ...
    public function add($quote_id = null)
    {
       ...
       // $this->form_validation->set_message('is_natural_no_zero', $this->lang->line("no_zero_required"));
           ...
           // $reference = $this->input->post('reference_no') ? $this->input->post('reference_no') : $this->site->getReference('po');
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                   ...
                   /* $avail_qty = $this->site->getProductQuantity($product_details->id);*/
                    ...
                    $product = array(
						...
						'mfg' => date('Y-m-d', strtotime($item_mfg)),
                        'expiry' => $item_expiry,
            ...
            $data = array('reference_no' => 'GRN-'.date('YmdHis'),
                'date' => $date,
                'supplier_id' => $supplier_id,
                'supplier' => $supplier,
                'warehouse_id' => $warehouse_id,
				'grn_details' => $this->input->post('grn_details') ? $this->input->post('grn_details') : '',  
                ...
                'due_date' => $due_date,
				'purchasesorders' => $this->input->post('poref')
            );
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products, $purchases_order_array, $this->input->post('poref'))) {
            ...
            if ($quote_id) {
                $this->data['quote'] = $this->purchases_model->getPurchaseOrderByID($quote_id);
                ...
                foreach ($items as $item) {
                    ...
                    if ($row->type == 'combo') {
                        ...
                        foreach ($combo_items as $citem) {
							...
							$crow->value_expiry = $item->value_expiry;
							$crow->type_expiry = $item->type_expiry;
						...
						$row->value_expiry = $item->value_expiry;
						$row->type_expiry = $item->type_expiry;
            ...
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->data['ponumber'] = $this->site->getAllPONUMBER();
			
            $this->load->helper('string');
            $value = random_string('alnum', 20);
            $this->session->set_userdata('user_csrf', $value);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('purchases'), 'page' => lang('Inventory')), array('link' => '#', 'page' => lang('add_purchase')));
    ...
    public function edit($id = null)
    {
		...
		$c = $this->purchases_model->getPurchaseByID($id);
		
        	if ($c->payment_status == 'paid' || $c->payment_status == 'partial') {
            	$this->session->set_flashdata('error', lang("Payment Status Is Already Paid Or Partial For The Purchase, Do not allowed edit option"));
			 	admin_redirect("purchases");
        	}	
       ...
       // $this->form_validation->set_rules('reference_no', $this->lang->line("ref_no"), 'required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'invoice_no' => $this->input->post('invoice_no'),
				'invoice_date' => date('Y-m-d', strtotime($this->input->post('invoice_date'))),
				'purchases_order_no' => $this->input->post('poref'),
				'purchases_order_date' => $this->input->post('purchases_order_date'),
                'note' => $note,
                'total' => $total,
                'product_discount' => $product_discount,
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePurchase($id, $data, $products)) {
            ...
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
            $this->data['warehouses'] = $this->site->getAllWarehouses();
			$this->data['quote_id'] = $id;
			
            $this->data['ponumber'] = $this->site->getAllPONUMBERedit();
    ...
    function bill_of_material($warehouse_id = NULL)
    {
        $this->sma->checkPermissions();
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('bill_of_material')));
        $meta = array('page_title' => lang('bill_of_material'), 'bc' => $bc);
        $this->page_construct('purchases/bill_of_material', $meta, $this->data);
    }
    function getBillOfMaterial($warehouse_id = NULL)
    {
        $this->sma->checkPermissions('index', TRUE);
        ...
        if ($warehouse_id) {
            $this->datatables
            ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, wp.rack as rack", FALSE)
            ->from('products');
            ...
            $this->datatables
                ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code,{$this->db->dbprefix('products')}.name as name, '' as rack", FALSE)
                ->from('products')
