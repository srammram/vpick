			...
			if(!empty($_POST['area_id'])){
				for($i=0; $i<count($_POST['area_id']); $i++){
					$data[] = array(
						...
						'work_per_load' => $this->input->post('base_work_per_load'),
						'labour_charge' => $this->input->post('base_labour_charge'),
			...
			if($this->input->post('is_peak') == 1){
				for($i=0; $i<count($_POST['peak_price_type']); $i++){
					...
					$slot_array[] = array(
						...
						'waiting_price'  => $_POST['peak_waiting_price'][$i],
						'work_per_load' => $_POST['peak_work_per_load'][$i],
						'labour_charge' => $_POST['peak_labour_charge'][$i],
						'type' => 1,
						'is_country' => $countryCode,
					);
				}
			...
			if($this->input->post('is_night') == 1){
				for($i=0; $i<count($_POST['night_price_type']); $i++){
					...
					$slot_array[] = array(
						...
						'waiting_price'  => $_POST['night_waiting_price'][$i],
						'work_per_load' => $_POST['night_work_per_load'][$i],
						'labour_charge' => $_POST['night_labour_charge'][$i],
						'type' => 2,
						'is_country' => $countryCode,
					);
				}
    ...
    function edit_daily($id){
        ...
        if ($this->form_validation->run() == true) {
           ...
           if(!empty($_POST['area_id'])){
			...
			//$this->locations_model->update_daily($id, $daily, $slot_array, $countryCode);
			//echo '@@@';
			//die;
        ...
        if ($this->form_validation->run() == true && $this->locations_model->update_daily($id, $daily, $slot_array, $countryCode)) { //check to see if we are updateing the customer
    ...
    function add_outstation(){
        ...
        if ($this->form_validation->run() == true) {
			...
			if($is_way == 1){
				...
				$work_per_load = $this->input->post('oneway_work_per_load');
				$labour_charge = $this->input->post('oneway_labour_charge');
			}elseif($is_way == 2){
				...
				$work_per_load = $this->input->post('twoway_work_per_load');
				$labour_charge = $this->input->post('twoway_labour_charge');
			}
