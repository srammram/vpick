...
class Enquiry extends MY_Controller
{
    ...
    {
		$this->sma->checkPermissions('crm_dashboard-index');
    ...
    {
		$this->sma->checkPermissions('crm_enquiry-index');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
	...
	function getEnquiry(){
        ...
        $this->datatables
			...
			$edit = "<a href='" . admin_url('enquiry/enquiry_view/$1') . "' data-toggle='tooltip'  data-original-title='' aria-describedby='tooltip' title='".lang('click_here_to_full_details')."'  ><i class='fa fa-eye' aria-hidden='true'  style='color:#656464; font-size:18px'></i></a>";
	...
	function create_customer(){
		$this->sma->checkPermissions('crm_dashboard-add');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
	...
	function existing_ticket(){
		$this->sma->checkPermissions('crm_dashboard-edit');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
	...
	function enquiry_view($enquiry_id){
		$this->sma->checkPermissions('crm_enquiry-view');
		$enquiry_details = $this->enquiry_model->getEnquiryID($enquiry_id);
	...
	function open($enquiry_id){
		$this->sma->checkPermissions('crm_enquiry-status');
		$enquiry_details = $this->enquiry_model->getEnquiryID($enquiry_id);
		if($this->session->userdata('group_id') == 1){
			if($enquiry_details->is_country != ''){
				$countryCode = $enquiry_details->is_country;	
	...
	function reopen($enquiry_id){
        ...
        if ($this->form_validation->run() == true && $this->enquiry_model->reopenenquiry($enquiry, $enquiry_support, $enquiry_follow, $this->input->post('enquiry_status'), $enquiry_id, $countryCode)){
            ...
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('enquiry/open'), 'page' => lang('reopen')), array('link' => '#', 'page' => lang('reopen')));
	...
	function close_transfer($enquiry_id){
        ...
        if ($this->form_validation->run() == true && $this->enquiry_model->closeenquiry($enquiry, $enquiry_support, $enquiry_follow, $enquiry_status, $enquiry_id, $countryCode)){
            ...
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('enquiry/close'), 'page' => lang('close')), array('link' => '#', 'page' => lang('close')));
