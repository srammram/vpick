...
class People extends MY_Controller
{
    ...
    function customer($action=false){
		$this->sma->checkPermissions('customer-index');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
	...
	function add_customer(){
		$this->sma->checkPermissions('customer-add');
	...
	function customer_view($id){
		$this->sma->checkPermissions('customer-view');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
    ...
    function employee($action=false){
		$this->sma->checkPermissions('staff-index');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
	...
	function add_employee(){
		$this->sma->checkPermissions('staff-add');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
	...
	function employee_status($status, $id){
		$this->sma->checkPermissions('staff-approved');
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
	...
	function employee_edit($user_id, $view){
		$this->sma->checkPermissions('staff-edit');
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
    ...
    function driver($action=false){
		$this->sma->checkPermissions('driver-index');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
    ...
    function getDriver(){
        ...
        $this->datatables
            ->select("{$this->db->dbprefix('users')}.id as id,{$this->db->dbprefix('users')}.created_on, {$this->db->dbprefix('users')}.refer_code, {$this->db->dbprefix('users')}.first_name, {$this->db->dbprefix('users')}.last_name, {$this->db->dbprefix('users')}.email, {$this->db->dbprefix('users')}.mobile,  up.gender, ROUND(ROUND(SUM(m.overall)) / (COUNT(m.id) * 5) * 5 , 2) as mmm,
			t.name as taxi_name, t.number, t.model,  tt.name as type_name, tm.name as make_name, tf.name as fuel_type, 
			
			If(up.is_approved = 1 && t.is_verify = 1 && t.complete_taxi = 1 && ud.pancard_verify = 1 && uadd.local_verify = 1 && uadd.permanent_verify = 1 && ub.is_verify = 1 && ud.aadhar_verify = 1 && ud.license_verify = 1  && ud.badge_verify = 1  && ud.training_certificate_verify = 1  && ud.experience_certificate_verify = 1  && ud.medical_certificate_verify = 1  && ud.health_insurance_verify = 1  && ud.term_insurance_verify = 1  && ud.additional_contact_verify = 1 && ud.police_verify = 1, '1', '0') as status, 
			
			If({$this->db->dbprefix('users')}.complete_user = 1 && t.complete_taxi = 1 , '1', '0') as join_type, country.name as instance_country, {$this->db->dbprefix('users')}.is_block as is_block
			
			")
            ->from("users")
			->join("countries country", " country.iso = users.is_country", "left")
			->join("multiple_rating m", " m.driver_id = users.id", "left")
			->join("user_profile up", "up.user_id = users.id AND up.is_edit = 1 ", 'left')
			->join("user_bank ub", "ub.user_id = users.id AND ub.is_edit = 1 ", 'left')
			->join("user_document ud", "ud.user_id = users.id AND ud.is_edit = 1 ", 'left')
			->join("user_address uadd", "uadd.user_id = users.id AND uadd.is_edit = 1 ", 'left')
			...
			->join("taxi_model tm", 'tm.id = t.make_id ', 'left')
			
			
			->join("taxi_fuel tf", 'tf.id = t.fuel_type ', 'left')
			...
			if($approved == 1){
				...
				$this->datatables->where('ud.badge_verify', 1);
				$this->datatables->where('ud.training_certificate_verify', 1);
				$this->datatables->where('ud.experience_certificate_verify', 1);
				$this->datatables->where('ud.medical_certificate_verify', 1);
				$this->datatables->where('ud.health_insurance_verify', 1);
				$this->datatables->where('ud.term_insurance_verify', 1);
				$this->datatables->where('ud.additional_contact_verify', 1);
				...
				WHEN ud.badge_verify = 0 AND {$this->db->dbprefix('users')}.group_id =  ".$group_id." THEN {$this->db->dbprefix('users')}.group_id =  ".$group_id." 
				WHEN ud.training_certificate_verify = 0 AND {$this->db->dbprefix('users')}.group_id =  ".$group_id." THEN {$this->db->dbprefix('users')}.group_id =  ".$group_id." 
				WHEN ud.experience_certificate_verify = 0 AND {$this->db->dbprefix('users')}.group_id =  ".$group_id." THEN {$this->db->dbprefix('users')}.group_id =  ".$group_id." 
				WHEN ud.medical_certificate_verify = 0 AND {$this->db->dbprefix('users')}.group_id =  ".$group_id." THEN {$this->db->dbprefix('users')}.group_id =  ".$group_id." 
				WHEN ud.health_insurance_verify = 0 AND {$this->db->dbprefix('users')}.group_id =  ".$group_id." THEN {$this->db->dbprefix('users')}.group_id =  ".$group_id." 
				WHEN ud.term_insurance_verify = 0 AND {$this->db->dbprefix('users')}.group_id =  ".$group_id." THEN {$this->db->dbprefix('users')}.group_id =  ".$group_id." 
				WHEN ud.additional_contact_verify = 0 AND {$this->db->dbprefix('users')}.group_id =  ".$group_id." THEN {$this->db->dbprefix('users')}.group_id =  ".$group_id." 
			...
			->edit_column('join_type', '$1__$2', 'id, join_type')->edit_column('is_block', '$1__$2', 'id, is_block');
	...
	function add_driver(){
		$this->sma->checkPermissions('driver-add');
		$current_date = date('Y-m-d');
		$checkcountry = '';
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
        ...
        if ($this->form_validation->run() == true) {
		   ...
		   if ($_FILES['photo']['size'] > 0) {
			...
			$user_document = array(
				...
				'badge_on' => $this->input->post('badge_on'),
				'badge_til' => $this->input->post('badge_til'),
				
				'training_certificate_no' => $this->input->post('training_certificate_no'),
				'training_certificate_on' => $this->input->post('training_certificate_on'),
				'training_certificate_til' => $this->input->post('training_certificate_til'),
				
				'experience_certificate_no' => $this->input->post('experience_certificate_no'),
				'experience_certificate_on' => $this->input->post('experience_certificate_on'),
				'experience_certificate_til' => $this->input->post('experience_certificate_til'),
				
				'medical_certificate_no' => $this->input->post('medical_certificate_no'),
				'medical_certificate_on' => $this->input->post('medical_certificate_on'),
				'medical_certificate_til' => $this->input->post('medical_certificate_til'),
				
				'health_insurance_no' => $this->input->post('health_insurance_no'),
				'health_insurance_on' => $this->input->post('health_insurance_on'),
				'health_insurance_til' => $this->input->post('health_insurance_til'),
				
				'term_insurance_no' => $this->input->post('term_insurance_no'),
				'term_insurance_on' => $this->input->post('term_insurance_on'),
				'term_insurance_til' => $this->input->post('term_insurance_til'),
		   ...
		   if ($_FILES['taxi_photo']['size'] > 0) {
			...
			$taxi_document = array(
				...
				'emission_norms_on' => $this->input->post('emission_norms_on'),
				'emission_norms_til' => $this->input->post('emission_norms_til'),
				
				'vehicle_tracking_on' => $this->input->post('vehicle_tracking_on'),
				'vehicle_tracking_til' => $this->input->post('vehicle_tracking_til'),
				'fire_extinguisher_on' => $this->input->post('fire_extinguisher_on'),
				
				'fire_extinguisher_til' => $this->input->post('fire_extinguisher_til'),
				'child_lock_mechanism_on' => $this->input->post('child_lock_mechanism_on'),
				'child_lock_mechanism_til' => $this->input->post('child_lock_mechanism_til'),
				
				'interior_vehicle_on' => $this->input->post('interior_vehicle_on'),
				'interior_vehicle_til' => $this->input->post('interior_vehicle_til'),
				'taxi_roof_sign_on' => $this->input->post('taxi_roof_sign_on'),
				'taxi_roof_sign_til' => $this->input->post('taxi_roof_sign_til'),
				'e_challans_clearance_on' => $this->input->post('e_challans_clearance_on'),
				'e_challans_clearance_til' => $this->input->post('e_challans_clearance_til'),
	...
	function driver_status($status, $id){
		$this->sma->checkPermissions('driver-approved');
		...
		$b = $this->db->select("u.id as id, t.is_verify as taxi_verify, If(up.is_approved = 1 && ud.aadhar_verify = 1  && ud.pancard_verify = 1 && ud.badge_verify = 1 && ud.training_certificate_verify = 1 && ud.experience_certificate_verify = 1 && ud.medical_certificate_verify = 1 && ud.health_insurance_verify = 1 && ud.term_insurance_verify = 1 && ud.additional_contact_verify = 1 && ub.is_verify = 1 && ud.aadhar_verify = 1 && ud.license_verify = 1 && ud.police_verify = 1 && u.is_approved = 1 && uadd.local_verify = 1 && uadd.permanent_verify = 1, '1', '0') as status")
            ->from("users u")
			->join("user_profile up", "up.user_id = u.id AND up.is_edit = 1", 'left')
			->join("user_bank ub", "ub.user_id = u.id AND ub.is_edit = 1", 'left')
			->join("user_document ud", "ud.user_id = u.id AND ud.is_edit = 1", 'left')
        ...
        if ($this->form_validation->run() == true) {
			...
			$data_badge = array(
				...
				'badge_on' => $this->input->post('badge_on'),
				'badge_til' => $this->input->post('badge_til'),
				'badge_verify' => $this->input->post('is_approved'),
				'badge_approved_by' => $this->session->userdata('user_id'),
				'is_country' => $countryCode,
				'badge_approved_on' => date('Y-m-d H:i:s'),
			);
			$data_training_certificate = array(
				'training_certificate_no' => $this->input->post('training_certificate_no'),
				'training_certificate_on' => $this->input->post('training_certificate_on'),
				'training_certificate_til' => $this->input->post('training_certificate_til'),
				'training_certificate_verify' => $this->input->post('is_approved'),
				'training_certificate_approved_by' => $this->session->userdata('user_id'),
				'is_country' => $countryCode,
				'training_certificate_approved_on' => date('Y-m-d H:i:s'),
			);
			$data_experience_certificate = array(
				'experience_certificate_no' => $this->input->post('experience_certificate_no'),
				'experience_certificate_on' => $this->input->post('experience_certificate_on'),
				'experience_certificate_til' => $this->input->post('experience_certificate_til'),
				'experience_certificate_verify' => $this->input->post('is_approved'),
				'experience_certificate_approved_by' => $this->session->userdata('user_id'),
				'is_country' => $countryCode,
				'experience_certificate_approved_on' => date('Y-m-d H:i:s'),
			);
			$data_medical_certificate = array(
				'medical_certificate_no' => $this->input->post('medical_certificate_no'),
				'medical_certificate_on' => $this->input->post('medical_certificate_on'),
				'medical_certificate_til' => $this->input->post('medical_certificate_til'),
				'medical_certificate_verify' => $this->input->post('is_approved'),
				'medical_certificate_approved_by' => $this->session->userdata('user_id'),
				'is_country' => $countryCode,
				'medical_certificate_approved_on' => date('Y-m-d H:i:s'),
			);
			$data_health_insurance = array(
				'health_insurance_no' => $this->input->post('health_insurance_no'),
				'health_insurance_on' => $this->input->post('health_insurance_on'),
				'health_insurance_til' => $this->input->post('health_insurance_til'),
				'health_insurance_verify' => $this->input->post('is_approved'),
				'health_insurance_approved_by' => $this->session->userdata('user_id'),
				'is_country' => $countryCode,
				'health_insurance_approved_on' => date('Y-m-d H:i:s'),
			);
			
			$data_term_insurance = array(
				'term_insurance_no' => $this->input->post('term_insurance_no'),
				'term_insurance_on' => $this->input->post('term_insurance_on'),
				'term_insurance_til' => $this->input->post('term_insurance_til'),
				'term_insurance_verify' => $this->input->post('is_approved'),
				'term_insurance_approved_by' => $this->session->userdata('user_id'),
				'is_country' => $countryCode,
				'term_insurance_approved_on' => date('Y-m-d H:i:s'),
			);
			
			$data_additional_contact = array(
				'additional_contact_one_type' => $this->input->post('additional_contact_one_type'),
				'additional_contact_one_name' => $this->input->post('additional_contact_one_name'),
				'additional_contact_one_phone' => $this->input->post('additional_contact_one_phone'),
				'additional_contact_one_phonecode' => $this->input->post('additional_contact_one_phonecode'),
				...
				'additional_contact_two_phone' => $this->input->post('additional_contact_two_phone'),
				'additional_contact_two_phonecode' => $this->input->post('additional_contact_two_phonecode'),
				'additional_contact_verify' => $this->input->post('is_approved'),
				'additional_contact_approved_by' => $this->session->userdata('user_id'),
				'is_country' => $countryCode,
				'additional_contact_approved_on' => date('Y-m-d H:i:s'),
			);
			...
			if ($_FILES['badge_image']['size'] > 0) {
				...
				$badge_image = $this->upload->file_name;
				$data_badge['badge_image'] = 'document/badge/'.$badge_image;
				$config = NULL;
			}
			...
			if ($_FILES['training_certificate_image']['size'] > 0) {
				...
				$training_certificate_image = $this->upload->file_name;
				$data_training_certificate['training_certificate_image'] = 'document/training_certificate/'.$training_certificate_image;
				$config = NULL;
			}
			...
			if ($_FILES['experience_certificate_image']['size'] > 0) {
				...
				$experience_certificate_image = $this->upload->file_name;
				$data_experience_certificate['experience_certificate_image'] = 'document/experience_certificate/'.$experience_certificate_image;
				$config = NULL;
			}
			if ($_FILES['medical_certificate_image']['size'] > 0) {
				$config['upload_path'] = $this->upload_path.'document/medical_certificate/';
				...
				$medical_certificate_image = $this->upload->file_name;
				$data_medical_certificate['medical_certificate_image'] = 'document/medical_certificate/'.$medical_certificate_image;
				$config = NULL;
			}
			if ($_FILES['health_insurance_image']['size'] > 0) {
				$config['upload_path'] = $this->upload_path.'document/health_insurance/';
				...
				$health_insurance_image = $this->upload->file_name;
				$data_health_insurance['health_insurance_image'] = 'document/health_insurance/'.$health_insurance_image;
				$config = NULL;
			}
			if ($_FILES['term_insurance_image']['size'] > 0) {
				$config['upload_path'] = $this->upload_path.'document/term_insurance/';
				...
				$term_insurance_image = $this->upload->file_name;
				$data_term_insurance['term_insurance_image'] = 'document/term_insurance/'.$term_insurance_image;
				$config = NULL;
			}
			
			
			
			$check_verify = $this->verification_model->update_vendor_status($this->input->post('user_id'), $data, $udata, $countryCode) && $this->verification_model->update_address_status($this->input->post('address_id'), $data_address, $countryCode) && $this->verification_model->update_account_status($this->input->post('bank_id'), $data_bank, $countryCode) && $this->verification_model->update_document_common_status($this->input->post('document_id'), $data_aadhar, $countryCode) && $this->verification_model->update_document_common_status($this->input->post('document_id'), $data_pancard, $countryCode) && $this->verification_model->update_document_common_status($this->input->post('document_id'), $data_police, $countryCode) && $this->verification_model->update_document_common_status($this->input->post('document_id'), $data_license, $countryCode) && $this->verification_model->update_document_common_status($this->input->post('document_id'), $data_badge, $countryCode) && $this->verification_model->update_document_common_status($this->input->post('document_id'), $data_training_certificate, $countryCode) && $this->verification_model->update_document_common_status($this->input->post('document_id'), $data_experience_certificate, $countryCode) && $this->verification_model->update_document_common_status($this->input->post('document_id'), $data_medical_certificate, $countryCode) && $this->verification_model->update_document_common_status($this->input->post('document_id'), $data_health_insurance, $countryCode) && $this->verification_model->update_document_common_status($this->input->post('document_id'), $data_term_insurance, $countryCode) && $this->verification_model->update_document_common_status($this->input->post('document_id'), $data_additional_contact, $countryCode);
        ...
        if ($this->form_validation->run() == true && $check_verify){
			...
			$q = $this->db->select("u.id as id, up.first_name, up.last_name, u.email, u.mobile, t.is_verify as taxi_verify,  up.gender, If(up.is_approved = 1 && t.is_verify = 1 && t.complete_taxi = 1 && ud.pancard_verify = 1 && ub.is_verify = 1 && ud.aadhar_verify = 1 && ud.pancard_verify = 1 && ud.badge_verify = 1 && ud.training_certificate_verify = 1 && ud.experience_certificate_verify = 1 && ud.medical_certificate_verify = 1 && ud.health_insurance_verify = 1 && ud.term_insurance_verify = 1 && ud.additional_contact_verify = 1  && ud.license_verify = 1 && ud.police_verify = 1 && u.is_approved = 1 && uadd.local_verify = 1 && uadd.permanent_verify = 1, '1', '0') as status")
            ->from("users u")
			->join("user_profile up", "up.user_id = u.id AND up.is_edit = 1", 'left')
			->join("user_bank ub", "ub.user_id = u.id AND ub.is_edit = 1", 'left')
			->join("user_document ud", "ud.user_id = u.id AND ud.is_edit = 1", 'left')
	...
	function driverblock($status,$id){
		$this->sma->checkPermissions('driver-approved');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
        ...
        $this->people_model->update_user_block_status($is_block,$id, $countryCode);
		redirect($_SERVER["HTTP_REFERER"]);
    }
	...
	function driver_edit($user_id, $view){
		$this->sma->checkPermissions('driver-edit');
		$result = $this->users_model->getUserEdit($user_id);
		...
		$this->db->select("u.id as id, up.first_name, up.last_name, u.email, u.mobile,  up.gender, If(up.is_approved = 1 && t.complete_taxi = 1 &&  ud.pancard_verify = 1 && ud.badge_verify = 1 && ud.training_certificate_verify = 1 && ud.experience_certificate_verify = 1 && ud.medical_certificate_verify = 1 && ud.health_insurance_verify = 1 && ud.term_insurance_verify = 1 && ud.additional_contact_verify = 1   && ub.is_verify = 1 && ud.aadhar_verify = 1 && ud.license_verify = 1 && ud.police_verify = 1 && u.is_approved = 1 && uadd.local_verify = 1 && uadd.permanent_verify = 1, '1', '0') as status")
            ->from("users u")
			->join("user_profile up", "up.user_id = u.id AND up.is_edit = 1 ", 'left')
			->join("user_bank ub", "ub.user_id = u.id AND ub.is_edit = 1 ", 'left')
			->join("user_document ud", "ud.user_id = u.id AND ud.is_edit = 1 ", 'left')
        ...
        if ($this->form_validation->run() == true) {
		   ...
		   if ($_FILES['photo']['size'] > 0) {
			...
			if($this->input->post('badge_no') == $result->badge_no && $this->input->post('badge_on') == $result->badge_on && $this->input->post('badge_til') == $result->badge_til && $_FILES['badge_image']['size'] == 0){
				$badge_verify = $result->badge_verify;
				$badge_approved_by = $result->badge_approved_by;
				$badge_approved_on = $result->badge_approved_on;
			}else{
				$badge_verify = 0;
				$badge_approved_by = 0;
				$badge_approved_on = '0000:00:00 00:00:00';
			}
			
			
			if($this->input->post('training_certificate_no') == $result->training_certificate_no && $this->input->post('training_certificate_on') == $result->training_certificate_on && $this->input->post('training_certificate_til') == $result->training_certificate_til && $_FILES['training_certificate_image']['size'] == 0){
				$training_certificate_verify = $result->training_certificate_verify;
				$training_certificate_approved_by = $result->training_certificate_approved_by;
				$training_certificate_approved_on = $result->training_certificate_approved_on;
			}else{
				$training_certificate_verify = 0;
				$training_certificate_approved_by = 0;
				$training_certificate_approved_on = '0000:00:00 00:00:00';
			}
			
			if($this->input->post('experience_certificate_no') == $result->experience_certificate_no && $this->input->post('experience_certificate_on') == $result->experience_certificate_on && $this->input->post('experience_certificate_til') == $result->experience_certificate_til && $_FILES['experience_certificate_image']['size'] == 0){
				$experience_certificate_verify = $result->experience_certificate_verify;
				$experience_certificate_approved_by = $result->experience_certificate_approved_by;
				$experience_certificate_approved_on = $result->experience_certificate_approved_on;
			}else{
				$experience_certificate_verify = 0;
				$experience_certificate_approved_by = 0;
				$experience_certificate_approved_on = '0000:00:00 00:00:00';
			}
			if($this->input->post('medical_certificate_no') == $result->medical_certificate_no && $this->input->post('medical_certificate_on') == $result->medical_certificate_on && $this->input->post('medical_certificate_til') == $result->medical_certificate_til && $_FILES['medical_certificate_image']['size'] == 0){
				$medical_certificate_verify = $result->medical_certificate_verify;
				$medical_certificate_approved_by = $result->medical_certificate_approved_by;
				$medical_certificate_approved_on = $result->medical_certificate_approved_on;
			}else{
				$medical_certificate_verify = 0;
				$medical_certificate_approved_by = 0;
				$medical_certificate_approved_on = '0000:00:00 00:00:00';
			}
			if($this->input->post('health_insurance_no') == $result->health_insurance_no && $this->input->post('health_insurance_on') == $result->health_insurance_on && $this->input->post('health_insurance_til') == $result->health_insurance_til && $_FILES['health_insurance_image']['size'] == 0){
				$health_insurance_verify = $result->health_insurance_verify;
				$health_insurance_approved_by = $result->health_insurance_approved_by;
				$health_insurance_approved_on = $result->health_insurance_approved_on;
			}else{
				$health_insurance_verify = 0;
				$health_insurance_approved_by = 0;
				$health_insurance_approved_on = '0000:00:00 00:00:00';
			}
			
			if($this->input->post('term_insurance_no') == $result->term_insurance_no && $this->input->post('term_insurance_on') == $result->term_insurance_on && $this->input->post('term_insurance_til') == $result->term_insurance_til && $_FILES['term_insurance_image']['size'] == 0){
				$term_insurance_verify = $result->term_insurance_verify;
				$term_insurance_approved_by = $result->term_insurance_approved_by;
				$term_insurance_approved_on = $result->term_insurance_approved_on;
			}else{
				$term_insurance_verify = 0;
				$term_insurance_approved_by = 0;
				$term_insurance_approved_on = '0000:00:00 00:00:00';
			}
			...
			if($this->input->post('additional_contact_one_type') == $result->additional_contact_one_type && $this->input->post('additional_contact_one_name') == $result->additional_contact_one_name && $this->input->post('additional_contact_one_phone') == $result->additional_contact_one_phone && $this->input->post('additional_contact_two_type') == $result->additional_contact_two_type && $this->input->post('additional_contact_two_name') == $result->additional_contact_two_name && $this->input->post('additional_contact_two_phone') == $result->additional_contact_two_phone){
				$additional_contact_verify = $result->additional_contact_verify;
				$additional_contact_approved_by = $result->additional_contact_approved_by;
				$additional_contact_approved_on = $result->additional_contact_approved_on;
			}else{
				$additional_contact_verify = 0;
				$additional_contact_approved_by = 0;
				$additional_contact_approved_on = '0000:00:00 00:00:00';
			}
			...
			$user_document = array(
				...
				'badge_on' => $this->input->post('badge_on'),
				'badge_til' => $this->input->post('badge_til'),				
				'badge_verify' => $badge_verify,
				'badge_approved_by' => $badge_approved_by,
				'badge_approved_on' => $badge_approved_on,
				
				'training_certificate_no' => $this->input->post('training_certificate_no'),
				'training_certificate_on' => $this->input->post('training_certificate_on'),
				'training_certificate_til' => $this->input->post('training_certificate_til'),				
				'training_certificate_verify' => $training_certificate_verify,
				'training_certificate_approved_by' => $training_certificate_approved_by,
				'training_certificate_approved_on' => $training_certificate_approved_on,
				
				'experience_certificate_no' => $this->input->post('experience_certificate_no'),
				'experience_certificate_on' => $this->input->post('experience_certificate_on'),
				'experience_certificate_til' => $this->input->post('experience_certificate_til'),				
				'experience_certificate_verify' => $experience_certificate_verify,
				'experience_certificate_approved_by' => $experience_certificate_approved_by,
				'experience_certificate_approved_on' => $experience_certificate_approved_on,
				
				'medical_certificate_no' => $this->input->post('medical_certificate_no'),
				'medical_certificate_on' => $this->input->post('medical_certificate_on'),
				'medical_certificate_til' => $this->input->post('medical_certificate_til'),				
				'medical_certificate_verify' => $medical_certificate_verify,
				'medical_certificate_approved_by' => $medical_certificate_approved_by,
				'medical_certificate_approved_on' => $medical_certificate_approved_on,
				
				'health_insurance_no' => $this->input->post('health_insurance_no'),
				'health_insurance_on' => $this->input->post('health_insurance_on'),
				'health_insurance_til' => $this->input->post('health_insurance_til'),				
				'health_insurance_verify' => $health_insurance_verify,
				'health_insurance_approved_by' => $health_insurance_approved_by,
				'health_insurance_approved_on' => $health_insurance_approved_on,
				
				'term_insurance_no' => $this->input->post('term_insurance_no'),
				'term_insurance_on' => $this->input->post('term_insurance_on'),
				'term_insurance_til' => $this->input->post('term_insurance_til'),				
				'term_insurance_verify' => $term_insurance_verify,
				'term_insurance_approved_by' => $term_insurance_approved_by,
				'term_insurance_approved_on' => $term_insurance_approved_on,
				...
				'additional_contact_verify' => $additional_contact_verify,
				'additional_contact_approved_by' => $additional_contact_approved_by,
				'additional_contact_approved_on' => $additional_contact_approved_on,
			...
			if ($_FILES['badge_image']['size'] > 0) {
				...
				$user_document['badge_image'] = $result->badge_image;
			}
			...
			if ($_FILES['training_certificate_image']['size'] > 0) {
				...
				$user_document['training_certificate_image'] = $result->training_certificate_image;
			}
			...
			if ($_FILES['experience_certificate_image']['size'] > 0) {
				...
				$user_document['experience_certificate_image'] = $result->experience_certificate_image;
			}
			...
			if ($_FILES['medical_certificate_image']['size'] > 0) {
				...
				$user_document['medical_certificate_image'] = $result->medical_certificate_image;
			}
			...
			if ($_FILES['health_insurance_image']['size'] > 0) {
				...
				$user_document['health_insurance_image'] = $result->health_insurance_image;
			}
			...
			if ($_FILES['term_insurance_image']['size'] > 0) {
				...
				$user_document['term_insurance_image'] = $result->term_insurance_image;
			}
