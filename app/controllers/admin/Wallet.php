...
class Wallet extends MY_Controller
{
	...
	function index($action=false){
		$this->sma->checkPermissions('wallets_dashboard-index');
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
	...
	function customer($action=false){
		$this->sma->checkPermissions('wallets_customer-index');
		if($this->session->userdata('group_id') == 1){
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
    ...
    function getCustomer(){
        ...
        $this->datatables
            ->select("{$this->db->dbprefix('wallet')}.id as id, {$this->db->dbprefix('wallet')}.created as created_on, CONCAT({$this->db->dbprefix('wallet')}.join_id),  u.first_name as fname, u.last_name as lname, 
			...
			( CASE WHEN {$this->db->dbprefix('wallet')}.wallet_type = 2 THEN {$this->db->dbprefix('wallet')}.cash ELSE 0 END) as wallet_credit,
			 ...
			 {$this->db->dbprefix('wallet')}.description as description,  {$this->db->dbprefix('wallet')}.flag_method as flag, country.name as instance_country
			")
            ->from("wallet")
	...
	function driver($action=false){
		$this->sma->checkPermissions('wallets_driver-index');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
	...
	function owner($action=false){
		$this->sma->checkPermissions('wallets_owner-index');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
    ...
    function getOwner(){
        ...
        $this->datatables
            ->select("{$this->db->dbprefix('wallet')}.id as id, {$this->db->dbprefix('wallet')}.created as created_on, CONCAT({$this->db->dbprefix('wallet')}.join_id),  u.first_name as fname, u.last_name as lname, ( CASE WHEN {$this->db->dbprefix('wallet')}.wallet_type = 1 THEN {$this->db->dbprefix('wallet')}.cash ELSE 0 END) as wallet_cash,
	...
	function icicipayment(){
		//$base=new Eazypay();
		$amount = 900;
		$reference_no = 'AA12345';
		$url=$this->eazypay->getPaymentUrl($amount, $reference_no, $optionalField=null);
	}
	
	function icicipayment_addmoney($group_id){
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
		...
		$this->data['hash'] = $this->firstdata->createHash(13.00,978);
		//die;
		$user_data = $this->site->get_user($user_id, $countryCode);
		
				if (!empty($this->input->post('icici_payment_id')) && !empty($this->input->post('merchant_order_id'))) {
			...
			$meta = array('page_title' => lang('icicipayment'), 'bc' => $bc);
			$this->page_construct('wallet/icicipayment_addmoney', $meta, $this->data);	
	...
	function ccavenu_addmoney($group_id){
		...
		$user_data = $this->site->get_user($user_id, $countryCode);
				...
				if (!empty($this->input->post('merchant_id')) && !empty($this->input->post('order_id'))) {
			...
			//$this->data['payment_gateway'] = $this->account_model->getPaymentgateway($countryCode);
			$this->data['return_url'] = admin_url().'wallet/callback';
			$this->data['surl'] = base_url().'main/ccavenu_redirect';
			$this->data['furl'] = base_url().'main/ccavenu_cancel';
			$this->data['currency_code'] = 'INR';
			$this->data['paid_amount'] = $paid_amount;
			$this->data['payment_gateway'] = $payment_gateway;
			$this->data['payment_mode'] = $payment_mode;
			...
			$bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('driver_status')));
			$meta = array('page_title' => lang('ccavenu'), 'bc' => $bc);
			$this->page_construct('wallet/ccavenu_addmoney', $meta, $this->data);		
	}
    ...
    public function stripe_addmoney($group_id) {
	 ...
	 public function kpayment_addmoney($group_id) {
		
		$KpayToken = $this->input->post('token');
		if($this->session->userdata('group_id') == 1){
			if($this->input->get('is_country') != ''){
				$countryCode = $this->input->get('is_country');	
			}else{
		...
		$user_data = $this->site->get_user($user_id, $countryCode);
			...
			$bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('driver_status')));
			$meta = array('page_title' => lang('kpayment'), 'bc' => $bc);
			$this->page_construct('wallet/kpayment_addmoney', $meta, $this->data);		
