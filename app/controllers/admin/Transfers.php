...
class Transfers extends MY_Controller
{
    ...
    function getTransfers()
    {
        ...
        $this->datatables
            ->select("'sno',id, date, transfer_no, from_warehouse_name as fname, from_warehouse_code as fcode, to_warehouse_name as tname,to_warehouse_code as tcode, total, total_tax, grand_total, status, attachment")
            ->from('transfers')
...
print_r($this->input->post());die;*/
            ...
            $material_request_no = $this->input->post('material_request_no') ? $this->input->post('material_request_no') : 0;
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_expiry = $_POST['expiry'][$r] ? $_POST['expiry'][$r] : date('Y-m-d H:i:s'); /*? $this->sma->fsd($_POST['expiry'][$r]) : NULL;*/
            ...
            $data = array('transfer_no' => $transfer_no,
                'date' => $date,
                'material_request_id' => $material_request_no,
...
/*echo "<pre>";
        ...
        if ($this->form_validation->run() == true && $this->transfers_model->addTransfer($data, $products)) {
            ...
            $this->data['material_request'] = $this->site->getAllMaterial_RequestNo();
            /*echo "<pre>";
            print_r($this->data['material_request']);die;*/
    ...
    public function material_request_list(){
        $material_request_id =  $this->input->get('material_request_no');

        
        $data['materialrequests'] = $this->transfers_model->getMaterialRequestByID($material_request_id);

        $inv_items = $this->transfers_model->getAllMaterialRequestItems($material_request_id);
/*echo "<pre>";
       ...
       // krsort($inv_items);
        ...
        foreach ($inv_items as $item) {
           ...
           /* $row->received = $item->quantity_received ? $item->quantity_received : $item->quantity;*/
            ...
            $options = $this->transfers_model->getProduct_Options($row->id);
            $row->option = $item->option_id;
                ...
                $units = $this->site->getUnitsByBUID($row->base_unit);
                $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
            $ri = $this->Settings->item_addition ? $row->product_id : $row->id;
