...
class Drivers extends REST_Controller {
	...
	public function driverwaitingcancel_post(){
		...
		if ($this->form_validation->run() == true) {
			...
			if(!empty($rides->customer_id)){
				...
				$driver_cancel_charge = $setting->ride_cancel_driver_on_the_way_km_fare_enable == 0 ? $type_cancel->driver_cancel_charge : number_format($data['total_fare'],2);
			...
			//die;
				if($data == true){
					...
					$result = array( 'status'=> true , 'no_of_customer_cancel' => $type_cancel->no_of_driver_cancel, 'check_cancel_limit' => $check_cancel_limit, 'message'=> $check_msg, 'cancel_fare' => $setting->ride_cancel_driver_on_the_way_km_fare_enable == 0 ? $type_cancel->driver_cancel_charge : number_format($data['total_fare'],2), 'ride_cancel_driver_on_the_way_km_fare_enable' => $setting->ride_cancel_driver_on_the_way_km_fare_enable, 'data' => $details);
				}else{
	...
	public function start_ride_post(){
		...
		if ($this->form_validation->run() == true) {
			...
			$this->site->users_logs($countryCode, $user_data->id, $this->getUserIpAddr, json_encode($_POST), $_SERVER['REQUEST_URI']);
			$row['driver_id'] = $user_data->id;
			$onridecheck = $this->site->RidecancelCheckStartorOTP($user_data->id, $this->input->post('ride_otp'));
			if($onridecheck == 1){
				$result = array( 'status'=> false , 'message' => 'Ride Already Cancel.');
			}else{
			$row['ride_otp'] = $this->input->post('ride_otp');
