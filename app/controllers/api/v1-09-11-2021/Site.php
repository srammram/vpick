...
function GetDrivingDistanceNew1($point1_lat, $point1_long, $point2_lat, $point2_long, $unit = 'km', $decimals = 2, $countryCode) {
			...
			if($start_city_id != 0){
				...
				if ($daily_withcity->num_rows() > 0) {
					...
					if($slot_type != 'empty'){
							...
							$base_min_distance_price = $slot_include_fare;
							$base_per_distance = $daily_withcity->row('base_per_distance');
							$base_per_distance_price = $slot_extra_fare;
						}else{
							$base_min_distance = $daily_withcity->row('base_min_distance');
	...
	function getFareestimate1($start_lat, $start_lng, $taxi_type, $ride_type, $countryCode){
		$data = array();
		$start_pincode = $this->findLocationPINCODE1($start_lat, $start_lng);
			...
			if($start_city_id != 0){
				...
				if ($daily_withcity->num_rows() > 0) {
					...
					if($total_distance > $base_min_distance){
						...
						if($slot_type != 'empty'){
							$estimate_fare = round((($estimate_distance - $base_min_distance) * $slot_extra_fare) + $slot_include_fare + $actual_waiting_fare + $waiting_price );
							$actual_fare = round((($estimate_distance - $base_min_distance) * $slot_extra_fare) + $slot_include_fare + $actual_waiting_fare + $waiting_price);
							$total_fare = round((($estimate_distance - $base_min_distance) * $slot_extra_fare) + $slot_include_fare + $actual_waiting_fare + $waiting_price);
							$round_fare = round((($estimate_distance - $base_min_distance) * $slot_extra_fare) + $slot_include_fare + $actual_waiting_fare + $waiting_price);
						...
						if($slot_type != 'empty'){
							$estimate_fare = round($slot_include_fare + $actual_waiting_fare + $waiting_price);
							$actual_fare = round($slot_include_fare + $actual_waiting_fare + $waiting_price);
							$total_fare = round($slot_include_fare + $actual_waiting_fare + $waiting_price);
							$round_fare = round($slot_include_fare + $actual_waiting_fare + $waiting_price);
						}else{
