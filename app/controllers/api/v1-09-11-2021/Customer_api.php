...
$this->db->where('is_country', $countryCode);
	...
	function get_driver_location($user_id, $countryCode){
		$myQuery = "SELECT d.id, IFNULL(dcs.current_latitude, 0) as lat, IFNULL(dcs.current_longitude, 0) as lng,  r.start_lat as pickup_lat, r.start_lng as pickup_lng, r.end_lat as drop_lat, r.end_lng as drop_lng, IFNULL(r.middle, 0) as middle, IFNULL(r.middle_lat, 0) as middle_lat, IFNULL(r.middle_lng, 0) as middle_lng, r.change_type, r.customer_id, r.id AS ride_id, IFNULL(df.location, 0) as location, IFNULL(df.final_distance, 0) as final_distance, IFNULL(df.final_distance_total, 0) as final_distance_total, r.status, us.socket_id, us.id AS usid FROM kapp_users AS d  LEFT  JOIN kapp_rides AS r ON r.customer_id = d.id AND (r.status = 2 OR  r.status = 3) LEFT  JOIN kapp_driver_frequency AS df ON df.ride_id = r.id AND df.driver_id = d.id LEFT  JOIN kapp_user_socket AS us ON us.user_id = r.customer_id AND us.user_type = 1 LEFT JOIN kapp_driver_current_status dcs ON dcs.driver_id = r.driver_id AND dcs.is_connected = 1 AND (dcs.mode = 2 OR dcs.mode = 3) AND dcs.allocated_status = 1
...
ORDER BY distance ASC LIMIT 1";
	...
	function FaregetAllTaxiTypesnew($latitude, $longitude, $distance, $countryCode){
		...
		if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
				...
				$query1 = "SELECT  d.id as available, dcs.current_latitude, dcs.current_longitude, r.status, r.end_lat, r.end_lng, ( 6371 * acos( cos( radians({$latitude}) ) * cos( radians( dcs.current_latitude ) ) * cos( radians( dcs.current_longitude ) - radians({$longitude}) ) + sin( radians({$latitude}) ) * sin( radians( dcs.current_latitude ) ) ) ) AS distance FROM {$this->db->dbprefix('users')}  AS d 
	LEFT JOIN {$this->db->dbprefix('driver_current_status')} AS dcs ON dcs.driver_id = d.id  
	...
	LEFT JOIN {$this->db->dbprefix('groups')} AS g ON g.id = d.group_id 
	LEFT JOIN {$this->db->dbprefix('rides')} AS r ON r.driver_id = d.id AND r.taxi_id = t.id
	LEFT JOIN {$this->db->dbprefix('user_setting')} AS us ON us.user_id = d.id  AND us.ride_stop = 0
	WHERE tt.id = ".$row->id."  AND (dcs.mode = 1 OR dcs.mode = 3)  AND dcs.is_connected = 1 AND dcs.allocated_status = 1 GROUP BY d.id   HAVING distance <= {$distance}  
ORDER BY distance ASC";
				...
				if ($t->num_rows() > 0) {
					...
					foreach($t->result() as $trow){
							...
							if($trow->end_lat != 0 && $trow->end_lng != 0){
								$dis = $this->site->GetDrivingDistance_New($latitude, $longitude, $trow->end_lat, $trow->end_lng, $row->id, $countryCode);
								if(round($dis) < 3){
...
//(dcs.mode = 1 OR dcs.mode = 3)   AND dcs.is_connected = 1  AND dcs.allocated_status = 1 
	...
	function customerCancel($data, $countryCode){
		...
		$this->db->where('is_country', $countryCode);
		$q = $this->db->update('rides', array('cancel_status' => 1, 'cancel_id' => $data['cancel_id'], 'cancel_msg' => $data['cancel_msg'], 'cancelled_by' => $data['customer_id'], 'cancelled_type' => 2, 'status' => 6));
		if($q){
			
			$this->db->insert('outstandingfare', array('customer_id' => $data['customer_id'], 'customer_created' => date('Y-m-d H:i:s'), 'is_edit' => 1, 'customer_status' => 1, 'customer_fare' => $data['customer_cancel_charge']));
	...
	function customerChangeDrop($data, $distance_km, $waypoint_array, $countryCode){
		...
		if($q){
			...
			$this->db->select('cab_type_id, start, start_lat, start_lng, end, end_lat, end_lng, change_type, middle, middle_lat, middle_lng');
			$this->db->where('id', $data['booking_id']);
			$r = $this->db->get('rides');
			if($r->num_rows()>0){
				...
				$row->distance_price = $distance_price ? (string)$distance_price : '0.00';
				return $row;
			}
		}
		return false;	
