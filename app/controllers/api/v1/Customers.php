...
class Customers extends REST_Controller {
	...
	public function ridebase_main_post(){
		...
		if ($this->form_validation->run() == true) {
			...
			$data = $this->customer_api->getHelpmain($user_data->id, 'Ride Support', $countryCode);
	...
	public function general_main_post(){
		...
		if ($this->form_validation->run() == true) {
			...
			$customer_type = '2'; //1 - Credit, 2- Debit
			$data = $this->customer_api->getHelpmain($user_data->id, 'General Support', $countryCode);
	...
	public function customercancel_post(){
		...
		if ($this->form_validation->run() == true) {
			...
			$cancel['customer_cancel_type'] = $type_cancel->customer_cancel_type;
			...
			if(!empty($rides->driver_id)){
				...
				$driver_phone = $driver_data->country_code.$driver_data->mobile;
					...
					$socket_id = $this->site->getSocketID($rides->driver_id, 2, $countryCode);
					$customer_cancel_charge = number_format($data['customer_cancel_charge'],2);
			...
			if($data == true){
					...
					$result = array( 'status'=> 1 , 'no_of_customer_cancel' => $type_cancel->no_of_customer_cancel, 'check_cancel_limit' => $check_cancel_limit, 'message'=> $check_msg, 'cancel_fare' =>  number_format($data['customer_cancel_charge'],2), 'ride_cancel_driver_on_the_way_km_fare_enable' => $setting->ride_cancel_driver_on_the_way_km_fare_enable, 'data' => $details);
