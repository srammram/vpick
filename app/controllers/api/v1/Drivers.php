...
class Drivers extends REST_Controller {
	...
	public function driverstatus_post(){
		...
		if ($this->form_validation->run() == true) {
			...
			if($total_check > $check_wallet){
				...
				if($data == 1){
					$result = array( 'status'=> 1 , 'message'=> 'Driver status has been updated', 'mode' => $this->input->post('mode'));
				}elseif($data == 2){
	...
	public function startwithoutotp_ride_post(){
		...
		if ($this->form_validation->run() == true) {
			...
			$edata = array(
				'sos' => "http://13.233.109.60/sos?id=".$data->id,
	...
	public function submitride_post(){
		...
		if ($this->form_validation->run() == true) {
			...
			if(!empty($res)){
				...
				$labour_charge = $res->labour_charge;
				...
				$actual_fare = $res->actual_fare;
				$trip_distance = $res->min_distance;
				$trip_time = $res->min_time;				
				$trip_fare = $res->min_fare;
				$additional_distance_fare = $res->additional_fare;
				$additional_time_fare = $res->additional_time_fare;
				$labour_charge = $res->labour_charge;
				$discounts = $res->discount_fare;
				$total_toll = $this->input->post('total_toll') != NULL ? $this->input->post('total_toll') : 0;
				$total_parking = $this->input->post('total_parking') != NULL ? $this->input->post('total_parking') : 0;
				...
				if(!empty($user_data->id)){
					...
					$edata = array(
						...
						'customer_id' => $customer_id,				
						'socket_id' => $socket_id,
						'trip_fare' => (string)$trip_fare,
						'trip_distance' => (string)$trip_distance,
						'trip_time' => (string)$trip_time,
						'additional_distance_fare' => (string)$additional_distance_fare,
						'additional_time_fare' => (string)$additional_time_fare,
						...
						'outstanding_from_last_trip' =>  (string)$outstanding_from_last_trip,
						'total' => (string)$total_amount,
						'msg' => 'Your Ride has been Completed Successfully. Your Ride Amount is: '.$actual_fare.', Toll Amount:'.$total_toll.' Your Outstanding from last trip Amount is: '.$outstanding_from_last_trip.',  Total Payable Amount:'.$total_amount.''
			...
			if($status == 1){
				...
				if(!empty($ride_data)){
					...
					if($data[0] != FALSE){
						...
						$result = array( 'status'=> 1 , 'message'=> lang('accept_ride'), 'data' => $data_value);
						
					}else{
						$result = array( 'status'=> 0 , 'message'=> 'not accept ride');
					}
				}else{
					$result = array( 'status'=> 0 , 'message'=> lang('ride_cancel'));
				}
	...
	public function taxi_type_get(){
		...
		$countryCode = $this->input->get('is_country');
		$data = $this->drivers_api->getTypebyCategory($category_id, $countryCode);
		if($data == TRUE){
			$result = array( 'status'=> true , 'message'=> 'Success', 'data' => $data);
		}else{
			$result = array( 'status'=> false , 'message'=> 'Data is empty');
	...
	public function taxi_category_get(){
		$countryCode = $this->input->get('is_country');
		$data = $this->drivers_api->getTaxicategory($countryCode);
		if($data == TRUE){
			$result = array( 'status'=> true , 'message'=> 'Success', 'data' => $data);
		}else{
			$result = array( 'status'=> false , 'message'=> 'Data is empty');
	...
	/*public function taxi_type_post(){
