...
function cab_status(x) {
    ...
    $(document).ready(function () {
        ...
        oTable = $('#<?= lang('rides') ?>Table').dataTable({
            "aaSorting": [[0, "asc"]],
            "aLengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "<?= lang('all') ?>"]],
            "iDisplayLength": <?= $Settings->rows_per_page ?>,
            'bProcessing': true, 'bServerSide': true,
            'sAjaxSource': '<?= admin_url('enquiry/getOnRides?sdate='.$_GET['sdate'].'&edate='.$_GET['edate'].'&customer_type='.$_GET['customer_type'].'&user_id='.$_GET['user_id'].'&is_country='.$_GET['is_country']) ?>',
            ...
            <div class="col-lg-12">
               ...
               // echo admin_form_open_multipart("enquiry/create_ticket/?customer_id=".$_GET['user_id'], $attrib);
                ...
                <div class="table-responsive">
                    <table id="<?= lang('rides') ?>Table" cellpadding="0" cellspacing="0" border="0"
                           class="table table-bordered table-hover table-striped">
                        <thead>
                        <tr>
                            <th style="width:50px;"><?php echo lang('rides'); ?></th>
                            <th style="width:100px;"><?php echo lang('booking_time'); ?></th>
                            <th class="col-xs-2"><?php echo lang('Trip Code'); ?></th>
                            
                            
                          
                            <th style="width:100px;"><?php echo lang('pickup'); ?></th>
                            
                            <th style="width:100px;"><?php echo lang('drop'); ?></th>
...
$(document).ready(function(e) {
    $('#filte_ride').click(function(e) {
		...
		var is_country = '<?php echo $_GET['is_country'] ?>';
		...
		window.location.href = site+"admin/enquiry/create_ticket/?customer_id="+customer_id+"&ride_id="+ride_id+"&is_country="+is_country;
