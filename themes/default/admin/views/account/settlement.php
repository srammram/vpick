...
<div class="box">
    ...
    <div class="box-content">
           ...
           <div class="col-lg-12">
            ...
            echo admin_form_open_multipart("account/settlement/", $attrib);
            ?>
           ...
           <div class="col-lg-3 <?php if($this->session->userdata('group_id') != 1){ echo 'hidden'; } ?>">
            ...
            <div class="col-sm-12 last_sa_se">
                <?php echo form_submit('settlement', lang('submit'), 'class="btn btn-primary change_btn_save center-block settlement"'); ?>
             </div>
            <?php echo form_close(); ?>
            </div>
...
$(document).on('change', '#from_company_id', function(){
	...
	$.ajax({
		...
		success: function (scdata) {
			...
			$pending = '';
			if(scdata.pending.length != 0){
				$.each(scdata.pending,function(n,v){
					$pending += '<input type="checkbox" name="account_id[]" value="'+v.id+'"> "'+v.account_transaction_no+'" - "'+v.debit+'" ';
				});
...
$(document).on('change', '#is_country', function(){
	...
	var is_country = $('#is_country').val();
	window.location.href = site+"admin/account/settlement/?is_country="+is_country;
});
</script>
