...
io.sockets.on('connection', function (socket){
	...
	socket.on('driver_location', function(data){
		...
		if(oauth_token == '' || latitude == '' || longitude == ''){
			...
			//"SELECT d.id, dcs.is_connected, dcs.current_latitude as cu_latitude, dcs.current_longitude as cu_longitude,  r.start_lat, r.start_lng, r.end_lat, r.end_lng, r.customer_id, r.id AS ride_id, df.location, df.final_distance, df.final_distance_total, r.status, us.socket_id, us.id AS usid FROM kapp_users AS d LEFT JOIN kapp_driver_current_status dcs ON dcs.driver_id = d.id AND dcs.is_connected = 1 AND dcs.mode = 3 AND dcs.allocated_status = 1 LEFT  JOIN kapp_rides AS r ON r.driver_id = d.id AND (r.status = 2 OR  r.status = 3) LEFT  JOIN kapp_driver_frequency AS df ON df.ride_id = r.id AND df.driver_id = d.id LEFT  JOIN kapp_user_socket AS us ON us.user_id = r.customer_id AND us.user_type = 1 WHERE d.oauth_token = '"+oauth_token+"' ORDER BY d.id DESC LIMIT 1"
			pool.query("SELECT d.id, dcs.is_connected, dcs.current_latitude as cu_latitude, dcs.current_longitude as cu_longitude,  r.start_lat, r.start_lng, r.end_lat, r.end_lng, r.customer_id, r.id AS ride_id, df.location, df.final_distance, df.final_distance_total, r.status, us.socket_id, us.id AS usid FROM kapp_users AS d LEFT JOIN kapp_driver_current_status dcs ON dcs.driver_id = d.id AND dcs.allocated_status = 1  LEFT  JOIN kapp_rides AS r ON r.driver_id = d.id AND (r.status = 2 OR  r.status = 3) LEFT  JOIN kapp_driver_frequency AS df ON df.ride_id = r.id AND df.driver_id = d.id LEFT  JOIN kapp_user_socket AS us ON us.user_id = r.customer_id AND us.user_type = 1 WHERE d.oauth_token = '"+oauth_token+"' ORDER BY d.id DESC LIMIT 1", function (error, results, fields){
				if(error){
					console.log('Data is empty');
					return;
				}
				...
				if(results.length == 0){
					...
					var admin_driver_id1= results[0].id;
						...
						var admin_is_connected1 = results[0].is_connected ? results[0].is_connected : 0;
						socket.emit('admin_drivers_location', {'admin_driver_id': admin_driver_id1, 'admin_ride_id': admin_ride_id1, 'admin_lat': admin_lat1, 'admin_lng': admin_lng1, 'admin_is_connected' : admin_is_connected1 });
					...
					if(results[0].ride_id != null){
						...
						var admin_is_connected = results[0].is_connected ? results[0].is_connected : 0;
						
						
						socket.emit('admin_drivers_location', {'admin_driver_id': admin_driver_id, 'admin_ride_id': admin_ride_id, 'admin_lat': admin_lat, 'admin_lng': admin_lng, 'admin_is_connected' : admin_is_connected });
						...
						var admin_lng4 = longitude;
						var admin_is_connected4 = results[0].is_connected ? results[0].is_connected : 0;
						
						io.sockets.emit('admin_drivers_location', {'admin_driver_id': admin_driver_id4, 'admin_ride_id': admin_ride_id4, 'admin_lat': admin_lat4, 'admin_lng': admin_lng4, 'admin_is_connected' : admin_is_connected4 });
