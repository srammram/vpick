<script>
    /*function vendor_status(x) {
        var y = x.split("__");
        return y[0] == 1 ?
        '<a href="'+site.base_url+'people/vendor_status/active/'+ y[1] +'"><span class="label label-success">  '+lang['active']+'</span></a>' :
        '<a href="'+site.base_url+'people/vendor_status/deactive/'+ y[1] +'"><span class="label label-danger">  '+lang['inactive']+'</span><a/>';
    }*/
	...
	function jointype_status(w) {
        ...
        return c[1] == 1 ?
        '<a href="'+site.base_url+'people/vendor_adminedit/'+ c[0] +'"><span class="label label-info">  Pending </span></a>' :
	...
	function verify_status(b) {
        ...
        return z[1] == 1 ?
        '<a href="'+site.base_url+'people/vendor_status/active/'+ z[0] +'"><span class="label label-success">  Verified</span></a>' :
        '<a href="'+site.base_url+'people/vendor_status/deactive/'+ z[0] +'"><span class="label label-danger">  Not Verified</span><a/>';
    }
    ...
    $(document).ready(function () {
        ...
        oTable = $('#UsrTable').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('people/getVendor?sdate='.$_GET['sdate'].'&edate='.$_GET['edate'].'&approved='.$_GET['approved'].'&cab='.$_GET['cab'].'&driver='.$_GET['driver'].'&is_country='.$_GET['is_country']) ?>',
            ...
            "aoColumns": [ {"mRender": empty_status}, {"mRender": empty_status}, {"mRender": empty_status}, {"mRender": empty_status}, {"mRender": mobile_status},  {"mRender": empty_status}, {"mRender": verify_status}, {"mRender": jointype_status}, {"mRender": empty_status}, {"bSortable": true}]
        });
    });
</script>
...
<?php if ($Owner) {
     echo admin_form_open('people/vendor_actions', 'id="action-form"');
} ?>
<div class="box">
 <a href="<?= admin_url('people/add_vendor'); ?>"><button type="button" class="btn btn-primary add_se_btn center-block"><i class="fa fa-plus-circle"></i> <?= lang("add_vendor"); ?></button></a>
   <a href="javascript:void(0)" id="excel" data-action="export_excel"><button type="button" class="btn btn-primary add_se_btn center-block"><i class="fa fa-file-excel-o"></i> <?= lang("export_to_excel"); ?></button></a>
    <div class="box-content">
        <div class="row">
            ...
            <div class="col-lg-3">
            <div class="form-group">
            <?php echo lang('Approved', 'approved'); ?>
            <?php
            $s[''] = array('0' => lang('all'), '1' => lang('Verified'), '2' => lang('Not Verified'));
            echo form_dropdown('approved', $s, $_GET['approved'], 'class="tip form-control" id="approved" data-placeholder="' . lang("select") . ' ' . lang("approved") . '"');
            ?>
            </div>
            </div> 
...
$(document).ready(function(){
	...
	$('#filte_ride').click(function(e) {
		...
		window.location.href = site+"admin/people/vendor?sdate="+sdate+"&edate="+edate+"&approved="+approved+"&is_country="+is_country;
