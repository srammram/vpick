...
<script>
	...
	$(document).ready(function() {	
	 oTable = $('#UsrTable').DataTable({
		 ...
		 paging: true,
    	 searching: true,
         'sAjaxSource': '<?= admin_url('masters/getDiscount?is_country='.$_GET['is_country']) ?>',
		 "aoColumns": [ {"mRender": empty_status}, {"mRender": empty_status}, {"mRender": type_status}, {"mRender": empty_status}, {"mRender": empty_status},  {"mRender": discount_status}, {"mRender": empty_status}, {"bSortable": true}],
		 "pageLength": 10,
		 "dom": 'lBfrtip<"actions">',
		 buttons: [
        
        {
            ...
            text: 'Export to CSV',
			title: 'discount-csv-<?=date('YmdHis');?>',
            exportOptions: {
        ...
        },
        {
			...
			title: 'discount-excel-<?=date('YmdHis');?>',
            exportOptions: {
                columns:  [0,1,2,3,4,5,6]
            },
...
<script>
	function type_status(d) {
        var y = d;
		if(y[0] == 0){
			return '<span class="label label-success">All Days</span>';
		}else if(y[0] == 1){
			return '<span class="label label-success">Particular Days</span>';
		}else if(y[0] == 2){
			return '<span class="label label-success">Dates</span>';
		}
       /* return y[0] == 1 ?
    ...
    function discount_status(x) {
        var y = x.split("__");
        return y[0] == 1 ?
        '<a href="'+site.base_url+'masters/discount_status/deactivate/'+ y[1] +'"><span class="label label-success">  '+lang['active']+'</span></a>' :
        '<a href="'+site.base_url+'masters/discount_status/activate/'+ y[1] +'"><span class="label label-danger">  '+lang['inactive']+'</span><a/>';
    }
...
<div class="box">
    ...
    <a href="<?= admin_url('masters/add_discount'); ?>"><button type="button" class="btn btn-primary add_se_btn center-block"><i class="fa fa-plus-circle"></i> <?= lang("add_discount"); ?></button></a>
   <!-- <a href="<?= admin_url('masters/import_csv_discount'); ?>" id="import"><button type="button" class="btn btn-primary add_se_btn center-block"><i class="fa fa-file-excel-o"></i> <?= lang("import_to_csv"); ?></button></a>-->
                ...
                <div class="table-responsive">
                    <table id="UsrTable" cellpadding="0" cellspacing="0" border="0"
                        ...
                        <tr>
                            ...
                            <th class="col-xs-2"><?php echo lang('percentage'); ?></th>
                            <th class="col-xs-2"><?php echo lang('discount_type'); ?></th>
                            <th class="col-xs-2"><?php echo lang('days'); ?></th>
                            <th class="col-xs-2"><?php echo lang('dates'); ?></th>
    ...
    <script language="javascript">
        $(document).ready(function () {
            $('#filte_ride').click(function(e) {
				...
				window.location.href = site+"admin/masters/discount?is_country="+is_country;
