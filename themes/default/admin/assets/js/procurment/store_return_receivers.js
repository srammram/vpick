...
if (store_rtnrec_discount = localStorage.getItem('store_rtnrec_discount')) {
    $('#store_rtnrec_discount').val(store_rtnrec_discount);
}
...
$('#postatus').change(function (e) {
    localStorage.setItem('store_rtnrec_status', $(this).val());
});
if (postatus = localStorage.getItem('store_rtnrec_status')) {
...
$('#poshipping').focus(function () {
    ...
    localStorage.setItem('store_rtnrec_shipping', shipping);
    var gtotal = ((total + invoice_tax) - order_discount) + shipping;
    $('#gtotal').text(formatMoney(gtotal));
    $('#tship').text(formatMoney(shipping));
});
if (poshipping = localStorage.getItem('store_rtnrec_shipping')) {
    shipping = parseFloat(poshipping);
    $('#poshipping').val(shipping);
}

$('#popayment_term').change(function (e) {
    localStorage.setItem('store_rtnrec_payment_term', $(this).val());
});
if (popayment_term = localStorage.getItem('store_rtnrec_payment_term')) {
...
if (localStorage.getItem('store_rtnrec_items')) {
    ...
    $('#reset').click(function (e) {
        bootbox.confirm(lang.r_u_sure, function (result) {
            if (result) {
                if (localStorage.getItem('store_rtnrec_items')) {
                    localStorage.removeItem('store_rtnrec_items');
                }
                if (localStorage.getItem('store_rtnrec_discount')) {
                    localStorage.removeItem('store_rtnrec_discount');
                }
                if (localStorage.getItem('store_rtnrec_tax2')) {
                    localStorage.removeItem('store_rtnrec_tax2');
                }
                if (localStorage.getItem('store_rtnrec_shipping')) {
                    localStorage.removeItem('store_rtnrec_shipping');
                }
                if (localStorage.getItem('store_rtnrec_ref')) {
                    localStorage.removeItem('store_rtnrec_ref');
                }
                if (localStorage.getItem('store_rtnrec_warehouse')) {
                    localStorage.removeItem('store_rtnrec_warehouse');
                }
                if (localStorage.getItem('store_rtnrec_note')) {
                    localStorage.removeItem('store_rtnrec_note');
                }
                if (localStorage.getItem('store_rtnrec_supplier')) {
                    localStorage.removeItem('store_rtnrec_supplier');
                }
                if (localStorage.getItem('store_rtnrec_currency')) {
                    localStorage.removeItem('store_rtnrec_currency');
                }
                if (localStorage.getItem('store_rtnrec_extras')) {
                    localStorage.removeItem('store_rtnrec_extras');
                }
                if (localStorage.getItem('store_rtnrec_date')) {
                    localStorage.removeItem('store_rtnrec_date');
                }
                if (localStorage.getItem('store_rtnrec_status')) {
                    localStorage.removeItem('store_rtnrec_status');
                }
				
				if (localStorage.getItem('store_rtnrec_from_store_id')) {
                    localStorage.removeItem('store_rtnrec_from_store_id');
                }
				if (localStorage.getItem('store_rtnrec_to_store_id')) {
                    localStorage.removeItem('store_rtnrec_to_store_id');
                }
				
                if (localStorage.getItem('store_rtnrec_payment_term')) {
                    localStorage.removeItem('store_rtnrec_payment_term');
                }
...
var $supplier = $('#store_rtnrec_supplier'), $currency = $('#pocurrency');

$('#poref').change(function (e) {
    localStorage.setItem('store_rtnrec_ref', $(this).val());
});

if (poref = localStorage.getItem('store_rtnrec_ref')) {
    $('#poref').val(poref);
}

$('#invoice_no').change(function (e) {
    localStorage.setItem('store_rtnrec_invoice_no', $(this).val());
});

if (store_rtnrec_invoice_no = localStorage.getItem('store_rtnrec_invoice_no')) {
    $('#invoice_no').val(store_rtnrec_invoice_no);
}


$('#powarehouse').change(function (e) {
    localStorage.setItem('store_rtnrec_warehouse', $(this).val());
});
if (powarehouse = localStorage.getItem('store_rtnrec_warehouse')) {
    $('#powarehouse').select2("val", powarehouse);
        ...
        $('#ponote').redactor({
            ...
            changeCallback: function (e) {
                var v = this.get();
                localStorage.setItem('store_rtnrec_note', v);
            }
        });
        if (ponote = localStorage.getItem('store_rtnrec_note')) {
            $('#ponote').redactor('set', ponote);
        }
        $supplier.change(function (e) {
            localStorage.setItem('store_rtnrec_supplier', $(this).val());
            $('#supplier_id').val($(this).val());
        });
        if (store_rtnrec_supplier = localStorage.getItem('store_rtnrec_supplier')) {
            $supplier.val(store_rtnrec_supplier).select2({
    ...
    /*$('.rexpiry').change(function (e) {
        ...
        store_rtnrec_items[item_id].row.expiry = $(this).val();
        localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
    });*/
if (localStorage.getItem('store_rtnrec_extras')) {
    $('#extras').iCheck('check');
    $('#extras-con').show();
...
$('#extras').on('ifChecked', function () {
    localStorage.setItem('store_rtnrec_extras', 1);
...
$(document).on('change', '.rmfg', function () {
    var item_id = $(this).closest('tr').attr('data-item-id');
    store_rtnrec_items[item_id].row.mfg = $(this).val();
    localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
});
*/
...
$(document).on('change', '.rbatch_no', function () {
    var item_id = $(this).closest('tr').attr('data-item-id');
    store_rtnrec_items[item_id].row.batch_no = $(this).val();
    localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
});
...
if (site.settings.tax2 != 0) {
    $('#potax2').change(function () {
        localStorage.setItem('store_rtnrec_tax2', $(this).val());
        loadItems();
        return;
    });
}
...
var old_store_rtnrec_discount;
$('#store_rtnrec_discount').focus(function () {
    old_store_rtnrec_discount = $(this).val();
}).change(function () {
    var pod = $(this).val() ? $(this).val() : 0;
    if (is_valid_discount(pod)) {
        localStorage.removeItem('store_rtnrec_discount');
        localStorage.setItem('store_rtnrec_discount', pod);
        loadItems();
        return;
    } else {
        $(this).val(old_store_rtnrec_discount);
        bootbox.alert(lang.unexpected_value);
        return;
    }
     ...
     * ---------------------- */
     ...
     $(document).on('click', '.podel', function () {
        ...
        delete store_rtnrec_items[item_id];
        row.remove();
        if(store_rtnrec_items.hasOwnProperty(item_id)) { } else {
            localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
            loadItems();
            return;
        }
     ...
     * Edit Row Modal Hanlder
     ...
     $(document).on('click', '.edit', function () {
        ...
        item = store_rtnrec_items[item_id];
                ...
                $.each(tax_rates, function () {
                    if(this.id == pr_tax){
                        if (this.type == 1) {
                            ...
                            if (store_rtnrec_items[item_id].row.tax_method == 0) {
    ...
    $(document).on('change', '#pcost, #ptax, #ptax_method, #pdiscount', function () {
        ...
        var item = store_rtnrec_items[item_id];
    ...
    $(document).on('change', '#punit', function () {
        ...
        var unit = $('#punit').val();
        if(unit != store_rtnrec_items[item_id].row.base_unit) {
            $.each(item.units, function() {
                if (this.id == unit) {
     ...
     * Edit Row Method
     ...
     $(document).on('click', '#editItem', function () {
        ...
        if(unit != store_rtnrec_items[item_id].row.base_unit) {
            $.each(store_rtnrec_items[item_id].units, function(){
                if (this.id == unit) {
        ...
        store_rtnrec_items[item_id].row.fup = 1,
        store_rtnrec_items[item_id].row.qty = parseFloat($('#pquantity').val()),
        store_rtnrec_items[item_id].row.base_quantity = parseFloat(base_quantity),
        store_rtnrec_items[item_id].row.unit = unit,
        store_rtnrec_items[item_id].row.real_unit_cost = parseFloat($('#pcost').val()),
        store_rtnrec_items[item_id].row.tax_rate = new_pr_tax,
		store_rtnrec_items[item_id].row.tax_method = new_pr_tax_method,
        store_rtnrec_items[item_id].tax_rate = new_pr_tax_rate,
        store_rtnrec_items[item_id].row.discount = $('#pdiscount').val() ? $('#pdiscount').val() : '0',
        store_rtnrec_items[item_id].row.option = $('#poption').val(),		
		store_rtnrec_items[item_id].row.tax1 = $('#ptax1').val(),
		store_rtnrec_items[item_id].row.tax2 = $('#ptax2').val(),		
        localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
        $('#prModal').modal('hide');               
        loadItems();
        return;
     ...
     * Edit Row Quantity Method
     ...
     $(document).on("focus", '.rquantity', function () {
        ...
        item_id = row.attr('data-item-id');
        store_rtnrec_items[item_id].row.base_quantity = new_qty;
        if(store_rtnrec_items[item_id].row.unit != store_rtnrec_items[item_id].row.base_unit) {
            $.each(store_rtnrec_items[item_id].units, function(){
                if (this.id == store_rtnrec_items[item_id].row.unit) {
                    store_rtnrec_items[item_id].row.base_quantity = unitToBaseQty(new_qty, this);
                }
            });
        }
        store_rtnrec_items[item_id].row.qty = new_qty;
        store_rtnrec_items[item_id].row.received = new_qty;
        localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
        loadItems();
    });
	...
	$(document).on("change", '.rtax1', function () {
		...
		item_id = row.attr('data-item-id');
		store_rtnrec_items[item_id].row.tax_method = current_tax_method;
		localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
		loadItems();
	});
	...
	$(document).on("change", '.rtax2', function () {
		...
		item_id = row.attr('data-item-id');
		store_rtnrec_items[item_id].tax_rate = current_tax;
		localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
		loadItems();
	});
    ...
    var old_received;
     $(document).on("focus", '.received', function () {
        ...
        item_id = row.attr('data-item-id');
        if (new_received > store_rtnrec_items[item_id].row.qty) {
            $(this).val(old_received);
            bootbox.alert(lang.unexpected_value);
            return;
        }
        ...
        store_rtnrec_items[item_id].row.unit_received = new_received;
        store_rtnrec_items[item_id].row.received = qty_received;
        localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
        loadItems();
    });
    ...
    $(document).on("change", '.rucost', function () {
        ...
        item_id = row.attr('data-item-id');
        store_rtnrec_items[item_id].row.real_unit_cost = current_ucost;
        localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
        loadItems();
    });
    ...
    $(document).on("change", '.rdiscount', function () {
        ...
        item_id = row.attr('data-item-id');
        store_rtnrec_items[item_id].row.discount = current_discount;
        localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
        loadItems();
    });       
    /* --------------------------
     * Edit Row Cost Method
     ...
     $(document).on("focus", '.rcost', function () {
        ...
        item_id = row.attr('data-item-id');
        store_rtnrec_items[item_id].row.cost = new_cost;
        localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
        loadItems();
    });

    $(document).on("click", '#removeReadonly', function () {
     $('#store_rtnrec_supplier').select2('readonly', false);
     return false;
 });

    if (store_return_receivers_edit) {
        $('#store_rtnrec_supplier').select2("readonly", true);
    }
...
function loadItems() {
    ...
    if (localStorage.getItem('store_rtnrec_items')) {
        ...
        order_discount = 0;
        total_discount = 0;
        $("#store_return_receiversTable tbody").empty();
		
        store_rtnrec_items = JSON.parse(localStorage.getItem('store_rtnrec_items'));

		sortedItems = store_rtnrec_items;
       // sortedItems = (site.settings.item_addition == 1) ? _.sortBy(store_rtnrec_items, function(o){return [parseInt(o.order)];}) : store_rtnrec_items;
        ...
        $.each(sortedItems, function () {			
            ...
            var supplier = localStorage.getItem('store_rtnrec_supplier'), belong = false;
		  ...
		  tr_html += '<td><input name="available_qty[]" type="hidden" class="ravailable_qty" id="available_qty_' + row_no + '" value="' + item_available_qty + '"><span class="savailable_qty" >' + item_available_qty +'</span> </td>';
           ...
           /* if (store_rtnrec_edit) {
            ...
            newTr.html(tr_html);
            newTr.appendTo("#store_return_receiversTable");
        ...
        //$('#store_return_receiversTable tfoot').html(tfoot);

        // Order level discount calculations
        if (store_rtnrec_discount = localStorage.getItem('store_rtnrec_discount')) {
            var ds = store_rtnrec_discount;
            if (ds.indexOf("%") !== -1) {
                var pds = ds.split("%");
                if (!isNaN(pds[0])) {
        ...
        if (site.settings.tax2 != 0) {
            if (potax2 = localStorage.getItem('store_rtnrec_tax2')) {
                $.each(tax_rates, function () {
                    if (this.id == potax2) {
                        if (this.type == 2) {
 ...
 * @returns {Boolean}
 ---------------------------- */
 function add_store_return_receivers_item(item) {
    if (count == 1) {
        store_rtnrec_items = {};
        if ($('#store_rtnrec_supplier').val()) {
   ...
   var item_id = item.item_id;
    ...
    if (store_rtnrec_items[item_id]) {
		
		bootbox.confirm('This item is already added. Do you want to add it again?', function (result) {
			
			if (result) {
				var new_qty = parseFloat(store_rtnrec_items[item_id].row.qty) + 1;
				store_rtnrec_items[item_id].row.base_quantity = new_qty;
				if(store_rtnrec_items[item_id].row.unit != store_rtnrec_items[item_id].row.base_unit) {
					$.each(store_rtnrec_items[item_id].units, function(){
						if (this.id == store_rtnrec_items[item_id].row.unit) {
				...
				store_rtnrec_items[item_id].row.qty = new_qty;
				store_rtnrec_items[item_id].row.mfg = '';
				store_rtnrec_items[item_id].order = new Date().getTime();
				localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
				loadItems();
				return true;
        ...
        store_rtnrec_items[item_id] = item;
		store_rtnrec_items[item_id].row.mfg = '';
		store_rtnrec_items[item_id].order = new Date().getTime();
		localStorage.setItem('store_rtnrec_items', JSON.stringify(store_rtnrec_items));
		loadItems();
