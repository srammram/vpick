...
$('#ptax2').change(function (e) {
    localStorage.setItem('ptax2', $(this).val());
});
if (ptax1= localStorage.getItem('ptax1')) {
...
$(document).on('change', '.rmfg', function () {
    ...
    poitems[item_id].row.mfg = $(this).val();
	poitems[item_id].row.expiry = expiry_date;
    localStorage.setItem('poitems', JSON.stringify(poitems));
});

$(document).on('change', '.rbatch_no', function () {
    var item_id = $(this).closest('tr').attr('data-item-id');
    poitems[item_id].row.batch_no = $(this).val();
    localStorage.setItem('poitems', JSON.stringify(poitems));
});
...
$('#podiscount').focus(function () {
     ...
     * Edit Row Method
     ...
     $(document).on('click', '#editItem', function () {
		...
		poitems[item_id].row.tax_method = new_pr_tax_method,
		...
		poitems[item_id].row.batch_no = $('#pbatch_no').val() ? $('#pbatch_no').val() : '',
		poitems[item_id].row.tax1 = $('#ptax1').val(),
		poitems[item_id].row.tax2 = $('#ptax2').val(),
		poitems[item_id].row.mfg = $('#pmfg').val() ? $('#pmfg').val() : '',
	...
	$(document).on("change", '.rtax1', function () {
		...
		item_id = row.attr('data-item-id');
		poitems[item_id].row.tax_method = current_tax_method;
		localStorage.setItem('poitems', JSON.stringify(poitems));
		loadItems();
	});
	...
	$(document).on("change", '.rtax2', function () {
		...
		item_id = row.attr('data-item-id');
		poitems[item_id].tax_rate = current_tax;
		localStorage.setItem('poitems', JSON.stringify(poitems));
		loadItems();
	});
	...
	$(document).on("change", '.rucost', function () {
		...
		item_id = row.attr('data-item-id');
		poitems[item_id].row.real_unit_cost = current_ucost;
		localStorage.setItem('poitems', JSON.stringify(poitems));
		loadItems();
	});
	...
	$(document).on("change", '.rdiscount', function () {
		...
		item_id = row.attr('data-item-id');
		poitems[item_id].row.discount = current_discount;
		localStorage.setItem('poitems', JSON.stringify(poitems));
		loadItems();
	});
    ...
    if (localStorage.getItem('poitems')) {
        ...
        $.each(sortedItems, function () {
           ...
           // var item_id = site.settings.item_addition == 1 ? item.item_id : item.id;
			...
			var total_expiry = 0;
			if(type_expiry == 'day'){
				total_expiry = value_expiry * 1;
			}else if(type_expiry == 'month'){
				total_expiry = value_expiry * 30;
			}else if(type_expiry == 'year'){
				total_expiry = value_expiry * 365;
			}else{
				total_expiry = value_expiry * 0;
			}
                ...
                /*if(item.row.fup != 1 && product_unit != item.row.base_unit) {
                    $.each(item.units, function(){
                        if (this.id == product_unit) {
                            ...
                            unit_cost_calculated = formatDecimal((parseFloat(item.row.base_unit_cost)*(unitToBaseQty(1, this))), 4);
							unit_cost = (unit_cost_calculated) ? unit_cost_calculated : unit_cost;
                        }
                    });
                }*/
            ...
            var newTr = $('<tr id="row_' + row_no + '" class="row_' + item_id + '" data-total-expiry="'+total_expiry+'" data-item-id="' + item_id + '"></tr>');
			...
			//<i class="pull-right fa fa-edit tip edit" id="' + row_no + '" data-item="' + item_id + '" title="Edit" style="cursor:pointer;"></i>
			 ...
			 tr_html += '<td><input class="form-control rbatch_no" name="batch_no[]" type="text" value="' + (item_batch_no ? item_batch_no : '') + '" data-id="' + row_no + '" data-item="' + item_id + '" id="batch_no_' + row_no + '"></td>';
			 
            if (site.settings.product_expiry == 1) {
				 tr_html += '<td><input class="form-control date rmfg" name="mfg[]" type="text" value="' + (item_mfg ? item_mfg :  '') + '" data-id="' + row_no + '" data-item="' + item_id + '" id="mfg_' + row_no + '"></td>';
                tr_html += '<td><input class="form-control date rexpiry" name="expiry[]" type="text" value="' + (item_expiry ? item_expiry :  '') + '" data-id="' + row_no + '" data-item="' + item_id + '" id="expiry_' + row_no + '"></td>';
            }
        ...
        if (site.settings.product_discount == 1) {
		 ...
		 tfoot += '<th class="text-right">'+formatMoney(0)+'</th>';
    ...
    if (poitems[item_id]) {
		...
		bootbox.confirm('This item is already added. Do you want to add it again?', function (result) {
			...
			if (result) {
				...
				poitems[item_id].row.qty = new_qty;
				poitems[item_id].row.batch_no = '';
				poitems[item_id].row.mfg = '';
