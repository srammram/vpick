...
if (podiscount = localStorage.getItem('quotediscount')) {
    $('#podiscount').val(podiscount);
}
$('#ptax2').change(function (e) {
    localStorage.setItem('quotetax2', $(this).val());
});
if (ptax1= localStorage.getItem('quotetax1')) {
    $('#ptax1').select2("val", ptax1);
}
$('#postatus').change(function (e) {
    localStorage.setItem('quotestatus', $(this).val());
});
if (postatus = localStorage.getItem('quotestatus')) {
...
if (poshipping = localStorage.getItem('quoteshipping')) {
    shipping = parseFloat(poshipping);
    $('#poshipping').val(shipping);
}

$('#popayment_term').change(function (e) {
    localStorage.setItem('quotepayment_term', $(this).val());
});
if (popayment_term = localStorage.getItem('quotepayment_term')) {
...
$('#poref').change(function (e) {
    localStorage.setItem('quoteref', $(this).val());
});
if (poref = localStorage.getItem('quoteref')) {
    $('#poref').val(poref);
}
$('#powarehouse').change(function (e) {
    localStorage.setItem('quotewarehouse', $(this).val());
});
if (powarehouse = localStorage.getItem('quotewarehouse')) {
    $('#powarehouse').select2("val", powarehouse);
        ...
        $('#ponote').redactor({
            ...
            changeCallback: function (e) {
                var v = this.get();
                localStorage.setItem('quotenote', v);
            }
        });
        if (ponote = localStorage.getItem('quotenote')) {
...
$(document).on('change', '.rbatch_no', function () {
    var item_id = $(this).closest('tr').attr('data-item-id');
    quoteitems[item_id].row.batch_no = $(this).val();
    localStorage.setItem('quoteitems', JSON.stringify(quoteitems));
});
...
$('#podiscount').focus(function () {
    ...
    if (is_valid_discount(pod)) {
        localStorage.removeItem('quotediscount');
        localStorage.setItem('quotediscount', pod);
        loadItems();
        return;
    } else {
     ...
     * ---------------------- */
     ...
     $(document).on('click', '.podel', function () {
        ...
        delete quoteitems[item_id];
        row.remove();
        if(quoteitems.hasOwnProperty(item_id)) { } else {
            localStorage.setItem('quoteitems', JSON.stringify(quoteitems));
            loadItems();
            return;
        }
                ...
                $.each(tax_rates, function () {
                    if(this.id == pr_tax){
                        if (this.type == 1) {
                            ...
                            if (quoteitems[item_id].row.tax_method == 0) {
    ...
    $(document).on('change', '#punit', function () {
        ...
        var unit = $('#punit').val();
        if(unit != quoteitems[item_id].row.base_unit) {
            $.each(item.units, function() {
                if (this.id == unit) {
     ...
     * Edit Row Method
     ...
     $(document).on('click', '#editItem', function () {
        ...
        quoteitems[item_id].row.fup = 1,
        quoteitems[item_id].row.qty = parseFloat($('#pquantity').val()),
        quoteitems[item_id].row.base_quantity = parseFloat(base_quantity),
        quoteitems[item_id].row.unit = unit,
        quoteitems[item_id].row.real_unit_cost = parseFloat($('#pcost').val()),
        quoteitems[item_id].row.tax_rate = new_pr_tax,
		quoteitems[item_id].row.tax_method = new_pr_tax_method,
        quoteitems[item_id].tax_rate = new_pr_tax_rate,
        quoteitems[item_id].row.discount = $('#pdiscount').val() ? $('#pdiscount').val() : '0',
        quoteitems[item_id].row.option = $('#poption').val(),		
		quoteitems[item_id].row.tax1 = $('#ptax1').val(),
		quoteitems[item_id].row.tax2 = $('#ptax2').val(),		
        localStorage.setItem('quoteitems', JSON.stringify(quoteitems));
        $('#prModal').modal('hide');               
        loadItems();
        return;
     ...
     * Edit Row Quantity Method
     ...
     $(document).on("focus", '.rquantity', function () {
        ...
        item_id = row.attr('data-item-id');
        quoteitems[item_id].row.base_quantity = new_qty;
        if(quoteitems[item_id].row.unit != quoteitems[item_id].row.base_unit) {
            $.each(quoteitems[item_id].units, function(){
                if (this.id == quoteitems[item_id].row.unit) {
                    quoteitems[item_id].row.base_quantity = unitToBaseQty(new_qty, this);
                }
            });
        }
        quoteitems[item_id].row.qty = new_qty;
        quoteitems[item_id].row.received = new_qty;
        localStorage.setItem('quoteitems', JSON.stringify(quoteitems));
        loadItems();
    });
	...
	$(document).on("change", '.rtax1', function () {
		...
		item_id = row.attr('data-item-id');
		quoteitems[item_id].row.tax_method = current_tax_method;
		localStorage.setItem('quoteitems', JSON.stringify(quoteitems));
		loadItems();
	});
	...
	$(document).on("change", '.rtax2', function () {
		...
		quoteitems[item_id].tax_rate = current_tax;
		localStorage.setItem('quoteitems', JSON.stringify(quoteitems));
		loadItems();
	});
    ...
    var old_received;
     $(document).on("focus", '.received', function () {
        ...
        item_id = row.attr('data-item-id');
        if (new_received > quoteitems[item_id].row.qty) {
            $(this).val(old_received);
            bootbox.alert(lang.unexpected_value);
            return;
        }
        ...
        quoteitems[item_id].row.unit_received = new_received;
        quoteitems[item_id].row.received = qty_received;
        localStorage.setItem('quoteitems', JSON.stringify(quoteitems));
        loadItems();
    });
    ...
    $(document).on("change", '.rucost', function () {
        ...
        item_id = row.attr('data-item-id');
        quoteitems[item_id].row.real_unit_cost = current_ucost;
        localStorage.setItem('quoteitems', JSON.stringify(quoteitems));
        loadItems();
    });
    ...
    $(document).on("change", '.rdiscount', function () {
        ...
        item_id = row.attr('data-item-id');
        quoteitems[item_id].row.discount = current_discount;
        localStorage.setItem('quoteitems', JSON.stringify(quoteitems));
        loadItems();
    });    
    /* --------------------------
     * Edit Row Cost Method
     ...
     $(document).on("focus", '.rcost', function () {
        ...
        item_id = row.attr('data-item-id');
        quoteitems[item_id].row.cost = new_cost;
        localStorage.setItem('quoteitems', JSON.stringify(quoteitems));
        loadItems();
    });
...
function loadItems() {
    ...
    if (localStorage.getItem('quoteitems')) {
        ...
        if (site.settings.tax2 != 0) {
            if (potax2 = localStorage.getItem('quotetax2')) {
                $.each(tax_rates, function () {
                    if (this.id == potax2) {
                        if (this.type == 2) {
  ...
  var item_id = item.item_id;
    ...
    if (quoteitems[item_id]) {
		bootbox.confirm('This item is already added. Do you want to add it again?', function (result) {
			...
			quoteitems[item_id].row.qty = new_qty;
			quoteitems[item_id].row.batch_no = '';
			quoteitems[item_id].row.mfg = '';
			quoteitems[item_id].order = new Date().getTime();
			localStorage.setItem('quoteitems', JSON.stringify(quoteitems));
			loadItems();
			return true;
			}
