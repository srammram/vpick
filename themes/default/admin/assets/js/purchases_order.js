...
if (podiscount = localStorage.getItem('po_discount')) {
    ...
    $('#powarehouse').select2("val", powarehouse);
        ...
        $('#ponote').redactor({
            ...
            changeCallback: function (e) {
                var v = this.get();
                localStorage.setItem('po_note', v);
            }
        });
        ...
        if (posupplier = localStorage.getItem('po_supplier')) {
...
$('#podiscount').focus(function () {
    ...
    $(document).on("change", '.rdiscount', function () {
        ...
        item_id = row.attr('data-item-id');
        po_items[item_id].row.discount = current_discount;
        localStorage.setItem('po_items', JSON.stringify(po_items));
        loadItems();
    });       
    /* --------------------------
...
function loadItems() {
    ...
    if (localStorage.getItem('po_items')) {
        ...
        order_discount = 0;
        total_discount = 0;
        $("#purchasesorderTable tbody").empty();
		
        po_items = JSON.parse(localStorage.getItem('po_items'));

		sortedItems = po_items;
       // sortedItems = (site.settings.item_addition == 1) ? _.sortBy(po_items, function(o){return [parseInt(o.order)];}) : po_items;
        ...
        $.each(sortedItems, function () {			
           ...
           /* if (po_edit) {
            ...
            newTr.html(tr_html);
            newTr.prependTo("#purchasesorderTable");
        ...
        if (site.settings.product_discount == 1) {
            tfoot += '<th colspan="2" class="text-right">'+formatMoney(product_discount)+'</th>';
        }
        if (site.settings.tax1 == 1) {
            tfoot += '<th colspan="2"  class="text-right">'+formatMoney(product_tax)+'</th>';
        }
        tfoot += '<th class="text-right">'+formatMoney(total)+'</th><th class="text-center"><i class="fa fa-trash-o" style="opacity:0.5; filter:alpha(opacity=50);"></i></th></tr>';
        $('#purchasesorderTable tfoot').html(tfoot);
 ...
 * @returns {Boolean}
 ---------------------------- */
 function add_purchase_order_item(item) {
    if (count == 1) {
        po_items = {};
        if ($('#posupplier').val()) {
