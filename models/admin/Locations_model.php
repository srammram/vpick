...
class Locations_model extends CI_Model
{
    ...
    public function __construct()
    {
   	...
   	/*### Daily*/
	function add_daily($data, $slot_array, $is_default, $taxi_type, $countryCode){
		if($is_default == 1){
	...
	/*### Rental*/
	function add_rental($data, $is_default, $taxi_type, $package_name, $countryCode){
		if($is_default == 1){
			$this->db->where('is_default', $is_default);
			$this->db->where('taxi_type', $taxi_type);
	...
	/*### Outstation*/
	function add_outstation($data, $is_default, $taxi_type, $countryCode){
		if($is_default == 1){
			$this->db->where('is_default', $is_default);
			$this->db->where('taxi_type', $taxi_type);
			$this->db->where('is_country', $countryCode);
