...
class Heatmap extends MY_Controller
{
	...
	function available_cityride()
    {
		...
		$this->site->users_logs($countryCode,$this->session->userdata('user_id'), $this->getUserIpAddr, json_encode($_POST), $_SERVER['REQUEST_URI']);
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('heatmap/available_cityride'), 'page' => lang('City Ride - Cab Available')));
        $meta = array('page_title' => lang('Live Tracking'), 'bc' => $bc);    
	...
	function search_heatmap()
    {
        ...
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('heatmap/search_heatmap'), 'page' => lang('City Ride - Cab Available')));
