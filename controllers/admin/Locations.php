...
class Locations extends MY_Controller
{
    ...
    function daily($action=false){
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('daily')));
        $meta = array('page_title' => lang('daily'), 'bc' => $bc);
    ...
    function getDailyFares(){
        ...
        $this->datatables
			...
			$edit = "<a href='" . admin_url('locations/edit_daily/$1') . "' data-toggle='tooltip'  data-original-title='' aria-describedby='tooltip' title='Click here to full details'  ><i class='fa fa-pencil-square-o' aria-hidden='true'  style='color:#656464; font-size:18px'></i></a>";
			
			$delete = "<a href='" . admin_url('welcome/delete/daily_fare/$1') . "' data-toggle='tooltip'  data-original-title='' aria-describedby='tooltip' title='Delete'  ><i class='fa fa-trash' style='color:#656464; font-size:18px'></i></a>";
    ...
    function add_daily(){
        ...
        if ($this->form_validation->run() == true) {
			...
			if($this->input->post('city_id') == 0){
				...
				if($check_daily == 1){
					$this->session->set_flashdata('error', lang("already_exit_taxi_type_in_same_city"));
					admin_redirect('locations/add_daily');
				}
			}
			
			if($this->input->post('is_base') == 0 && $this->input->post('is_peak') == 0 && $this->input->post('is_night') == 0){
				$this->session->set_flashdata('error', lang("fare time not selected. please selected any one."));
				admin_redirect('locations/add_daily');
			}
        ...
        if ($this->form_validation->run() == true && $this->locations_model->add_daily($data, $slot_array, $is_default, $taxi_type, $countryCode)){
            ...
            $this->session->set_flashdata('message', lang("daily_added"));
            admin_redirect('locations/daily');
        } else {
			$this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('location/daily'), 'page' => lang('daily')), array('link' => '#', 'page' => lang('add_daily')));
            $meta = array('page_title' => lang('add_daily'), 'bc' => $bc);
    ...
    function edit_daily($id){
        ...
        if ($this->form_validation->run() == true && $this->locations_model->update_daily($id, $data, $slot_array, $countryCode)) { //check to see if we are updateing the customer
            $this->session->set_flashdata('message', lang("daily_updated"));
            admin_redirect("locations/daily");
        } else {
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
    ...
    function getRentalFares(){
        ...
        $this->datatables
			...
			$edit = "<a href='" . admin_url('locations/edit_rental/$1') . "' data-toggle='tooltip'  data-original-title='' aria-describedby='tooltip' title='Click here to full details'  ><i class='fa fa-pencil-square-o' aria-hidden='true'  style='color:#656464; font-size:18px'></i></a>";
			$delete = "<a href='" . admin_url('welcome/delete/rental_fare/$1') . "' data-toggle='tooltip'  data-original-title='' aria-describedby='tooltip' title='Delete'  ><i class='fa fa-trash' style='color:#656464; font-size:18px'></i></a>";
    ...
    function add_rental(){
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
				...
				'day_allowance' => $this->input->post('day_allowance'),
				'overnight_allowance' => $this->input->post('overnight_allowance'),
                'status' => 1,
				'is_default' => $is_default,
				'created_on' => date('Y-m-d H:i:s')
            );
...
$this->data['commoncountry'] = $this->site->getcountryCodeID($countryCode);
    ...
    function getOutstationFares(){
        ...
        $this->datatables
			...
			$edit = "<a href='" . admin_url('locations/edit_outstation/$1') . "' data-toggle='tooltip'  data-original-title='' aria-describedby='tooltip' title='Click here to full details'  ><i class='fa fa-pencil-square-o' aria-hidden='true'  style='color:#656464; font-size:18px'></i></a>";
			$delete = "<a href='" . admin_url('welcome/delete/outstation_fare/$1') . "' data-toggle='tooltip'  data-original-title='' aria-describedby='tooltip' title='Delete'  ><i class='fa fa-trash' style='color:#656464; font-size:18px'></i></a>";
